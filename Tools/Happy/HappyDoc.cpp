...
BYTE CHappyDoc::NewUploadPatchFiles(LPCTSTR strSourcePath)
{
	...
	CFile fileLocal;
	if(fileLocal.Open(m_strZippedPatchFile, CFile::modeRead))
	{
		m_dwSizeZippedPatch = (DWORD)fileLocal.GetLength();
		fileLocal.Close();
	}
	else
		return FALSE;
...
{
	if(!ZipPatchFile(strTargetPath)) return FALSE;
...
BOOL CHappyDoc::ZipPatchFile(LPCTSTR strTargetPath)
{
	/*int errclose = ZIP_OK;
	int err = 0;

	LPPATCHFILE pPatchFile;
	...
	/*strZipFile = strTargetPath;
	strZipFile += _T("\\GamePatch_%d%d%d.zip");

	CTime t = CTime::GetCurrentTime().GetTime();
	m_strZippedPatchFile.Format(strZipFile, t.GetYear(), t.GetMonth(), t.GetDay());
	strZipFile = m_strZippedPatchFile;
	m_strZippedPatchFileOnly.Format("GamePatch_%d%d%d.zip", t.GetYear(), t.GetMonth(), t.GetDay());
	...
	for(DWORD i=0; i<m_vPatchFiles.size(); i++)
	{
		pPatchFile = m_vPatchFiles[i];
		...
		if(!pPatchFile->m_strPath.IsEmpty())
		{
			pPatchFile->m_strPath = _T("\\") + pPatchFile->m_strPath; 
			pPatchFile->m_strPath.Replace("\\\\", "");
			strDirInZip = pPatchFile->m_strPath;
			strDirInZip += _T("\\");
			strDirInZip += pPatchFile->m_strFileName;
		}
		else
			strDirInZip = pPatchFile->m_strFileName;
		...
		if(err == ZIP_OK)
		{
			...
			if(fin != NULL)
			{
				...
				fileSizeTotal += total_read;
				m_pPatchProgress->SetProgress(BYTE((float)fileSizeTotal/this->m_dwPatchSize*100));
	...
	return (err == ZIP_OK && errclose == ZIP_OK);*/
return false;
}
