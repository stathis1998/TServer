...
void CTPlayer::SendCS_CHARINFO_ACK(DWORD dwTick,
	...
	BYTE bSecureCreated = FALSE;
	BYTE bSecureDisabled = FALSE;
	...
	LPTSECURE pSECURE = GetSecureCode();
	if (pSECURE) {
		...
		bSecureDisabled = pSECURE->m_bDisabled;
	...
	CString strTajm;
	strTajm.Format("%s %s", m_bAM == TRUE ? _T("AM") : _T("PM"), strTime);
...
#ifndef BOW_COMPILE_MODE
	FLOAT fD = _AtlModule.wVCount ? (100.0f * _AtlModule.wVCount) / FLOAT(_AtlModule.wVCount + _AtlModule.wDCount) : 0.0f;
	FLOAT fC = _AtlModule.wDCount ? (100.0f * _AtlModule.wDCount) / FLOAT(_AtlModule.wVCount + _AtlModule.wDCount) : 0.0f;
	FLOAT fDTotal = (100.0f * _AtlModule.wTotalCountV) / FLOAT(_AtlModule.wTotalCountV + _AtlModule.wTotalCountD);
	FLOAT fCTotal = (100.0f * _AtlModule.wTotalCountD) / FLOAT(_AtlModule.wTotalCountV + _AtlModule.wTotalCountD);

	CString strWarMessage;
	CString strStage = "No Penalty";
	if (_AtlModule.m_bStage)
		strStage.Format("%d", _AtlModule.m_bStage);

	strWarMessage.Format("Kingdom Balancing in Iberia\n Valorian: %.1f%%\n Derion: %.1f%%\n Current Stage: %s", fD, fC, strStage);
	SendCS_CHAT_ACK(CHAT_WORLD, 0, _AtlModule.GetSvrMsg(NAME_OPERATOR), _AtlModule.BuildNetString(NAME_NULL, strWarMessage));
#endif
...
void CTPlayer::SendCS_ENTER_ACK(CTPlayer* pPlayer,
	...
	MAPBOWPLAYER::iterator itPlayer = _AtlModule.m_mapBOWPLAYER.find(pPlayer->m_dwID);
	if (itPlayer != _AtlModule.m_mapBOWPLAYER.end()) {
		(*pMSG)
			<< (*itPlayer).second->m_bCountry
			<< BYTE(TCONTRY_N);
	} else {
		(*pMSG)
			<< pPlayer->m_bCountry
...
void CTPlayer::SendCS_MOVE_ACK(DWORD dwCharID,
	...
	try {
		...
		_AtlModule.LogEvent("MOVE CRASH!");
	}
}
...
void CTPlayer::SendCS_ADDSPOLECNIKMON_ACK(CTCompanion* pMON,
																					BYTE bNewMember,
																					DWORD dwTick) {
	if (!pMON)
		return;
	try {
		...
		pMSG->SetID(CS_ADDSPOLECNIKMON_ACK)
			<< pMON->m_dwHostID
			<< pMON->m_dwID
			<< pMON->m_pMON->m_wID
			<< pMON->m_wPetID
		...
		_AtlModule.LogSmartly("Exception in SendCS_ADDSPOLECNIKMON_ACK.");
	}
}
...
void CTPlayer::SendCS_DELSPOLECNIKMON_ACK(DWORD dwHostID,
																					DWORD dwMonID,
																					BYTE bExitMap) {
	try {
		CPacket* pMSG = new CPacket();

		pMSG->SetID(CS_DELSPOLECNIKMON_ACK)
			<< dwHostID
			<< dwMonID
			<< bExitMap;
		...
		_AtlModule.LogSmartly("Exception in SendCS_DELSPOLECNIKMON_ACK.");
	}
}
...
void CTPlayer::SendCS_MONACTION_ACK(DWORD dwMonID,
	...
	try {
		...
		_AtlModule.LogEvent("MONACTION FAIL");
	}
}
...
void CTPlayer::SendCS_SKILLBUY_ACK(BYTE bRet, WORD wSkillID, BYTE bLevel, DWORD Tick) {
	CPacket* pMsg = new CPacket();
...
void CTPlayer::SendCS_SKILLUSE_ACK(BYTE bResult,
	...
	try {
		...
		_AtlModule.LogEvent("SKILL USE CRASH");
	}
}
...
void CTPlayer::SendCS_GUILDLOCALLIST_ACK(BYTE bBOWStatus, BYTE bBOWWinner, DWORD dwNextBOW, BYTE bBRStatus, DWORD dwNextBR, BYTE bNextBRType) {
	CPacket* pMSG = new CPacket();
	...
	for (DWORD ll = 0; ll < _AtlModule.m_vCastle.size(); ll++) {
		...
		BYTE bStatus = pCastle->m_bStatus == BS_BATTLE && !pCastle->m_bOccupied ? BS_BATTLE : (pCastle->m_bStatus == BS_PEACE || (pCastle->m_bStatus == BS_BATTLE && pCastle->m_bOccupied)) ? BS_PEACE : BS_NORMAL;
		...
		for (itLOCAL = pCastle->m_mapLocal.begin(); itLOCAL != pCastle->m_mapLocal.end(); itLOCAL++) {
			BYTE bStatus = (*itLOCAL).second->m_bStatus == BS_BATTLE && !(*itLOCAL).second->m_bOccupied ? BS_BATTLE : ((*itLOCAL).second->m_bStatus == BS_PEACE || ((*itLOCAL).second->m_bStatus == BS_BATTLE && (*itLOCAL).second->m_bOccupied)) ? BS_PEACE : BS_NORMAL;
			(*pMSG)
	...
	for (DWORD ms = 0; ms < _AtlModule.m_vMission.size(); ms++) {
		...
		BYTE bStatus = pMission->m_bStatus == BS_BATTLE && !pMission->m_bOccupied ? BS_BATTLE : (pMission->m_bStatus == BS_PEACE || (pMission->m_bStatus == BS_BATTLE && pMission->m_bOccupied)) ? BS_PEACE : BS_NORMAL;
	...
	for (DWORD jk = 0; jk < _AtlModule.m_vSkygarden.size(); jk++) {
		...
		BYTE bStatus = pSkyGarden->m_bStatus == BS_BATTLE && !pSkyGarden->m_bOccupied ? BS_BATTLE : (pSkyGarden->m_bStatus == BS_PEACE || (pSkyGarden->m_bStatus == BS_BATTLE && pSkyGarden->m_bOccupied)) ? BS_PEACE : BS_NORMAL;
...
void CTPlayer::SendCS_GETREWARDS_ACK(BYTE bKind, WORD m_wUseValue) {
	CPacket* pMSG = new CPacket();

	if (bKind == IK_SPECIALBOX) {
		MAPSPECIALBOX::iterator itBOX = _AtlModule.m_mapSpecialBox.find(m_wUseValue);
		if (itBOX == _AtlModule.m_mapSpecialBox.end())
			return;

		BYTE bCount = 0;
		for (DWORD cg = 0; cg < (*itBOX).second.size(); cg++) {
			if ((*itBOX).second[cg]->m_bClass != m_bClass && (*itBOX).second[cg]->m_bClass != TCLASS_COUNT)
				continue;
			bCount++;
		}

		pMSG->SetID(CS_GETREWARDS_ACK)
			<< (WORD) bCount;
		...
		for (DWORD cg = 0; cg < (*itBOX).second.size(); cg++) {
			if ((*itBOX).second[cg]->m_bClass == m_bClass || (*itBOX).second[cg]->m_bClass == TCLASS_COUNT)
				(*itBOX).second[cg]->m_pITEM->WrapPacketClient(pMSG, m_dwID);
		}
	...
	{
		MAPCASHGAMBLE::iterator itCG = _AtlModule.m_mapCashGameble.find(m_wUseValue);
		if (itCG == _AtlModule.m_mapCashGameble.end())
			return;

		LPVTCASHGAMBLE pVTCASHGAMBLE = (*itCG).second;

		pMSG->SetID(CS_GETREWARDS_ACK)
			<< (WORD) pVTCASHGAMBLE->size();

		LPTCASHGAMBLE pCG = NULL;
		for (DWORD cg = 0; cg < pVTCASHGAMBLE->size(); cg++) {
			pCG = pVTCASHGAMBLE->at(cg);
			pCG->m_pItem->WrapPacketClient(pMSG, m_dwID);
		}
...
void CTPlayer::SendCS_TITLEGAIN_ACK(WORD wTitleID, BYTE bStart) {
#ifndef BOW_COMPILE_MODE
	CPacket* pMsg = new CPacket();

	pMsg->SetID(CS_TITLEGAIN_ACK)
...
#ifdef SKYGARDEN
void CTPlayer::SendCS_SKYGARDENOCCUPY_ACK(BYTE bType, WORD wID, BYTE bCountry) {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_SKYGARDENOCCUPY_ACK)
		<< bType
		<< wID
		<< bCountry;
...
void CTPlayer::SendCS_ITEMUPGRADE_ACK(BYTE bResult, BYTE bTargetInven, BYTE bTargetItemID, BYTE bGrade, BYTE bGem, BYTE bGradeEffect, WORD wColor, WORD wMoggItemID) {
	CPacket* pMsg = new CPacket();
...
void CTPlayer::SendCS_PETEFFECTCHANGE_ACK(BYTE bResult, BYTE bEffect) {
	CPacket* pMsg = new CPacket();

	pMsg->SetID(CS_PETEFFECTCHANGE_ACK)
		<< bResult
		<< bEffect;
...
void CTPlayer::SendCS_TITLELIST_ACK() {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_TITLELIST_ACK)
		<< BYTE(m_mapTTITLE.size());
...
void CTPlayer::SendCS_TITLERESET_ACK() {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_TITLERESET_ACK);
...
void CTPlayer::SendCS_SYSTEMMSG_ACK(BYTE bType,
	...
	case SM_SKYGARDEN_NORMAL:
	case SM_CASTLE_START:
	case SM_BATTLE_START:
	case SM_SKYGARDEN_START:
		break;
	...
	case SM_SKYGARDEN_START_ALARM:
	case SM_CASTLE_END_ALARM:
	case SM_BATTLE_END_ALARM:
	case SM_SKYGARDEN_END_ALARM:
...
void CTPlayer::SendCS_LEAVESKYGARDEN_ACK() {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_LEAVESKYGARDEN_ACK);

	Say(pMsg);
}

void CTPlayer::SendCS_SKYGARDEN_OCCUPY_LEFT_ACK(BYTE bCountry) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_SKYGARDEN_OCCUPY_LEFT_ACK)
		<< bCountry;

	Say(pMSG);
}

void CTPlayer::SendCS_SKYGARDEN_OCCUPY_CENTER_ACK(BYTE bCountry) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_SKYGARDEN_OCCUPY_CENTER_ACK)
		<< bCountry;

	Say(pMSG);
}

void CTPlayer::SendCS_SKYGARDEN_OCCUPY_RIGHT_ACK(BYTE bCountry) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_SKYGARDEN_OCCUPY_RIGHT_ACK)
		<< bCountry;
...
void CTPlayer::SendCS_TOURNAMENTENTER_ACK(CTMap* pMap) {
	...
	CPacket* pMSG = new CPacket();
	this->DeletePositiveMaintainSkill();
...
void CTPlayer::SendCS_SETTIME_ACK(DWORD dwHour, DWORD dwMinute) {
	CString strTime;

	strTime.Format("%s %02d : %02d", dwHour <= 12 ? "AM" : "PM", dwHour, dwMinute);
...
void CTPlayer::SendCS_SOULLOTTERY_TRIGGER_ACK(DWORD dwNumber,
	...
	pMSG->SetID(CS_SOULLOTTERY_TRIGGER_ACK)
		<< dwNumber
		<< dwMoney
		<< bRet;
...
void CTPlayer::SendCS_SC_CREATECODE_ACK(BYTE bResult) {
	CPacket* pMSG = new CPacket();

	pMSG->SetID(CS_SC_CREATECODE_ACK)
		<< bResult;

	Say(pMSG);
}

void CTPlayer::SendCS_SC_CHANGECODE_ACK(BYTE bResult) {
	CPacket* pMSG = new CPacket();

	BYTE bTries = 0;
	if (m_pSECURE)
		bTries = m_pSECURE->m_bTries;

	pMSG->SetID(CS_SC_CHANGECODE_ACK)
...
void CTPlayer::SendCS_SC_DISABLECODE_ACK(BYTE bResult) {
	CPacket* pMSG = new CPacket();
	...
	pMSG->SetID(CS_SC_DISABLECODE_ACK)
...
void CTPlayer::SendCS_SC_ENABLECODE_ACK(BYTE bResult) {
	CPacket* pMSG = new CPacket();

	pMSG->SetID(CS_SC_ENABLECODE_ACK)
		<< bResult;

	Say(pMSG);
}
void CTPlayer::SendCS_SENDSADDLE_REQ(DWORD m_dwSaddleID, __time64_t tEndTime, BYTE m_bType, BOOL bOpenUI) {
	CPacket* pMSG = new CPacket();

	pMSG->SetID(CS_SENDSADDLE_REQ)
		<< m_dwSaddleID
		<< m_bType
		<< tEndTime
		<< bOpenUI;
...
void CTPlayer::SendCS_SC_UNLOCK_ACK(BYTE bResult) {
	CPacket* pMSG = new CPacket();

	BYTE bTries = 0;
	if (m_pSECURE) {
		bTries = m_pSECURE->m_bTries;
		if (bTries >= MAX_SECURITY_TRIES) {
			SendCS_SC_CHECKCODE_ACK(TSC_ACK_TOOMANYWRONGATTEMPTS);
			return;
		}
	}

	pMSG->SetID(CS_SC_UNLOCK_ACK)
...
void CTPlayer::SendCS_SC_CHECKCODE_ACK(BYTE bResult) {
	CPacket* pMSG = new CPacket();

	BYTE bTries = 0;
	if (m_pSECURE) {
		if (m_pSECURE->m_bTries >= MAX_SECURITY_TRIES)
			bResult = TSC_ACK_TOOMANYWRONGATTEMPTS;

		bTries = m_pSECURE->m_bTries;
	}

	pMSG->SetID(CS_SC_CHECKCODE_ACK)
...
void CTPlayer::SendCS_SC_GETLOCKTIME_ACK(BYTE bHours, BYTE bMinutes) {
	CPacket* pMSG = new CPacket();

	pMSG->SetID(CS_SC_GETLOCKTIME_ACK)
		<< bHours
		<< bMinutes;
...
void CTPlayer::SendCS_COMPANIONLIST_ACK() {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_COMPANIONLIST_ACK)
		<< BYTE(m_mapComp.size());
	...
	for (jirk = m_mapComp.begin(); jirk != m_mapComp.end(); jirk++) {
		(*pMSG)
			<< (*jirk).second->m_bSlot
			<< (*jirk).second->m_dwMonID
			<< (*jirk).second->m_strName
			<< (*jirk).second->m_dwExp
			<< (*jirk).second->m_dwNextExp
			<< (*jirk).second->m_bLevel
			<< (*jirk).second->m_wLife
			<< (*jirk).second->m_bSPoints
			<< (*jirk).second->m_bEffect
			<< (*jirk).second->m_wSTR
			<< (*jirk).second->m_wDEX
			<< (*jirk).second->m_wCON
			<< (*jirk).second->m_wINT
		...
		for (BYTE l = 0; l < PETITEMS_COUNT; ++l) {
			(*pMSG)
				<< (*jirk).second->m_wItemID[l]
				<< (*jirk).second->m_dEndTime[l];
		}

		FLOAT fBonusValue = 0.0f;
		MAPTCOMPBONUS::iterator jirker = _AtlModule.m_mapCompBonus.find((*jirk).second->m_bBonusID);
		if (jirker != _AtlModule.m_mapCompBonus.end())

			fBonusValue = (*jirker).second->m_fBase + ((*jirker).second->m_fMultiplier * ((*jirk).second->m_bLevel - 1));

		(*pMSG)
			<< (*jirk).second->m_dwTick
			<< (*jirk).second->m_bBonusID
			<< fBonusValue;
	}

	Say(pMSG);
}
void CTPlayer::SendCS_COMPANIONUPDATE_REQ(BYTE bSlot, BYTE bSTR, BYTE bDEX, BYTE bCON, BYTE bINT, BYTE bWIS, BYTE bMEN, BYTE bStatusPoints) {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_COMPANIONUPDATE_REQ)
		<< bSlot
		<< bStatusPoints
		<< bSTR
		<< bDEX
...
void CTPlayer::SendCS_COMPANIONLUPDATE_REQ(BYTE bSlot, BYTE bLevel, DWORD dwExp, BYTE bSPoints, DWORD dwNextExp, FLOAT fBonusValue) {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_COMPANIONLUPDATE_REQ)
		<< bSlot
		<< bLevel
		<< dwExp
		<< bSPoints
...
void CTPlayer::SendCS_UPDATECOMPANIONBYITEM_REQ(DWORD dwCharID, BYTE bCompanionSlot, DWORD wLife, DWORD dwExp) {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_UPDATECOMPANIONBYITEM_REQ)
		<< dwCharID
		<< bCompanionSlot
...
void CTPlayer::SendCS_UPDATECOMPANIONBYSYSTEM_REQ(BYTE bCompanionSlot, DWORD dwLife, DWORD dwExp) {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_UPDATECOMPANIONBYSYSTEM_REQ)
		<< bCompanionSlot
...
void CTPlayer::SendCS_UPDATESPAWNEDCOMPANION_REQ(BYTE bCompanionSlot) {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_UPDATESPAWNEDCOMPANION_REQ)
		<< bCompanionSlot;
...
void CTPlayer::SendCS_CREATECOMPANION_ACK(BYTE bResult, BYTE bSlot) {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_CREATECOMPANION_ACK)
		<< bResult
		<< bSlot;
...
void CTPlayer::SendCS_UPDATECOMPANIONITEMS_REQ(BYTE bCompanionSlot, BYTE bSlot, WORD wItemID, __time64_t dEndTime) {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_UPDATECOMPANIONITEMS_REQ)
		<< bCompanionSlot
...
void CTPlayer::SendCS_UPDATECOMPANIONBONUS_REQ(BYTE bCompanionSlot) {
	CPacket* pMsg = new CPacket();

	FLOAT fBonusValue = 0.0f;
	MAPTCOMP::iterator jirk = m_mapComp.find(bCompanionSlot);
	if (jirk != m_mapComp.end()) {
		MAPTCOMPBONUS::iterator jirker = _AtlModule.m_mapCompBonus.find((*jirk).second->m_bBonusID);
		if (jirker != _AtlModule.m_mapCompBonus.end())
	...
	pMsg->SetID(CS_UPDATECOMPANIONBONUS_REQ)
		<< bCompanionSlot
...
void CTPlayer::SendCS_UPDATEMEDALS_REQ(DWORD dwMedals) {
	CPacket* pMsg = new CPacket();
	pMsg->SetID(CS_UPDATEMEDALS_REQ)
		<< dwMedals;

	Say(pMsg);
}

void CTPlayer::SendCS_EXITTOURNAMETMAINTAIN_ACK() {
	CPacket* pMSG = new CPacket();

	pMSG->SetID(CS_EXITTOURNAMETMAINTAIN_ACK)
		<< (BYTE) m_vMaintainSkill.size();

	for (BYTE is = 0; is < (BYTE) m_vMaintainSkill.size(); is++) {
		(*pMSG)
			<< m_vMaintainSkill[is]->GetID()
			<< m_vMaintainSkill[is]->m_bLevel
			<< m_vMaintainSkill[is]->GetRemainTick(_AtlModule.m_dwTick)
			<< m_vMaintainSkill[is]->m_dwAttackID
			<< m_vMaintainSkill[is]->m_bAttackType
			<< m_vMaintainSkill[is]->m_dwHostID
			<< m_vMaintainSkill[is]->m_bHostType
...
void CTPlayer::SendCS_EXITTOURNAMETHOTKEY_ACK() {
	CPacket* pMSG = new CPacket();

	pMSG->SetID(CS_EXITTOURNAMETHOTKEY_ACK)
		<< BYTE(m_mapHotkeyInven.size());
...
void CTPlayer::SendCS_BOWTIMEUPDATE_ACK(BYTE bStatus, DWORD dwSecond, BYTE bDefugelPoints, BYTE bCraxionPoints) {
	CPacket* pMSG = new CPacket();

	pMSG->SetID(CS_BOWTIMEUPDATE_ACK)
		<< bStatus
		<< dwSecond
		<< bDefugelPoints
		<< bCraxionPoints;
...
void CTPlayer::SendCS_BOWNOTIFY_ACK(const CString& strMessage) {
	CPacket* pMSG = new CPacket();

	pMSG->SetID(CS_BOWNOTIFY_ACK)
		<< strMessage;

	Say(pMSG);
}

void CTPlayer::SendCS_RELEASEFORBOW_REQ() {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_RELEASEFORBOW_REQ);

	Say(pMSG);
}

void CTPlayer::SendCS_REGISTERBOW_ACK(BYTE bResult, DWORD dwTick) {
	CPacket* pMSG = new CPacket();

	pMSG->SetID(CS_REGISTERBOW_ACK)
		<< bResult
		<< dwTick;
...
void CTPlayer::SendCS_CANCELBOWQUEUE_ACK(BYTE bResult, DWORD dwTick) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_CANCELBOWQUEUE_ACK)
		<< bResult
		<< dwTick;
...
void CTPlayer::SendCS_NOTIFYNONQUEUEDPLAYER_ACK() {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_NOTIFYNONQUEUEDPLAYER_ACK);

	Say(pMSG);
}

void CTPlayer::SendCS_PINGMEASUREMENT_ACK(DWORD dwTick) {
	CPacket* pMSG = new CPacket();

	pMSG->SetID(CS_PINGMEASUREMENT_ACK)
		<< dwTick;
...
void CTPlayer::SendCS_UPDATEBONUSPOINT_ACK(DWORD dwBB) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_UPDATEBONUSPOINT_ACK)
		<< dwBB;

	Say(pMSG);
}

void CTPlayer::SendCS_BOWKILLNOTIFY_ACK(CString strAttack) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_BOWKILLNOTIFY_ACK)
		<< strAttack;

	Say(pMSG);
}

void CTPlayer::SendCS_BOWRANKING_ACK(const VBOWRANKING& vPlayer, const MAPBOWGRANKING& mapGuild) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_BOWRANKING_ACK)
		<< (BYTE) vPlayer.size();

	for (BYTE i = 0; i < vPlayer.size(); ++i) {
		LPBOWRANKING pRANK = vPlayer[i];
		(*pMSG)
			<< pRANK->m_dwCharID
			<< pRANK->m_strName
			<< pRANK->m_bCountry
	...
	for (MAPBOWGRANKING::const_iterator itGuild = mapGuild.begin(); itGuild != mapGuild.end(); ++itGuild) {
		(*pMSG)
			<< (*itGuild).second->m_dwGuildID
			<< (*itGuild).second->m_strName
			<< (*itGuild).second->m_bCountry
			<< (*itGuild).second->m_dwBP
			<< (*itGuild).second->m_dwSP;
...
void CTPlayer::SendCS_LEAVEBATTLEFIELD_ACK(BYTE bResult) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_LEAVEBATTLEFIELD_ACK)
		<< bResult;

	Say(pMSG);
}
#endif
void CTPlayer::SendCS_BRTIMEUPDATE_ACK(BYTE bStatus, DWORD dwSecond, BYTE bType) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_BRTIMEUPDATE_ACK)
		<< bStatus
		<< dwSecond
		<< bType;
...
void CTPlayer::SendCS_ADDTOBRQUEUE_ACK(BYTE bResult, DWORD dwTick) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_ADDTOBRQUEUE_ACK)
		<< bResult
		<< dwTick;
...
void CTPlayer::SendCS_BRTEAMMATEADD_ACK(BYTE bResult, CString strNAME) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_BRTEAMMATEADD_ACK)
		<< bResult
		<< strNAME;
...
void CTPlayer::SendCS_UPDATEBRTEAM_ACK(const std::vector<BRPREMADEPLAYER>& vTeam, BYTE bTeamReady) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_UPDATEBRTEAM_ACK)
		<< bTeamReady
		<< (BYTE) vTeam.size();

	for (BYTE i = 0; i < (BYTE) vTeam.size(); ++i) {
		(*pMSG)
			<< vTeam[i].m_strName
			<< vTeam[i].m_bReady;
	}
...
#ifdef BR_COMPILE_MODE
void CTPlayer::SendCS_UPDATEBRMATE_ACK(const MAPBRPLAYER& mapPlayer) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_UPDATEBRMATE_ACK)
		<< (BYTE) mapPlayer.size(); //It should be 3, but you never know, and we don't want buffer over/underflow

	for (MAPBRPLAYER::const_iterator itPlayer = mapPlayer.begin(); itPlayer != mapPlayer.end(); ++itPlayer) {
		(*pMSG)
			<< (*itPlayer).second->m_dwCharID
			<< (*itPlayer).second->m_strNAME
			<< (*itPlayer).second->m_bClass
			<< (*itPlayer).second->m_bLife
			<< (*itPlayer).second->m_wKills
			<< (*itPlayer).second->m_bDead;
	}

	Say(pMSG);
}

void CTPlayer::SendCS_INITBRTRANKING_ACK(const MAPBRTEAM& mapTeam) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_INITBRTRANKING_ACK)
		<< (BYTE) mapTeam.size();

	for (MAPBRTEAM::const_iterator itTeam = mapTeam.begin(); itTeam != mapTeam.end(); ++itTeam) {
		(*pMSG)
			<< (*itTeam).second->m_dwTeamID
			<< (BYTE) (*itTeam).second->m_mapBRPLAYER.size(); //Again should be 3, but it's 4Story
...
void CTPlayer::SendCS_UPDATEBRTPLAYER_ACK(DWORD dwTeamID, DWORD dwCharID, WORD wKills, BYTE bLife) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_UPDATEBRTPLAYER_ACK)
		<< dwTeamID
		<< dwCharID
		<< wKills
		<< bLife;
...
void CTPlayer::SendCS_UPDATEREMAININGPLAYER_ACK(BYTE bRemaining) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_UPDATEREMAININGPLAYER_ACK)
		<< bRemaining;
...
void CTPlayer::SendCS_CUSTOMCLOAK_LIST_ACK() {
	CPacket* pMSG = new CPacket();
	...
	pMSG->SetID(CS_CUSTOMCLOAK_LIST_ACK)
		<< WORD(m_mapTCUSTOMCLOAK.size());

	for (auto Cloak : m_mapTCUSTOMCLOAK)
...
void CTPlayer::SendCS_CUSTOMCLOAK_CHANGE_ACK(DWORD dwCharID, WORD wID, BYTE bRet) {
	CPacket* pMSG = new CPacket();




	pMSG->SetID(CS_CUSTOMCLOAK_CHANGE_ACK)
		<< dwCharID
		<< wID
		<< bRet;
...
void CTPlayer::SendCS_GUILDSKILLACTION_ACK(DWORD dwCharID, DWORD dwKEY, BYTE Action, WORD Param[]) {
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_GUILDSKILLACTION_ACK)
		<< Action
		<< BYTE(sizeof(Param) / sizeof(WORD));





	for (BYTE i = 0; i < sizeof(Param) / sizeof(WORD); ++i)
		(*pMSG) << Param[i];

	Say(pMSG);
}

void CTPlayer::SendCS_GUILDSKILLUPDATE_ACK() {
#ifdef BOW_COMPILE_MODE
	return;
#endif
	...
	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_GUILDSKILLUPDATE_ACK)
		<< (BYTE) _AtlModule.GetGuildSkill(m_dwID, m_bGuildDuty, m_dwGuildID).size();

	for (auto Temp : _AtlModule.m_mapGuildSkill) {
		auto find = m_mapTSKILL.find(Temp.first);
		if (find != m_mapTSKILL.end()) {
			delete find->second;
			m_mapTSKILL.erase(find->first);
		}
	}

	for (auto Skill : _AtlModule.GetGuildSkill(m_dwID, m_bGuildDuty, m_dwGuildID)) {
		(*pMSG)
			<< Skill.m_wSkillID
			<< Skill.m_bLevel
			<< Skill.m_EndTime;

		m_mapGuildSkill.insert(std::make_pair(Skill.m_wSkillID, Skill));

		CTSkill* pSkill = new CTSkill();
		pSkill->m_pTSKILL = _AtlModule.FindTSkill(Skill.m_wSkillID);
		pSkill->m_bLevel = Skill.m_bLevel;

		auto Cooldown = _AtlModule.m_mapGuildCooldown.find(m_dwGuildID);
		if (Cooldown != _AtlModule.m_mapGuildCooldown.end()) {
			for (const auto& itSkill : (*Cooldown).second.m_vSkill) {
				if (Skill.m_wSkillID == itSkill.first) {
					if (INT(GetTickCount() - itSkill.second) < 0)
						continue;

					pSkill->m_dwUseTick = GetTickCount() - itSkill.second;
				}
			}
		}

		m_mapTSKILL.insert(MAPTSKILL::value_type(Skill.m_wSkillID, pSkill));
		RemainSkill(pSkill, 0);

		SendCS_SKILLBUY_ACK(SKILL_SUCCESS, Skill.m_wSkillID, Skill.m_bLevel, pSkill->m_dwUseTick);
	}

	Say(pMSG);
}

void CTPlayer::SendCS_ARENAINFO_ACK(LPTARENA Arena) {
	BOOL FirstTeam = Arena->m_mapFighter[0].begin() != Arena->m_mapFighter[0].end();
	BOOL SecondTeam = Arena->m_mapFighter[1].begin() != Arena->m_mapFighter[1].end();

	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_ARENAINFO_ACK)
		<< FirstTeam
		<< SecondTeam;

	if (FirstTeam) {
		auto Fighter = Arena->m_mapFighter[0].begin()->second;
		for (const auto& Fighters : Arena->m_mapFighter[0])
			if (Fighters.second->GetPartyChiefID() == Fighters.second->m_dwID)

				Fighter = Fighters.second;
		...
		(*pMSG)
			...
			<< Fighter->m_bLevel
			<< Fighter->m_bCountry
			<< BOOL(Fighter->GetPartyID() == Arena->m_wWinPartyID)
			<< Arena->m_bWinCount;
	}

	if (SecondTeam) {
		auto Fighter = Arena->m_mapFighter[1].begin()->second;
		for (const auto& Fighters : Arena->m_mapFighter[1])
			if (Fighters.second->GetPartyChiefID() == Fighters.second->m_dwID)
				Fighter = Fighters.second;
	...
	(*pMSG)
		<< (BYTE) _AtlModule.m_ArenaQueue[Arena->m_wID].size();

	for (const auto& Queue : _AtlModule.m_ArenaQueue[Arena->m_wID]) {
		if (Queue.begin() == Queue.end())
			continue;

		auto Fighter = Queue.begin()->second;
		if (!Fighter)
			continue;

		for (const auto& Fighters : Queue)
			if (Fighters.second->GetPartyChiefID() == Fighters.second->m_dwID)
				Fighter = Fighters.second;
...
void CTPlayer::SendCS_ARENARANKING_ACK() {

	const auto& Arena1 = _AtlModule.m_mapArena.find(1);
	const auto& Arena2 = _AtlModule.m_mapArena.find(2);

	CPacket* pMSG = new CPacket();
	pMSG->SetID(CS_ARENARANKING_ACK)



		<< (BYTE) Arena1->second->m_TopPlayer.size()
		<< Arena1->second->m_MaxWinCount
		<< (BYTE) Arena2->second->m_TopPlayer.size()
		<< Arena2->second->m_MaxWinCount;

	for (const auto& Players1 : Arena1->second->m_TopPlayer) {
		(*pMSG)
			<< Players1.m_Name
			<< Players1.m_Level
			<< Players1.m_Class;
	...
	for (const auto& Players2 : Arena2->second->m_TopPlayer) {
		(*pMSG)
			<< Players2.m_Name
			<< Players2.m_Level
			<< Players2.m_Class;
