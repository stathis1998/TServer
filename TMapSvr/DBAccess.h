...
public:
	...
	WHERE dwCharID = ?"))
		...
		COLUMN_ENTRY(m_bStatLevel)
		COLUMN_ENTRY(m_bStatPoint)
		COLUMN_ENTRY(m_dwStatExp)
		END_COL()
...
public:
	...
	DEF_QUERY(CTBLPet, _T("SELECT \
		...
		BEGIN_COLUMN(3)
		COLUMN_ENTRY(m_wID)
		COLUMN_ENTRY(m_wDefMonID)
		COLUMN_ENTRY(m_wUpgMonID)
		END_COL()
};


class CTBLSaddle: public CSqlQuery {
public:
	DWORD m_dwUserID;
	DWORD m_dwItemID;
	...
	DEF_QUERY(CTBLSaddle, _T("{CALL TGetMountSaddle(?,?,?,?)}"))
...
class CSPSetSaddle: public CSqlQuery {
public:
	DWORD m_dwUserID;
	DWORD m_dwItemID;
	TIMESTAMP_STRUCT m_dEndTime;
	...
	DEF_QUERY(CSPSetSaddle, _T("{CALL TSetMountSaddle(?,?,?,?)}"))
...
class CSPDelSaddle: public CSqlQuery {
public:
	DWORD m_dwUserID;

	DEF_QUERY(CSPDelSaddle, _T("{CALL TDelMountSaddle(?)}"))
...
class CTBLSkygardenTable: public CSqlQuery {
public:
	WORD m_wID;
	BYTE m_bCountry;
	TIMESTAMP_STRUCT m_dateWarTime;

	DEF_QUERY(CTBLSkygardenTable, _T("SELECT \
	wID, \
	bCountry, \
	dateWarTime \
	FROM TSKYGARDENTABLE"))
...
public:
	...
	WORD m_wValorianBossDefend;
	WORD m_wValorianBossAttack;
	WORD m_wDerionBossDefned;
	WORD m_wDerionBossAttack;
	WORD m_wMiddleSpawnID;
	WORD m_wRightSpawnID;
	WORD m_wLeftSpawnID;
	DWORD m_dwLSwitchID;
	DWORD m_dwRSwitchID;
	DWORD m_dwCSwitchID;
	...
	FROM TBATTLEZONECHART"))
		...
		COLUMN_ENTRY(m_wValorianBossDefend)
		COLUMN_ENTRY(m_wValorianBossAttack)
		COLUMN_ENTRY(m_wDerionBossDefned)
		COLUMN_ENTRY(m_wDerionBossAttack)
		COLUMN_ENTRY(m_wMiddleSpawnID)
		COLUMN_ENTRY(m_wRightSpawnID)
		COLUMN_ENTRY(m_wLeftSpawnID)
		END_COL()
};
...
class CTBLTitleChart: public CSqlQuery {
public:
	WORD m_wTitleID;
	WORD m_wCategory;
	BYTE m_bKind;
	...
	DEF_QUERY(CTBLTitleChart, _T("SELECT \
	wTitleID, \
	wCategory, \
	bKind, \
	dwRequirement \
	FROM TTITLECHART"))
		...
		BEGIN_COLUMN(4)
		COLUMN_ENTRY(m_wTitleID)
		COLUMN_ENTRY(m_wCategory)
		COLUMN_ENTRY(m_bKind)
		COLUMN_ENTRY(m_dwRequirement)
		END_COL()
};
...
class CTBLGemGradeChart: public CSqlQuery {
public:
	BYTE m_bGem;
	BYTE m_bProb;

	DEF_QUERY(CTBLGemGradeChart, _T("SELECT \
	bGem, \
	bProb \
	FROM TGEMGRADECHART"))
...
class CTBLInvenTournametChart: public CSqlQuery {
public:
	BYTE m_bInvenID;
	WORD m_wItemID;
	BYTE m_bELD;

	DEF_QUERY(CTBLInvenTournametChart, _T("SELECT \
	bInvenID, \
	wItemID, \
	bELD \
	FROM TINVENTOURNAMENTCHART"))
...
class CTBLItemTournamentChart: public CSqlQuery {
public:
	BYTE m_bClass;
	BYTE m_bInvenID;
	TITEMQUERY m_item;

	DEF_QUERY(CTBLItemTournamentChart, _T("SELECT \
	bClass, \
	bInvenID, \
	bItemID, \
	wItemID, \
	...
	FROM TITEMTOURNAMENTCHART"))
		...
		COLUMN_ENTRY(m_item.m_bGem)
		COLUMN_ENTRY(m_item.m_wMoggItemID)
		END_COL()
};
...
class CTBLTitle: public CSqlQuery {
public:
	DWORD m_dwCharID;
	WORD m_wTitleID;
	BYTE m_bSelected;

	DEF_QUERY(CTBLTitle, _T("SELECT \
	dwCharID, \
	wTitleID, \
	bSelected \
	FROM TTITLETABLE \
...
public:
	...
	DEF_QUERY(CTBLPostTable, _T("SELECT dwPostID, bRead, timeRecv, szSender, bType, szTitle, bContain FROM TPOSTTABLE WHERE dwCharID = ? AND dwPostID <= ? ORDER BY dwPostID DESC"));
...
public:
	...
	DEF_QUERY(CTBLRecallMon, _T("SELECT dwID, wMonID, wPetID, dwATTR, bLevel, dwHP, dwMP, bSkillLevel, wPosX, wPosY, wPosZ, dwTime, bEffect FROM TRECALLMONTABLE WHERE dwOwnerID=?"))
...
public:
	...
	DEF_QUERY(CTBLPetTable, _T("SELECT wPetID, szName, timeUse, bEffect FROM TPETTABLE WHERE dwUserID = ?"))
...
class CTBLCompanionTable: public CSqlQuery {
public:
	BYTE m_bSlot;
	DWORD m_dwCharID;
	...
	DEF_QUERY(CTBLCompanionTable, _T("SELECT bSlot, dwMonID, bLevel, strName, dwExp, wLife, bStatusPoints, bEffect, wSTR, wDEX, wCON, wINT, wWIS, wMEN, wBonusID FROM TCOMPANIONTABLE WHERE dwCharID = ?"))

		BEGIN_COLUMN(15)
		COLUMN_ENTRY(m_bSlot)
		COLUMN_ENTRY(m_dwMonID)
		COLUMN_ENTRY(m_bLevel)
		COLUMN_ENTRY_STR(m_szName)
		COLUMN_ENTRY(m_dwExp)
		COLUMN_ENTRY(m_wLife)
		COLUMN_ENTRY(m_bSPoints)
		COLUMN_ENTRY(m_bEffect)
		COLUMN_ENTRY(m_wSTR)
		COLUMN_ENTRY(m_wDEX)
		COLUMN_ENTRY(m_wCON)
...
class CTBLCompanionItemTable: public CSqlQuery {
public:
	DWORD m_dwCharID;
	BYTE m_bSlot;

	WORD m_wItemID[PETITEMS_COUNT];
	TIMESTAMP_STRUCT m_timeUse[PETITEMS_COUNT];
	DWORD m_dwTick;

	DEF_QUERY(CTBLCompanionItemTable, _T("SELECT bSlot, wFirstItemID, wSecondItemID, dFirstEndTime, dSecondEndTime, dwTick FROM TCOMPANIONITEMTABLE WHERE dwCharID = ?"))

		BEGIN_COLUMN(6)
		COLUMN_ENTRY(m_bSlot)
		COLUMN_ENTRY(m_wItemID[0])
		COLUMN_ENTRY(m_wItemID[1])
		COLUMN_ENTRY(m_timeUse[0])
		COLUMN_ENTRY(m_timeUse[1])
		COLUMN_ENTRY(m_dwTick)
		END_COL()
		BEGIN_PARAM(1)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
...
class CTBLCompanionBonusChart: public CSqlQuery {
public:
	BYTE m_bBonusID;
	FLOAT m_fBase;
	FLOAT m_fMultiplier;

	DEF_QUERY(CTBLCompanionBonusChart, _T("SELECT \
	bBonusID, \
	fBase, \
	fLevelMultiplier \
	FROM TCOMPANIONBONUSCHART"))

		BEGIN_COLUMN(3)
		COLUMN_ENTRY(m_bBonusID)
		COLUMN_ENTRY(m_fBase)
...
class CTBLBOWItemChart: public CSqlQuery {
public:
	BYTE m_bClass;
	BYTE m_bInvenID;
	WORD m_wMinItemID;
	WORD m_wMaxItemID;
	WORD m_wDependOnItemID;
	BYTE m_bCount;

	DEF_QUERY(CTBLBOWItemChart, _T("SELECT \
	bClass, \
	bInvenID, \
	wMinItemID, \
	wMaxItemID, \
	...
	FROM TBOWITEMCHART"))
		...
		COLUMN_ENTRY(m_bClass)
		COLUMN_ENTRY(m_bInvenID)
		COLUMN_ENTRY(m_wMinItemID)
		COLUMN_ENTRY(m_wMaxItemID)
		COLUMN_ENTRY(m_wDependOnItemID)
		COLUMN_ENTRY(m_bCount)
		END_COL()
};

class CTBLBOWBonusItems: public CSqlQuery {
public:
	BYTE m_bItemID;
	WORD m_wItemID;
	WORD m_wPrice;

	DEF_QUERY(CTBLBOWBonusItems, _T("SELECT \
	bItemID, \
	wItemID, \
	wPrice \
	FROM TBOWBONUSITEMCHART"))

		BEGIN_COLUMN(3)
		COLUMN_ENTRY(m_bItemID)
		COLUMN_ENTRY(m_wItemID)
		COLUMN_ENTRY(m_wPrice)
		END_COL()
};

class CTBLPremiumSkillChart: public CSqlQuery {
public:
	BYTE m_bType;
	WORD m_wSkillID;
	WORD m_wMedals;

	DEF_QUERY(CTBLPremiumSkillChart, _T("SELECT \
	bType, \
	wSkillID, \
	wMedals \
	FROM TPREMIUMSKILLCHART"))

		BEGIN_COLUMN(3)
		COLUMN_ENTRY(m_bType)
		COLUMN_ENTRY(m_wSkillID)
		COLUMN_ENTRY(m_wMedals)
		END_COL()
};

class CTBLBRSpawnPos: public CSqlQuery {
public:
	WORD m_wMapID;
	FLOAT m_fPosX;
	FLOAT m_fPosY;
	...
	DEF_QUERY(CTBLBRSpawnPos, _T("SELECT \
	wMapID, \
	fPosX, \
	fPosY, \
	fPosZ \
	FROM TBRSPAWNPOSCHART"))
...
class CTBLBRSuppliesChart: public CSqlQuery {
public:
	DWORD m_dwTick;
	...
	DEF_QUERY(CTBLBRSuppliesChart, _T("SELECT \
	dwTick, \
	wItemID, \
	bCount \
	FROM TBRSUPPLIESCHART \
	ORDER BY dwTick ASC"));
...
class CTBLBattleRankChart: public CSqlQuery {
public:
	BYTE m_bRank;
	DWORD m_dwPoints;

	DEF_QUERY(CTBLBattleRankChart, _T("SELECT \
	bRank, \
	dwPoint \
	FROM TBATTLERANKCHART \
	ORDER BY bRank ASC"));

	BEGIN_COLUMN(2)
		COLUMN_ENTRY(m_bRank)
		COLUMN_ENTRY(m_dwPoints)
		END_COL()
};

class CTBLSpecialBoxChart: public CSqlQuery {
public:
	WORD m_wID;
	WORD m_wGroup;
	WORD m_wUseTime;
	...
	DEF_QUERY(CTBLSpecialBoxChart, _T("SELECT \
	wID, \
	wGroup, \
	wItemID, \
	bLevel, \
...
class CTBLGuildSkillChart: public CSqlQuery {
public:
	BYTE m_bType;
	WORD m_wSkillID;

	DEF_QUERY(CTBLGuildSkillChart, _T("SELECT \
	bType, \
	wSkillID \
	FROM TGUILDSKILLCHART"));
...
class CTBLGuildSkillDuty: public CSqlQuery {
public:
	DWORD m_dwGuildID;
	BYTE m_bLevel;
	WORD m_wSkillID;
	...
	DEF_QUERY(CTBLGuildSkillDuty, _T("SELECT \
	bLevel, \
	tEndTime \
	FROM TGUILDMASTERSKILLTABLE \
	WHERE wSkillID = ? AND dwGuildID = ?"));
...
class CTBLGuildSkillMember: public CSqlQuery {
public:
	DWORD m_dwCharID;
	BYTE m_bLevel;
	WORD m_wSkillID;
	...
	DEF_QUERY(CTBLGuildSkillMember, _T("SELECT \
	wSkillID, \
	bLevel, \
	tEndTime \
	FROM TGUILDMEMBERSKILLTABLE \
	WHERE dwCharID = ?"));
...
class CTBLAccessoryMagic: public CSqlQuery {
public:
	BYTE m_ID;
	BYTE m_MagicID;
	WORD m_MinValue;
	...
	DEF_QUERY(CTBLAccessoryMagic, _T("SELECT \
	ID, \
	MagicID, \
	MinValue, \
	MaxValue \
	FROM ACCESSORYMAGICTABLE"));

	BEGIN_COLUMN(4)
		COLUMN_ENTRY(m_ID)
		COLUMN_ENTRY(m_MagicID)
		COLUMN_ENTRY(m_MinValue)
		COLUMN_ENTRY(m_MaxValue)
		END_COL()
};
...
class CSPGetPlayTime: public CSqlQuery {
public:
	DWORD m_dwPlayTime;
	DWORD m_dwCharID;

	DEF_QUERY(CSPGetPlayTime, _T("{CALL TGetPlayTime(?,?)}"))

		BEGIN_PARAM(2)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_dwPlayTime)
		END_PARAM()
};
...
/*class CSPCheckMapChar : public CSqlQuery
{
public:
	int m_nRET;
	DWORD m_dwCharID;
...
DEF_QUERY(CSPCheckMapChar, _T("{? = CALL TCheckMapChar(?,?,?)}"))
...
public:
	...
	DEF_QUERY(CSPCashItemGet, _T("{? = CALL TCashItemGet(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"))
		...
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_item.m_bGem)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_item.m_wMoggItemID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwUserID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
...
public:
	...
	DEF_QUERY(CSPSaveRecallMon, _T("{? = CALL TSaveRecallMon(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"))
		...
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bEffect)
...
class CSPCheckConnect: public CSqlQuery {
public:
	int m_nRET;
	...
	DEF_QUERY(CSPCheckConnect, _T("{ ? = CALL TCheckConnect(?,?)}"))
...
class CSPClearTitle: public CSqlQuery {
public:
	DWORD m_dwCharID;

	DEF_QUERY(CSPClearTitle, _T("{CALL TClearCharTitle(?)}"))
...
class CSPSaveTitle: public CSqlQuery {
public:
	DWORD m_dwCharID;
	WORD m_wTitleID;
	BYTE m_bSelected;

	DEF_QUERY(CSPSaveTitle, _T("{CALL TSaveCharTitle(?,?,?)}"))

		BEGIN_PARAM(3)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_wTitleID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bSelected)
		END_PARAM()
};
...
public:
	...
	BYTE m_StatLevel;
	BYTE m_StatPoint; // 3more right? ye
	DWORD m_StatExp;
	...
	DEF_QUERY(CSPSaveChar, _T("{ ? = CALL TSaveChar(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"))
		...
		PARAM_ENTRY(SQL_PARAM_INPUT, m_StatLevel)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_StatPoint)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_StatExp)
		END_PARAM()
};
...
public:
	...
	DEF_QUERY(CSPSaveItem, _T("{ ? = CALL TSaveItem(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"))
		...
		PARAM_ENTRY(SQL_PARAM_INPUT, m_item.m_bGem)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_item.m_wMoggItemID)
		END_PARAM()
};
...
class CSPSaveSkyGardenOccupy: public CSqlQuery {
public:
	WORD m_wID;
	BYTE m_bType;
	BYTE m_bCountry;

	DEF_QUERY(CSPSaveSkyGardenOccupy, _T("{CALL TSaveSkyGardenOccupy(?,?,?)}"))
...
class CSPSaveCharKill: public CSqlQuery {
public:
	DWORD m_dwKillerID;
	DWORD m_dwTargetID;

	DEF_QUERY(CSPSaveCharKill, _T("{CALL TSaveCharKill(?,?)}"))

		BEGIN_PARAM(2)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwKillerID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwTargetID)
		END_PARAM()
};
...
class CSPSaveCompanion: public CSqlQuery {
public:
	DWORD	m_dwCharID;
	BYTE    m_bSlot;
	...
	DWORD   m_dwMonID;
	BYTE    m_bLevel;
	TCHAR	m_strName[MAX_NAME + 1];
	DWORD   m_dwExp;
	DWORD   m_wLife;
	BYTE    m_bStatusPoints;
	BYTE    m_bEffect;
	BYTE    m_bSTR, m_bDEX, m_bCON, m_bINT, m_bWIS, m_bMEN;
	BYTE    m_bBonusID;
	WORD    m_wItemID[PETITEMS_COUNT];
	TIMESTAMP_STRUCT m_dEndTime[PETITEMS_COUNT];

	DEF_QUERY(CSPSaveCompanion, _T("{CALL TSaveCompanion(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"))
		...
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwMonID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bLevel)
		PARAM_ENTRY_STR(SQL_PARAM_INPUT, m_strName)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwExp)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_wLife)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bStatusPoints)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bEffect)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bSTR)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bDEX)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bCON)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bINT)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bWIS)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bMEN)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bBonusID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_wItemID[0])
		PARAM_ENTRY(SQL_PARAM_INPUT, m_wItemID[1])
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dEndTime[0])
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dEndTime[1])
		END_PARAM()
};
...
public:
	...
	BEGIN_PARAM(15)
		...
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_bContain)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwPostID)
		END_PARAM()
};
...
class CSPTCASH: public CSqlQuery {
public:
	DWORD	m_dwUserID;
	DWORD	m_dwCash;
	DWORD	m_dwBonus;
	BYTE	m_bType;
public:
	DEF_QUERY(CSPTCASH, _T("{ CALL TCASH(?,?,?,?) }"))

		BEGIN_PARAM(4)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwUserID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCash)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwBonus)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bType)
		END_PARAM()
};
...
class CTBLGETCASHAMOUNT: public CSqlQuery {
public:
	DWORD m_dwUserID;
	...
	DEF_QUERY(CTBLGETCASHAMOUNT, _T("SELECT \
	dwCash, \
	dwBonus \
	FROM TGLOBAL_GSP.dbo.TCASHTESTTABLE \
	WHERE dwUserID = ?"))

		BEGIN_COLUMN(2)
		COLUMN_ENTRY(m_dwCash)
		COLUMN_ENTRY(m_dwBonus)
		END_COL()
...
class CTBLSecureCode: public CSqlQuery {
public:
	DWORD m_dwUserID;
	TCHAR m_strCode[MAX_NAME + 1];
	BYTE m_bEnabled;
	BYTE m_bTries;
	DWORD m_dwLockTick;

	DEF_QUERY(CTBLSecureCode, _T("SELECT strSecurityCode, bEnabled, bTries, iLockTick FROM TGLOBAL_GSP.dbo.TSECURECODE WHERE dwUserID = ?"))

		BEGIN_COLUMN(4)
		COLUMN_ENTRY_STR(m_strCode)
		COLUMN_ENTRY(m_bEnabled)
		COLUMN_ENTRY(m_bTries)
		COLUMN_ENTRY(m_dwLockTick)
		END_COL()
...
class CSPSaveSecureCode: public CSqlQuery {
public:
	DWORD m_dwUserID;
	TCHAR m_strCode[MAX_NAME + 1];
	BYTE m_bTries;
...
public:
	DEF_QUERY(CSPSaveSecureCode, _T("{ CALL TSaveSecureCode(?,?,?,?,?) }"))

		BEGIN_PARAM(5)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwUserID)
		PARAM_ENTRY_STR(SQL_PARAM_INPUT, m_strCode)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bTries)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bEnabled)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwLockTick)
		END_PARAM()
};

class CSPSecureCodeStatus: public CSqlQuery {
public:
	DWORD m_dwUserID;
	BYTE m_bType;

public:
	DEF_QUERY(CSPSecureCodeStatus, _T("{ CALL TSecureCodeStatus(?,?) }"))
...
class CSPGetOnWarBalance: public CSqlQuery {
public:
	WORD dwValoCount;
	WORD dwDeroCount;

	DEF_QUERY(CSPGetOnWarBalance, _T("{CALL TGetOnWarBalance(?,?)}"))

		BEGIN_PARAM(2)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, dwValoCount)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, dwDeroCount)
		END_PARAM()
};
class CSPGetWarBalance: public CSqlQuery {
public:
	WORD dwValoCount;
	WORD dwDeroCount;

	DEF_QUERY(CSPGetWarBalance, _T("{CALL TGetWarBalance(?,?)}"))
...
class CSPDeleteCompanion: public CSqlQuery {
public:
	DWORD m_dwCharID;
	BYTE  m_bSlot;


	DEF_QUERY(CSPDeleteCompanion, _T("{CALL TDeleteCompanion(?,?)}"))
...
class CSPSaveLastCompanion: public CSqlQuery {
public:
	DWORD m_dwCharID;
	BYTE m_bCompanionSlot;


	DEF_QUERY(CSPSaveLastCompanion, _T("{CALL TSaveLastCompanion(?,?)}"))

		BEGIN_PARAM(2)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bCompanionSlot)
		END_PARAM()
};

class CSPGetLastCompanion: public CSqlQuery {
public:
	DWORD m_dwCharID;
	BYTE m_bCompanionSlot;


	DEF_QUERY(CSPGetLastCompanion, _T("{CALL TGetLastCompanion(?,?)}"))

		BEGIN_PARAM(2)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_bCompanionSlot)
		END_PARAM()
};

class CSPSaveMedals: public CSqlQuery {
public:
	DWORD m_dwCharID;
	DWORD m_dwMedals;

	DEF_QUERY(CSPSaveMedals, _T("{CALL TSaveMedals(?,?)}"))

		BEGIN_PARAM(2)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwMedals)
		END_PARAM()
};

class CSPGetMedals: public CSqlQuery {
public:
	DWORD m_dwCharID;
	DWORD m_dwMedals;

	DEF_QUERY(CSPGetMedals, _T("{CALL TGetMedals(?,?)}"))

		BEGIN_PARAM(2)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_dwMedals)
		END_PARAM()
};

class CSPClearBOWPlayers: public CSqlQuery {
public:
	DEF_QUERY(CSPClearBOWPlayers, _T("{ CALL TClearBOWPlayers}"))
};

class CSPGiveBOWReward: public CSqlQuery {
public:
	DWORD m_dwCharID;
	WORD m_wSP;
	WORD m_wItemID;
	...
	DEF_QUERY(CSPGiveBOWReward, _T("{CALL TGiveBOWReward(?,?,?,?,?,?)}"))

		BEGIN_PARAM(6)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_wSP)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_wItemID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bWinCountry)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bClass)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bRank)
		END_PARAM()
};

class CSPGiveTitle: public CSqlQuery {
public:
	DWORD m_dwCharID;
	WORD m_wTitleID;

	DEF_QUERY(CSPGiveTitle, _T("{CALL TGiveTitle(?,?)}"))
...
class CSPClearBRPlayers: public CSqlQuery {
public:
	DEF_QUERY(CSPClearBRPlayers, _T("{CALL TClearBRPlayers}"))
};

class CSPDeleteBRPlayer: public CSqlQuery {
public:
	DWORD m_dwCharID;

	DEF_QUERY(CSPDeleteBRPlayer, _T("{CALL TDeleteBRPlayer(?)}"))
...
class CSPGetRankPoint: public CSqlQuery {
public:
	DWORD m_dwCharID;
	DWORD m_dwRankPoint;

	DEF_QUERY(CSPGetRankPoint, _T("{CALL TGetBattleRankPoint(?,?)}"))

		BEGIN_PARAM(2)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_dwRankPoint)
		END_PARAM()
};

class CSPGiveBRReward: public CSqlQuery {
public:
	DWORD m_dwCharID;
	WORD m_wKills;
	WORD m_wItemID;
	...
	DEF_QUERY(CSPGiveBRReward, _T("{CALL TGiveBRReward(?,?,?,?,?,?,?)}"))

		BEGIN_PARAM(7)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_wKills)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_wItemID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bClass)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bRank)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_bIsWinner)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_wBonus)
		END_PARAM()
};

class CSPGiveBRTeamReward: public CSqlQuery {
public:
	DWORD m_dwCharID;
	WORD m_wItemID;

	DEF_QUERY(CSPGiveBRTeamReward, _T("{CALL TGiveBRTeamReward(?,?)}"))
...
class CSPGetPosition: public CSqlQuery {
public:
	DWORD m_dwCharID;
	WORD m_wMapID;
	FLOAT m_fPosX;
	...
	DEF_QUERY(CSPGetPosition, _T("{CALL TGetPosition(?,?,?,?,?)}"))

		BEGIN_PARAM(5)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_dwCharID)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_wMapID)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_fPosX)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_fPosY)
		PARAM_ENTRY(SQL_PARAM_OUTPUT, m_fPosZ)
		END_PARAM()
};

class CTBLCustomCloakTable: public CSqlQuery {
public:
	DWORD m_dwUserID;
	WORD m_wID;
	TIMESTAMP_STRUCT m_dDateCreate;

	DEF_QUERY(CTBLCustomCloakTable, _T("SELECT wID,dDateCreate FROM TCUSTOMCLOAKTABLE WHERE dwUserID = ?"))

		BEGIN_COLUMN(2)
		COLUMN_ENTRY(m_wID)
		COLUMN_ENTRY(m_dDateCreate)
		END_COL()
...
class CSPGetCustomCloak: public CSqlQuery {
public:
	int m_nRET;
	DWORD m_dwUserID;
	DWORD m_wID;

	DEF_QUERY(CSPGetCustomCloak, _T("{? = CALL TGetCustomCloak(?,?)}"))
...
class CSPSaveGuildSkill: public CSqlQuery {
public:
	...
	WORD m_SkillID;
	BYTE m_Level;
	TIMESTAMP_STRUCT m_EndTime;


	DEF_QUERY(CSPSaveGuildSkill, _T("{CALL TSaveGuildSkill(?,?,?,?,?,?)}"));

	BEGIN_PARAM(6)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_CharID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_GuildID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_Type)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_SkillID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_Level)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_EndTime)
		END_PARAM()
};
