...
void CTMonster::OnDie(DWORD dwAttackID, BYTE bObjectType, WORD wTempMonID) {
	...
	if (dwDamage) {
		...
		case OWNER_PRIVATE:
		{
			CTPlayer* pOwner = FindHost(m_dwKeeperID);
			if (pOwner && pOwner->m_bMain) {
				// Moose - enabling auto-loot for everyone
				if (_AtlModule.MonMoneyTake(pOwner, m_dwID)) {
					_AtlModule.MonItemTake(pOwner, m_dwID, INVALID_SLOT, INVEN_NULL, INVALID_SLOT);
				}

				BYTE bKinds[PETITEMS_COUNT] = { 0 };
				WORD wKinds = pOwner->GetPetItemKind();
				if (wKinds) {
					bKinds[0] = LOBYTE(wKinds);
					bKinds[1] = HIBYTE(wKinds);

					for (BYTE i = 0; i < PETITEMS_COUNT; ++i) {
						if (bKinds[i] == COLLECTION_KIND) {
							if (_AtlModule.MonMoneyTake(pOwner, m_dwID)) {
								_AtlModule.MonItemTake(pOwner, m_dwID, INVALID_SLOT, INVEN_NULL, INVALID_SLOT);
							}
						}

						if (bKinds[i] == FORTUNE_KIND)
							nAddItemDrop += 20 * 100;
					}
				}
			}
...
FLOAT CTMonster::GetTargetDistance() {
	...
	case OT_SELF:
		...
		break;
	case OT_COMPANION:
		pTarget = pHost->FindCompanion(m_dwTargetID);
		break;
	default:
		return fDis;
	}
...
#ifdef SKYGARDEN
	LPTLOCAL pSkygarden = _AtlModule.FindSkygarden(m_pSPAWN->m_pSPAWN->m_wLocalID);
	if (!pLocal && !pCastle && !pMission && !pSkygarden)
		return FALSE;
#else
...
#ifdef SKYGARDEN
	if (pSkygarden && pSkygarden->m_pZone->m_wMiddleSpawnID == m_pSPAWN->m_pSPAWN->m_wID) {
		CTMap* pMap = _AtlModule.FindTMap(DEFAULT_CHANNEL, 0, pSkygarden->m_pZone->m_wMapID);
		_AtlModule.PvPEvent(PVPE_GODMONKILL, NULL, dwAttackID);

		m_bCountry = (m_bCountry == TCONTRY_D) ? TCONTRY_C : TCONTRY_D;
		pSkygarden->m_bMiddleOwner = (pSkygarden->m_bMiddleOwner == CAMP_DEFEND) ? CAMP_ATTACK : CAMP_DEFEND;

		VPLAYER vPlayers;
		vPlayers.clear();

		pMap->GetMapPlayers(&vPlayers);

		for (DWORD i = 0; i < vPlayers.size(); i++) {
			CTPlayer* pChar = vPlayers[i];
			pChar->SendCS_SKYGARDEN_OCCUPY_CENTER_ACK(pSkygarden->m_bMiddleOwner);
		}
	}

	if (pSkygarden && pSkygarden->m_pZone->m_wLeftSpawnID == m_pSPAWN->m_pSPAWN->m_wID) {
		CTMap* pMap = _AtlModule.FindTMap(DEFAULT_CHANNEL, 0, pSkygarden->m_pZone->m_wMapID);
		_AtlModule.PvPEvent(PVPE_GODMONKILL, NULL, dwAttackID);

		m_bCountry = (m_bCountry == TCONTRY_D) ? TCONTRY_C : TCONTRY_D;
		pSkygarden->m_bLeftOwner = (pSkygarden->m_bLeftOwner == CAMP_DEFEND) ? CAMP_ATTACK : CAMP_DEFEND;
		...
		for (DWORD i = 0; i < vPlayers.size(); i++) {
			CTPlayer* pChar = vPlayers[i];
			pChar->SendCS_SKYGARDEN_OCCUPY_LEFT_ACK(pSkygarden->m_bLeftOwner);
		}
	}

	if (pSkygarden && pSkygarden->m_pZone->m_wRightSpawnID == m_pSPAWN->m_pSPAWN->m_wID) {
		CTMap* pMap = _AtlModule.FindTMap(DEFAULT_CHANNEL, 0, pSkygarden->m_pZone->m_wMapID);
		_AtlModule.PvPEvent(PVPE_GODMONKILL, NULL, dwAttackID);

		m_bCountry = (m_bCountry == TCONTRY_D) ? TCONTRY_C : TCONTRY_D;
		pSkygarden->m_bRightOwner = (pSkygarden->m_bRightOwner == CAMP_DEFEND) ? CAMP_ATTACK : CAMP_DEFEND;
		...
		for (DWORD i = 0; i < vPlayers.size(); i++) {
			CTPlayer* pChar = vPlayers[i];
			pChar->SendCS_SKYGARDEN_OCCUPY_RIGHT_ACK(pSkygarden->m_bRightOwner);
		}
	}

	if (pSkygarden && (pSkygarden->m_pZone->m_wDerionBossAttack == m_pSPAWN->m_pSPAWN->m_wID || pSkygarden->m_pZone->m_wDerionBossDefned == m_pSPAWN->m_pSPAWN->m_wID)) {
		pSkygarden->m_bOccupied = TRUE;
		pSkygarden->m_bStatus = BS_PEACE;

		_AtlModule.PvPEvent(PVPE_GODMONKILL, NULL, dwAttackID);
		_AtlModule.SendDM_SKYGARDENOCCUPY_REQ(pSkygarden->m_wLocal, OCCUPY_ACCEPT, TCONTRY_D);
	}

	if (pSkygarden && (pSkygarden->m_pZone->m_wValorianBossAttack == m_pSPAWN->m_pSPAWN->m_wID || pSkygarden->m_pZone->m_wValorianBossDefend == m_pSPAWN->m_pSPAWN->m_wID)) {
		pSkygarden->m_bOccupied = TRUE;
		pSkygarden->m_bStatus = BS_PEACE;

		_AtlModule.PvPEvent(PVPE_GODMONKILL, NULL, dwAttackID);
		_AtlModule.SendDM_SKYGARDENOCCUPY_REQ(pSkygarden->m_wLocal, OCCUPY_ACCEPT, TCONTRY_C);
	}
#endif
