...
class CTMapSvrModule : public CAtlServiceModuleT< CTMapSvrModule, IDS_SERVICENAME >, public TServerSystem
{
#ifdef CRASH_DEBUG_LOG
public:
	CLog* m_pDebugLog;
	std::set<DWORD> m_setChars;
#endif
...
public:
	...
	MAPTTNMTINVEN m_mapTTNMTINVEN;
	VTTNMTITEM m_vTTNMTITEM[TCLASS_COUNT];
	VPREMIUMSKILL m_vPREMIUMSKILL[PREMIUMSKILL_COUNT];
	VBATTLERANK m_vBATTLERANK;


#if defined(BOW_COMPILE_MODE) || defined(BR_COMPILE_MODE)
	VBOWITEM m_vBOWITEM[TCLASS_COUNT];
	MAPBOWBONUSITEM m_mapBOWBPITEM;
	MAPBOWPLAYER m_mapBOWPLAYER;
#endif
#ifdef BR_COMPILE_MODE
	VBRSUPPLIES m_vBRSUPPLIES;
	MAPBRTEAM m_mapBRTEAM;
#endif
	...
	std::map<WORD, GUILDSKILL> m_mapGuildSkill;
	VACCESSORYMAGIC m_vAccessoryMagic;
...
//	MAPTSECURE m_mapTSECURE;						// Securitysystem pool //SC
	...
	std::vector<MAPPLAYER> m_ArenaQueue[ARENA_COUNT];
	...
	MAPARENA		m_mapArena;
	MAPGUILDCOOLDOWN m_mapGuildCooldown;

	MAPDWORD		m_mapMeetingRoomTime;
#ifdef BR_COMPILE_MODE
	MAPBRSPAWNPOS   m_mapBRSPAWNPOS;
#endif
...
protected:
	MAPSAVEDPLAYER m_mapSAVEDPLAYER;
...
public:
	...
	DWORD* m_dwToggleableSkills;

#ifdef BR_COMPILE_MODE
	WORD m_wBRMapID;
	BYTE m_Mode;
	BYTE m_bRemainingPlayers;
	MAPDWORD m_mapLeftPlayer;
#endif
protected:
...
public:
	...
	std::vector<GUILDSKILL> GetGuildSkill(DWORD dwCharID, BYTE bDuty, DWORD dwGuildID);
	BYTE ChangeSwitch(
		CTMap *pMap,
		DWORD dwSwitchID,
		BYTE bOpen);

	void LogSmartly(CString strLogMessage, BOOL IsHackLog = FALSE);
...
void SafeTeleport(CTPlayer *pPlayer,
					  BYTE bChannel,
					  WORD wMapID,
					  FLOAT fPosX,
					  FLOAT fPosY,
	...
	void SetTournamentChar(CTPlayer * pPlayer);
...
#if defined(BOW_COMPILE_MODE) || defined(BR_COMPILE_MODE)
	void BOWNotify(const CString& strMSG);
	void ClearSavedPlayers();
	void ReceiveBOWTitle();
#endif
#ifdef BR_COMPILE_MODE
	LPBRPLAYER GetBRPlayer(DWORD dwCharID);
	LPBRTEAM GetBRTeam(DWORD dwCharID);
	LPBRPLAYER GetBRPlayerByTeamID(DWORD dwTeamID, DWORD dwCharID);
	BRRAWPOS GetBRRandomSpawnPos();
	void ClearBRTeam();

	void DeleteBRPlayer(DWORD dwCharID);
	void ClearBRPlayers();
	void OnBRPlayerDie(LPBRPLAYER pBRPLAYER);
	void ReceiveBRTitle();
	DWORD CalcWinningTeam();
#endif
	BOOL HasPlayerNoTarget(CTObjBase* pDEFEND);
	void OnFinishSkill(CPacket* buf);
	BYTE GetRankID(DWORD dwPoint);

	void SaveGuildSkill(const CTPlayer * pPlayer);
	////////////////////////////////////////////////
	...
	void SaveSecurecodeStats(CTPlayer * pPlayer, BYTE bType);
	void SaveCharKill(LPTRECORDSET pRec, DWORD dwKillerID);
	CTPlayer* FindPC(DWORD dwID);
	void MountOn(CTPlayer* pPlayer);
	LPPREMIUMSKILL FindPremiumSkill(WORD wSkillID);
...
protected:
	...
	DWORD OnMW_CREATESPOLECNIKMON_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_SPOLECNIKMONDEL_REQ(LPPACKETBUF pBUF);
	...
	DWORD OnMW_BOWTIMEUPDATE_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_ADDTOBOWQUEUE_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_CANCELBOWQUEUE_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_NOTIFYNONQUEUEDPLAYER_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_PREPAREFORBOW_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_RECALCCOUNTRYBALANCE_ACK(LPPACKETBUF pBUF);
#ifdef BOW_COMPILE_MODE
	DWORD OnMW_BOWCOMMANDEXEC_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_ENDBOWWAR_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_ADDBOWPLAYERS_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_RELEASESINGLEBOWPLAYER_REQ(LPPACKETBUF pBUF);
#endif

#ifdef BR_COMPILE_MODE
	DWORD OnMW_ADDBRTEAMS_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_BRCOMMANDEXEC_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_ENDBRWAR_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_RELEASESINGLEBRPLAYER_REQ(LPPACKETBUF pBUF);
#endif

	DWORD OnMW_BATTLEMODESTATUS_ACK(LPPACKETBUF pBUF);

	DWORD OnMW_ADDTOBRQUEUE_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_BRTIMEUPDATE_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_BRTEAMMATEADD_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_UPDATEBRTEAM_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_PREPAREFORBR_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_GUILDSKILLACTION_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_UPDATEGUILDCOOLDOWN_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_ADDCOOLDOWN_REQ(LPPACKETBUF pBUF);
	//Apex
...
protected:
	...
	DWORD OnCS_CUSTOMCLOAK_LIST_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_CUSTOMCLOAK_CHANGE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_REVIVAL_REQ( LPPACKETBUF pBUF);
	DWORD OnCS_ENTERLB_REQ( LPPACKETBUF pBUF);
	DWORD OnCS_CREATECOMPANION_REQ(LPPACKETBUF pBUF);
	...
	DWORD OnCS_GETREWARDS_REQ(LPPACKETBUF pBUF);
	...
	DWORD OnCS_DELSPOLECNIKMON_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_CHGMODERECALLMON_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_CHGMODESPOLECNIKMON_REQ(LPPACKETBUF pBUF);
	...
	DWORD OnCS_COMPANIONRECALL_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_PETEFFECTCHANGE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_PETRIDING_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_SPOLECNIKRECALL_REQ(LPPACKETBUF pBUF);
	...
	DWORD OnCS_COMPANIONCANCEL_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_USECOMPANIONITEM_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_COMPANIONUPGRADE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_DELETECOMPANION_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_HIDECOMPANION_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_DELETECOMPITEMS_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_CHANGECOMPANIONEFFECT_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_USECOMPANIONPOWDER_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_USECOMPRESET_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_USEMEDALITEM_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_VERIFYSESSION_ACK(LPPACKETBUF pBUF);
	DWORD OnCS_REGISTERBOW_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_CANCELBOWQUEUE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_CASHBOWRESPAWN_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_EXCHANGEBOWBP_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_BOWRANKING_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_LEAVEBATTLEFIELD_REQ(LPPACKETBUF pBUF);

	DWORD OnCS_PINGMEASUREMENT_REQ(LPPACKETBUF pBUF);

	DWORD OnCS_COMPANIONLUP_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_USEPETITEM_REQ(LPPACKETBUF pBUF);
	...
	//Securecodesystem
	DWORD OnCS_SC_CREATECODE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_SC_CHANGECODE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_SC_DISABLECODE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_SC_ENABLECODE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_SC_UNLOCK_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_SC_CHECKCODE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_SC_GETLOCKTIME_REQ(LPPACKETBUF pBUF);

	DWORD OnCS_REQUESTSADDLE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_CREATESADDLE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_DELETESADDLE_REQ(LPPACKETBUF pBUF);

	DWORD OnCS_ACTITEMSUSE_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_FINISHSKILL_ACK(LPPACKETBUF pBUF);

	DWORD OnCS_REGISTERBR_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_BRTEAMMATEADD_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_BRTEAMMATEADDRESULT_ACK(LPPACKETBUF pBUF);
	DWORD OnCS_BRTEAMMATEDEL_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_QUESTPOSEXEC_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_GUILDSKILLACTION_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_ARENAINFO_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_ARENARANKING_REQ(LPPACKETBUF pBUF);
	DWORD OnCS_VOTEFORBRMAP_REQ(LPPACKETBUF pBUF);
#ifdef BR_COMPILE_MODE
	DWORD OnCS_DONATELIFE_REQ(LPPACKETBUF pBUF);
#endif
	DWORD OnCS_FINISHCOMPANIONTRANSFER_ACK(LPPACKETBUF pBUF);


	DWORD OnCS_CMTELEPORTBATTLEMODE_REQ(LPPACKETBUF pBUF);
public:
	...
	void SendMW_CREATESPOLECNIKMON_ACK(
		DWORD dwCharID,
		DWORD dwKEY,
		DWORD dwMonID,
		WORD wMon,
	...
	void SendMW_SPOLECNIKMONDEL_ACK(
		DWORD dwCharID,
		DWORD dwKey,
		DWORD dwMonID,
		BYTE bForever=TRUE);
	...
	void SendMW_GUILDCABINETLIST_ACK(
		DWORD dwCharID,
		DWORD dwKEY);
	void SendMW_GUILDLOCALLIST_REQ(DWORD dwCharID, DWORD dwKEY);
	...
	void SendMW_ADDTOBOWQUEUE_REQ(
		DWORD dwCharID, 
		DWORD dwKEY);

	void SendMW_CANCELBOWQUEUE_REQ(
		DWORD dwCharID,
		DWORD dwKEY);

	void SendMW_BATTLEMODESTATUS_REQ(DWORD dwCharID, DWORD dwKEY);

	void SendMW_CMTELEPORTBATTLEMODE_REQ(DWORD dwCharID, DWORD dwKEY, BYTE bType);

	void SendMW_BOWPOINTSUPDATE_REQ(BYTE bCountry);
	void SendMW_LEAVEBATTLEFIELD_REQ(DWORD dwCharID, DWORD dwKEY);

	void SendMW_ADDTOBRQUEUE_REQ(DWORD dwCharID, DWORD dwKEY, BYTE bOnlyReady);
	void SendMW_BRTEAMMATEADD_REQ(DWORD dwCharID, DWORD dwKEY, CString strNAME);
	void SendMW_BRTEAMMATEDEL_REQ(DWORD dwCharID, DWORD dwKEY, CString strNAME);
	void SendMW_BRTEAMMATEADDRESULT_ACK(DWORD dwCharID, DWORD dwKEY, BYTE bResult, CString strNAME);
	void SendMW_VOTEFORBRMAP_REQ(DWORD dwCharID, DWORD dwKEY, CString strMap, BYTE Mode);



	void SendMW_GUILDSKILLACTION_REQ(DWORD dwCharID, DWORD dwKEY, BYTE Action, WORD Param[]);

	void SendMW_UPDATEGUILDCOOLDOWN_ACK(DWORD dwGuildID, WORD wSkillID, BYTE bLevel, BOOL DidRenew, BOOL DidUse);
	//Apex
...
public:
	...
	DECLARE_LIBID(LIBID_TBRSvrLib)
	DECLARE_REGISTRY_APPID_RESOURCEID(IDR_TBRSVR, "{cc38b6aa-b0ff-11e7-abc4-cec278b6b50a}")
#elif defined(BOW_COMPILE_MODE)
	DECLARE_LIBID(LIBID_TBoWSvrLib)
	DECLARE_REGISTRY_APPID_RESOURCEID(IDR_TBOWSVR, "{9f20367a-b0ff-11e7-abc4-cec278b6b50a}")
#else
	DECLARE_LIBID(LIBID_TMapSvrLib)
	//DECLARE_REGISTRY_APPID_RESOURCEID(IDR_TMAPSVR, "{98fae29a-b0ff-11e7-abc4-cec278b6b50a}")
