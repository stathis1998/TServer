// CSHandler.cpp : C/S Protocol Handler �Լ��� �����Դϴ�.

#include "StdAfx.h"
#include <SvrInc.h>
#include "TMapSvrModule.h"

#define COMPANION_MAX_LEVEL (BYTE(20))

void SplitMoney(INT64 dwPrice, DWORD* pbRune, DWORD* pbLuna, DWORD* pbCron) {
...
DWORD CTMapSvrModule::OnCT_USERKICKOUT_ACK(LPPACKETBUF pBUF) {
	...
	SSLogEvent(5, "KICK REGISTERED!");

	strUser.MakeUpper();

	strLog.Format("TCONTROL - FUNCTION DEBUG: %s", strUser);
	LogEvent(strLog);
...
DWORD CTMapSvrModule::OnCS_CONNECT_REQ(LPPACKETBUF pBUF) {
	...
	llChecksum2 = wVersion * dwUserID + dwKEY * dwID;
	...
	BYTE bRet = FALSE;
	/*DEFINE_QUERY(&m_db, CSPCheckMapChar)
	query->m_dwUserID = dwUserID;
	query->m_dwCharID = dwID;
	query->m_dwKEY = dwKEY;
	...
	if (bRet) {
		m_pDebugSocket->LogInvalidPacket(inet_ntoa(pPlayer->m_addr.sin_addr));
		return EC_SESSION_INVALIDCHAR;
	}
	...
	if (itCHAR != m_mapPLAYER.end()) {
		if (pPlayer == (*itCHAR).second) {
			LogEvent("Trying to crash my server, huh? greetings from %s (%s)", (LPCTSTR) pPlayer->m_strNAME, inet_ntoa(pPlayer->m_addr.sin_addr));
			return EC_SESSION_INVALIDCHAR;
		}
		...
		if ((*itCHAR).second->m_bExit) {
			...
			pPlayer->m_bCheckedSession = TRUE;
	...
	LPBRPLAYER pBRPlayer = GetBRPlayer(dwID);
	if (pBRPlayer)
		pBRPlayer->m_dwKEY = dwKEY;
#endif
...
DWORD CTMapSvrModule::OnCS_MOVE_REQ(LPPACKETBUF pBUF) {
	if (!pBUF->m_pSESSION) {
		LogEvent("No Session move"); //i go afk 2 mins
		return EC_NOERROR;
	}
	...
	pPlayer->m_fSpeed = fSpeed;

	if (pPlayer->m_fSpeed > 3.40f && !_AtlModule.IsOperator(pPlayer->m_dwID)) {
		CString output;
		output.Format("SpeedHack - CharID %d name %s speed %f", pPlayer->m_dwID, pPlayer->m_strNAME, pPlayer->m_fSpeed);
		LogSmartly(output);
	...
	if (fPosX != pPlayer->m_fPosX ||
			fPosY != pPlayer->m_fPosY ||
			fPosZ != pPlayer->m_fPosZ) {
		pPlayer->m_dwRealMoveTick = m_dwTick;
	}
#endif
...
DWORD CTMapSvrModule::OnCS_JUMP_REQ(LPPACKETBUF pBUF) {
	...
	case OT_RECALL:
		...
		break;
	case OT_COMPANION:
		pTOBJ = pPlayer->FindCompanion(dwID);
		break;
	default:
...
DWORD CTMapSvrModule::OnCS_MONMOVE_REQ(LPPACKETBUF pBUF) {
	...
	for (WORD wC = 0; wC < wMonCount; wC++) {
		...
		case OT_COMPANION:	pMON = pPlayer->FindCompanion(dwMonID); break;
		}
		if (!pMON ||
				!pMON->m_pMAP ||
		...
		if (pMON->m_bType == OT_MON) {
			if (pMON->m_bMode == MT_BATTLE) {
				...
				else if (pMON->m_bTargetType == OT_COMPANION)
					pTarget = pPlayer->FindSpolecnikMon(pMON->m_dwTargetID);
				else
...
DWORD CTMapSvrModule::OnCS_ACTION_REQ(LPPACKETBUF pBUF) {
	...
	case OT_SELF:
		...
		break;
	case OT_COMPANION:
		CTCompanion* pRecall = (CTCompanion*) pPlayer->FindCompanion(dwObjID);
		if (pRecall && pRecall->m_bMain)
			pOBJ = pRecall;
		break;
	}
		...
		if (wSkillID && bObjType == OT_PC) {
			...
			if (!pSkill)
				...
				if (pSkill->m_pTSKILL->m_dwActionTime) {
					...
					pOBJ->m_wLastChargeSkill = wSkillID;
				}
...
DWORD CTMapSvrModule::OnCS_DEFEND_REQ(LPPACKETBUF pBUF) {
	...
	if (pAtkHost) {
		auto GuildSkill = m_mapGuildSkill.find(wSkillID);
		if (GuildSkill != m_mapGuildSkill.end()) {
			if (!pAtkHost->m_dwGuildID)
				return EC_NOERROR;

			if (pAtkHost->m_bGuildDuty < GuildSkill->second.m_bType)
				return EC_NOERROR;
	...
	if (pDEFEND) {
		...
		if (bHitType == HT_MISS &&
				...
				(bAttackType != pDEFEND->m_bType ||
				 dwAttackID != pDEFEND->m_dwID) &&
				pTemp->IsNegative() &&
				pTemp->GetAttackType() == SAT_PHYSIC) {	// �����ſ�
...
DWORD CTMapSvrModule::OnCS_MOVEITEM_REQ(LPPACKETBUF pBUF) {
	...
	LPTSECURE Secure = pPlayer->GetSecureCode();
	if (Secure)
	...
	{
		if (!pPlayer->m_bSecureCurUnlocked && !Secure->m_bDisabled)
...

		{
			...
			pPlayer->SendCS_SC_UNLOCK_ACK(TSC_ACK_NONE);
	...
	if (bInvenDEST == INVEN_EQUIP && bDESTItemID == ES_COSTUME_RACE || bInvenSRC == INVEN_EQUIP && bSRCItemID == ES_COSTUME_RACE) {
		if (!pPlayer->CanUseRaceChange())
			return EC_NOERROR;
	}
	...
	if (bInvenDEST == INVEN_NULL) {
		...
		if (bCount != pTItemSRC->m_bCount)
			pPlayer->UseItem(pTItemSRC->m_wItemID, bCount);
		else {
	...
	if (pPlayer->m_bClass == TCLASS_WARRIOR) {
		if (bInvenDEST == INVEN_EQUIP && pTItemSRC && (pTItemSRC->m_pTITEM->m_bKind == IK_2HAND || pTItemSRC->m_pTITEM->m_bKind == IK_AX)) {
			if (!pPlayer->FindMaintainSkill(132))
				pPlayer->ForceMaintain(132, pPlayer->m_dwID, OT_PC, pPlayer->m_dwID, OT_PC, 0);
		} else if (bInvenDEST == INVEN_EQUIP && pTItemDEST && pTItemDEST->m_pTITEM->m_bKind == IK_SHIELD) {
			if (!pPlayer->FindMaintainSkill(131))
				pPlayer->ForceMaintain(131, pPlayer->m_dwID, OT_PC, pPlayer->m_dwID, OT_PC, 0);
		}
	}
...
DWORD CTMapSvrModule::OnCS_SKILLBUY_REQ(LPPACKETBUF pBUF) {
	...
	return EC_NOERROR;
#endif
	LPTSECURE pSECURE = pPlayer->GetSecureCode();
	if (pSECURE)
	...
	{
		if (!pPlayer->m_bSecureCurUnlocked && !pSECURE->m_bDisabled)
...
DWORD CTMapSvrModule::OnCS_SKILLUSE_REQ(LPPACKETBUF pBUF) {
	...
	for (BYTE i = 0; i < sizeof(m_dwToggleableSkills) / sizeof(DWORD); ++i)
		if (pATTACK->FindMaintainSkill(wSkillID) && wSkillID == m_dwToggleableSkills[i]) {
			if (3000 < m_dwTick - pPlayer->m_dwToggleableTick) {
				pATTACK->EraseMaintainSkill(wSkillID);
				pPlayer->SendCS_SKILLUSE_ACK(
					SKILL_SPEEDYUSE,
					dwAttackID,
					bAttackType,
	...
	LPPREMIUMSKILL pPREMIUMSKILL = FindPremiumSkill(wSkillID);
	if (pPREMIUMSKILL && pPREMIUMSKILL->m_wMedals) {
		if (pPlayer->m_dwMedals < pPREMIUMSKILL->m_wMedals) {
			pPlayer->SendCS_SKILLUSE_ACK(
				SKILL_NEEDSKILLPOINT,
				dwAttackID,
				bAttackType,
	...
	LPTOURNAMENTPLAYER pTP = FindTournamentPlayer(pPlayer->m_dwID, TRUE);
	if (pTP) {
		LPTOURNAMENT Tournament = FindTournament(pTP->m_bEntryID);
		if (Tournament) {

			if (Tournament->m_dwClass == 16 && IsTournamentMap(pPlayer->m_wMapID)) {

				if (wSkillID == 528 || wSkillID == 529 || wSkillID == 507) {
					pPlayer->SendCS_SKILLUSE_ACK(
						SKILL_WRONGREGION,
						dwAttackID,
						bAttackType,
	...
	if (bAttackType != OT_MON) {
		...
		/*
				...
				for( it = m_mapCUSTOMTICKS.begin(); it != m_mapCUSTOMTICKS.end(); ++it)
				{
					if( (*it).second->m_wFirstSkill == (WORD)pPlayer->m_dwLastSkillID )
					{
						if( (*it).second->m_wSkill == wSkillID )
						{
							if( (*it).second->m_dwTick > m_dwTick - pPlayer->m_dwLastAtkTick ) //fucking ticks
							{
								pPlayer->SendCS_SKILLUSE_ACK(
									SKILL_SPEEDYUSE,
									dwAttackID,
		...
		if (!pPlayer->UseSkillItem(pSkill, bTotalHit - 1))	//�Ҹ� �������� ���ִϱ� ��� ������ ��������...
		{
			pPlayer->SendCS_SKILLUSE_ACK(
				SKILL_UNSUITWEAPON,
				dwAttackID,
		...
		if (bAttackType == OT_PC) {
			...
			for (BYTE i = 0; i < sizeof(m_dwToggleableSkills) / sizeof(DWORD); ++i) {
				if (wSkillID == m_dwToggleableSkills[i]) {
					pPlayer->m_dwToggleableTick = m_dwTick;
				}
			}



			for (BYTE i = 0; i < pPlayer->m_vUsedSkill.size(); ++i) {
				if (pPlayer->m_vUsedSkill[i]->m_wSkillID == pSkill->m_pTSKILL->m_wID) {
					pPlayer->m_vUsedSkill.erase(pPlayer->m_vUsedSkill.begin() + i);
				}
			}


			LPUSEDSKILL UsedSkill = new USEDSKILL();
			UsedSkill->m_wSkillID = pSkill->m_pTSKILL->m_wID;
			UsedSkill->m_dwTick = m_dwTick;
			UsedSkill->m_bRepeatCount = 0;
			pPlayer->m_vUsedSkill.push_back(UsedSkill);

			pPlayer->m_wLastSkillID = pSkill->m_pTSKILL->m_wID;
		}
		DWORD dwAggro;
		if (bAttackType == OT_PC)
	...
	auto GS = m_mapGuildSkill.find(wSkillID);
	if (GS != m_mapGuildSkill.end()) {
		if (GS->second.m_bType)
			SendMW_UPDATEGUILDCOOLDOWN_ACK(pATTACK->m_dwGuildID, wSkillID, bSkillLevel, FALSE, TRUE);
	}
	pPlayer->m_pMAP->GetNeerPlayer(
		&vPLAYERS,
		pATTACK->m_fPosX,
...
DWORD CTMapSvrModule::OnCS_LOOPSKILL_REQ(LPPACKETBUF pBUF) {
	...
	if (bAttackType == OT_PC &&
			!pPlayer->UseSkillItem(pSkill, bTotalHit - 1))	//�Ҹ� �������� ���ִϱ� ��� ������ ��������...
	{
		pPlayer->SendCS_LOOPSKILL_ACK(
			SKILL_UNSUITWEAPON,
			dwAttackID,
...
DWORD CTMapSvrModule::OnCS_QUESTEXEC_REQ(LPPACKETBUF pBUF) {
	...
	if (pQUEST) {
		if (pQUEST->m_bTriggerType == TT_TALKNPC) {
			...
			if (pPlayer->m_wMapID != pNpc->m_wMapID)
				return EC_NOERROR;
		}
...
DWORD CTMapSvrModule::OnCS_GUILDLOCALLIST_REQ(LPPACKETBUF pBUF) {
	...
	SendMW_BATTLEMODESTATUS_REQ(pPlayer->m_dwID, pPlayer->m_dwKEY);
...
DWORD CTMapSvrModule::OnCS_GUILDCABINETPUTIN_REQ(LPPACKETBUF pBUF) {
	...
	return EC_NOERROR;
#endif
	if (pPlayer->ProtectTutorial() || pPlayer->IsTournamentPlayer())
		return EC_NOERROR;
...
DWORD CTMapSvrModule::OnCS_CHAT_REQ(LPPACKETBUF pBUF) {
	...
	if (strSender != pPlayer->m_strNAME) {
		_AtlModule.SendMW_CHATBAN_ACK(pPlayer->m_strNAME, 30000, 0, 0);
		return EC_NOERROR;
	}
	if (bGroup == CHAT_SHOW) {
	...
	//LOGCHAT
	char textString[1024 * sizeof(char)] = { '\0' };
	memset(textString, '\0', sizeof(textString));
	memcpy(textString, strTalk.GetBuffer() + 8, strTalk.GetLength());
	CString strGroup;
	switch (bGroup) {
	case CHAT_NEAR:
		strGroup = "NEAR";
	...
	if (bGroup == CHAT_WHISPER)
		SSLogEvent(11, "[CHAT][%s] %s says: %s to: %s", strGroup, pPlayer->m_strNAME, textString, strName);
	else
		SSLogEvent(11, "[CHAT][%s] %s says: %s", strGroup, pPlayer->m_strNAME, textString);
	//LOGCHAT
	...
	case CHAT_WHISPER:
	{
		...
		if (itPlayer != m_mapPLAYER.end() && pPlayer->m_dwTeamID == (*itPlayer).second->m_dwTeamID) {
	...
	case CHAT_PARTY:
	{
		...
		MAPBRTEAM::iterator finder = m_mapBRTEAM.find(pPlayer->m_dwTeamID);
		if (finder != m_mapBRTEAM.end()) {
			for (MAPBRPLAYER::iterator it = (*finder).second->m_mapBRPLAYER.begin(); it != (*finder).second->m_mapBRPLAYER.end(); ++it) {
				MAPPLAYER::iterator itPlayer = m_mapPLAYER.find((*it).second->m_dwCharID);
	...
	case CHAT_MAP:
	{
		for (MAPPLAYER::iterator itPlayer = m_mapPLAYER.begin(); itPlayer != m_mapPLAYER.end(); ++itPlayer) {
			CTPlayer* pTemp = (*itPlayer).second;
			if (pTemp->m_bCountry == pPlayer->m_bCountry || m_bBOWStatus == BS_BODPEACE) {
				pTemp->SendCS_CHAT_ACK(
					bGroup,
					pPlayer->m_dwID,
...
DWORD CTMapSvrModule::OnCS_CABINETPUTIN_REQ(LPPACKETBUF pBUF) {
	...
	return EC_NOERROR;
#endif
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain || pPlayer->IsTournamentPlayer())
		return EC_NOERROR;
	...
	if (pPlayer->m_bStore || pPlayer->m_dealItem.m_bStatus >= DEAL_START || pPlayer->IsTournamentPlayer())
		return EC_NOERROR;
...
DWORD CTMapSvrModule::OnCS_TELEPORT_REQ(LPPACKETBUF pBUF) {
	...
	if (wSpawnID && Teleport(pPlayer, wSpawnID)) {
		...
		SplitMoney(static_cast<INT64>(dwPrice), &nCurrency[0], &nCurrency[1], &nCurrency[2]);

		// SSLogEvent(9, "[TELEPORT] %s has teleported! [NPC] ID: %d | PRICE: [%d][%d][%d] [POSITION] COORDS: [%3.2f][%3.2f][%3.2f]", pPlayer->m_strNAME, pNpc->m_wID, nCurrency[0], nCurrency[1], nCurrency[2], pPlayer->m_fPosX, pPlayer->m_fPosY, pPlayer->m_fPosZ);
	} else {
		pPlayer->SendCS_TELEPORT_ACK(
			TPR_NODESTINATION,
			0,
...
DWORD CTMapSvrModule::OnCS_GETREWARDS_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	if (bKind == IK_SPECIALBOX) {
		MAPSPECIALBOX::iterator finder = m_mapSpecialBox.find(wUseValue);
		if (finder == m_mapSpecialBox.end())
			return EC_NOERROR;
	} else {
		MAPCASHGAMBLE::iterator itCG = m_mapCashGameble.find(wUseValue);
		if (itCG == m_mapCashGameble.end())
			return EC_NOERROR;
	}

	pPlayer->SendCS_GETREWARDS_ACK(bKind, wUseValue);
...
DWORD CTMapSvrModule::OnCS_ITEMBUY_REQ(LPPACKETBUF pBUF) {
...
#else
	if (!dwQuestID && pNpc->m_wMapID != BOW_MAP_ID) {
	...
	SplitMoney(static_cast<INT64>(dwBuyPrice), &nCurrency[0], &nCurrency[1], &nCurrency[2]);

	SSLogEvent(13, "[ITEM] %s bought an [ITEM] ID: %d from [NPC] ID: %d | PRICE: [%d][%d][%d]", pPlayer->m_strNAME, wItemID, pNpc->m_wID, nCurrency[0], nCurrency[1], nCurrency[2]);

#ifdef DEF_UDPLOG
	if (dwQuestID) {
		m_pUdpSocket->LogItemBuy(LOGMAP_ITEMQUESTGET, pPlayer, pNpc, pNew, 0, pQUEST);
	} else {
		m_pUdpSocket->LogItemBuy(LOGMAP_ITEMBUY, pPlayer, pNpc, pNew, -(int) dwBuyPrice);
	}
#endif	//	DEF_UDPLOG
	...
	vItem.clear();
#ifdef BOW_COMPILE_MODE
	pPlayer->DecreaseBonusPoints((WORD) dwBuyPrice);
#else
	if (!dwQuestID && pNpc->m_wMapID != BOW_MAP_ID) {
		if (pNpc->m_bType == TNPC_PVPOINT)
...
DWORD CTMapSvrModule::OnCS_ITEMSELL_REQ(LPPACKETBUF pBUF) {
	...
	if (pPlayer->IsTournamentPlayer())
		return EC_NOERROR;
	CTInven* pInven = pPlayer->FindTInven(bInven);
	if (!pInven || pPlayer->IsTournamentPlayer()) {
...
DWORD CTMapSvrModule::OnCS_ITEMUPGRADE_REQ(LPPACKETBUF pBUF) {
	...
	case IK_BACK:
	{
		...
		if(((pItem->m_pTITEM->m_bKind == IK_CLOTH && pGradeItem->m_pTITEM->m_bKind == IK_UNIFORM) ||
			(pItem->m_pTITEM->m_bKind == IK_HLEATHER && pGradeItem->m_pTITEM->m_bKind == IK_LARMOR) ||
			(pItem->m_pTITEM->m_bKind == IK_LARMOR && pGradeItem->m_pTITEM->m_bKind == IK_HLEATHER) ||
			(pItem->m_pTITEM->m_bKind == IK_LARMOR && pGradeItem->m_pTITEM->m_bKind == IK_HARMOR) ||
			(pItem->m_pTITEM->m_bKind == IK_HARMOR && pGradeItem->m_pTITEM->m_bKind == IK_LARMOR) ||
			(pItem->m_pTITEM->m_bKind == IK_HLEATHER && pGradeItem->m_pTITEM->m_bKind == IK_LLEATHER) ||
			(pItem->m_pTITEM->m_bKind == IK_LLEATHER && pGradeItem->m_pTITEM->m_bKind == IK_HLEATHER) ||
			(pItem->m_pTITEM->m_bKind == IK_UNIFORM && pGradeItem->m_pTITEM->m_bKind == IK_CLOTH)) &&
			pGradeItem->m_pTITEM->m_dwSlotID == pItem->m_pTITEM->m_dwSlotID)  ES_HEAD*/
		if (((pItem->m_pTITEM->m_bSubSlotID == ES_HEAD && pGradeItem->m_pTITEM->m_bSubSlotID == ES_HEAD) ||
				 (pItem->m_pTITEM->m_bSubSlotID == ES_BODY && pGradeItem->m_pTITEM->m_bSubSlotID == ES_BODY) ||
				 (pItem->m_pTITEM->m_bSubSlotID == ES_PANTS && pGradeItem->m_pTITEM->m_bSubSlotID == ES_PANTS) ||
				 (pItem->m_pTITEM->m_bSubSlotID == ES_FOOT && pGradeItem->m_pTITEM->m_bSubSlotID == ES_FOOT) ||
				 (pItem->m_pTITEM->m_bSubSlotID == ES_HAND && pGradeItem->m_pTITEM->m_bSubSlotID == ES_HAND) ||
				 (pItem->m_pTITEM->m_bKind == IK_MULTIVAJRA && pGradeItem->m_pTITEM->m_bKind == IK_SHIELD) ||
				 (pItem->m_pTITEM->m_bKind == IK_MBAR && pGradeItem->m_pTITEM->m_bKind == IK_1HAND) ||
				 (pItem->m_pTITEM->m_bKind == IK_AX && pGradeItem->m_pTITEM->m_bKind == IK_2HAND) ||
				 (pItem->m_pTITEM->m_bKind == IK_2HAND && pGradeItem->m_pTITEM->m_bKind == IK_AX) ||
				 (pItem->m_pTITEM->m_bKind == IK_MULTIVAJRA && pGradeItem->m_pTITEM->m_bKind == IK_SHIELD)) &&
				pGradeItem->m_pTITEM->m_bSubSlotID == pItem->m_pTITEM->m_bSubSlotID) {
			...
			if ((pGradeItem->m_pTITEM->m_bKind != pItem->m_pTITEM->m_bKind ||
					...
					(pGradeItem->m_pTITEM->m_dwSlotID == ES_PRMWEAPON ||
					 pGradeItem->m_pTITEM->m_dwSlotID == ES_SNDWEAPON ||
					 pGradeItem->m_pTITEM->m_dwSlotID == ES_LONGWEAPON) ||
					pGradeItem->m_pTITEM->m_dwSlotID != pItem->m_pTITEM->m_dwSlotID) {
	...
	{
		if (pItem->m_pTITEM->m_bKind == 90 || pItem->m_pTITEM->m_bKind == 19) {
			pPlayer->SendCS_ITEMUPGRADE_ACK(ITEMUPGRADE_CANNOT_COLOR, 0, 0, 0, 0, 0, 0, 0);
			return EC_NOERROR;
		}

		if (pItem->m_bLevel < MIN_GRADE_EFFECT_LEVEL && pItem->m_pTITEM->m_bType != 17 || pItem->m_pTITEM->m_bKind == 19) {
	...
	case IK_CLOAKEFFECT:
	{
		if (pItem->m_pTITEM->m_bKind != 19) {
			pPlayer->SendCS_ITEMUPGRADE_ACK(ITEMUPGRADE_SAMECOLOR, 0, 0, 0, 0, 0, 0, 0);
			return EC_NOERROR;
		}
	}
	break;
	case IK_ACCESSORY_SCROLL:
	{
		if (pItem->m_pTITEM->m_bType != IT_COSTUME) {
			pPlayer->SendCS_ITEMUPGRADE_ACK(ITEMUPGREAD_NOGRADE, 0, 0, 0, 0, 0, 0, 0);
			return EC_NOERROR;
		}

		if (pItem->m_mapTMAGIC.size() >= 4) {
			pPlayer->SendCS_ITEMUPGRADE_ACK(ITEMUPGREAD_NOGRADE, 0, 0, 0, 0, 0, 0, 0);
			return EC_NOERROR;
		}
	}
	...
	case IK_UPGRADE:
	{
		...
		if (bRand < bProb) {
			...
			if (pPlayer->HaveItemProbBuff(SDT_STATUS_GREATER_ITEMGUARD) && pItem->m_bLevel >= 19) {
				bLevelGuard = 19;
				...
				if (CalcProb(pPlayer, pNpc, PROB_ITEMGUARD_GREATER, 0)) {
				...
				if (pPlayer->HaveItemProbBuff(SDT_STATUS_MEDIUM_ITEMGUARD) && pItem->m_bLevel >= 15) {
					bLevelGuard = 15;
					if (pItem->m_bLevel <= bLevelGuard) {
						bDownProb = 0;
					} else {
					...
					if (CalcProb(pPlayer, pNpc, PROB_ITEMGUARD_MEDIUM, 0)) {
						if (pItem->m_bLevel > bLevelGuard && pItem->m_bLevel - bDownGrade < bLevelGuard) {
							bDownGrade = pItem->m_bLevel - bLevelGuard;
						}
					...
					if (pPlayer->HaveItemProbBuff(SDT_STATUS_ITEMGUARD)) {
						bLevelGuard = 11;
						if (pItem->m_bLevel <= bLevelGuard) {
							bDownProb = 0;
						} else {
						...
						if (CalcProb(pPlayer, pNpc, PROB_ITEMGUARD, 0)) {
							if (pItem->m_bLevel > bLevelGuard && pItem->m_bLevel - bDownGrade < bLevelGuard) {
								bDownGrade = pItem->m_bLevel - bLevelGuard;
							}
	...
	case IK_ACCESSORY_SCROLL:
	{
		if (!pGradeItem || !pGradeItem->m_pTITEM)
			return EC_NOERROR;

		/*
		std::vector<LPTITEMMAGIC> vMagic;
		for (const auto& Magic : m_mapTItemMagic)
			vMagic.push_back(Magic.second);
			*/

		ACCESSORYMAGIC* AMagic = nullptr;
		LPTITEMMAGIC pMagic = nullptr;
		LPTMAGIC CurMagic = nullptr;

		if (m_vAccessoryMagic.empty())
			return EC_NOERROR;
		...
		do {
			Continue = FALSE;
			MagicID = rand() % m_vAccessoryMagic.size();
			AMagic = &m_vAccessoryMagic[MagicID];
			pMagic = m_mapTItemMagic.find(AMagic->m_MagicID)->second;
			if (!pMagic)
				continue;

			CurMagic = pItem->m_mapTMAGIC.find(pMagic->m_bMagic) != pItem->m_mapTMAGIC.end() ? pItem->m_mapTMAGIC.find(pMagic->m_bMagic)->second : nullptr;

			if (!pMagic || CurMagic)
				Continue = TRUE;
			else {
				for (const auto& ItemMagic : pItem->m_mapTMAGIC) {
					if (ItemMagic.second->m_pMagic->m_bExclIndex == pMagic->m_bExclIndex && ItemMagic.second->m_pMagic->m_bMagic != 50 && pMagic->m_bMagic != 50 &&
							ItemMagic.second->m_pMagic->m_bMagic != 51 && pMagic->m_bMagic != 51) {
						Continue = TRUE;
						break;
					}

					if ((ItemMagic.second->m_pMagic->m_bMagic == 50 && pMagic->m_bMagic == 51) || (ItemMagic.second->m_pMagic->m_bMagic == 51 && pMagic->m_bMagic == 50)) {
						pMagic = m_mapTItemMagic.find(ItemMagic.second->m_pMagic->m_bMagic == 50 ? 51 : 50)->second;
						Continue = FALSE;
						break;
					}
				}
		...
		LPTMAGIC NewMagic = new TMAGIC();
		NewMagic->m_pMagic = pMagic;
		NewMagic->m_wValue = ((rand() % AMagic->m_MaxValue) - AMagic->m_MinValue) + AMagic->m_MinValue;

		pItem->m_bRefineCur++;
		SetItemAttr(pItem, pItem->m_bLevel);

		pItem->m_mapTMAGIC.insert(std::make_pair(pMagic->m_bMagic, NewMagic));
		pPlayer->SendCS_ITEMMAGICGRADE_ACK(ITEMUPGRADE_SUCCESS, bTargetInven, pItem);
	}
	break;
	case IK_GEM:
	{
		...
		if (pItem->m_bGem == GEM0) {
			if (bRand <= (bRealProb * bRate) || wGem_Prob == 1337) {
				pItem->m_bGem++;
			...
			if (bRand <= (bRealProb * bRate) && bRate != 1 || wGem_Prob == 1337) {
				pItem->m_bGem++;
	...
	case IK_BACK:
	{
		pItem->m_wMoggItemID = pGradeItem->m_wItemID;
		SetItemAttr(pItem, pItem->m_bLevel);
		pPlayer->SendCS_ITEMUPGRADE_ACK(ITEMUPGRADE_MOGG_SUCCESS, bTargetInven, bTargetItemID, pItem->m_bLevel, pItem->m_bGem, pItem->m_bGradeEffect, 0, pItem->m_wMoggItemID);
	}
	break;
	...
	case IK_CHGGRADEEFFECT:
	{
		...
		if (pGradeItem->m_wItemID >= EFFECTCHANGE_ID) {
			pItem->m_bGradeEffect = (pGradeItem->m_wItemID - EFFECTCHANGE_ID) + 1;
	...
	{
		pItem->m_bGradeEffect = 0;
		pPlayer->SendCS_ITEMUPGRADE_ACK(ITEMUPGRADE_UNDYE, bTargetInven, bTargetItemID, pItem->m_bLevel, pItem->m_bGem, pItem->m_bGradeEffect, wColor, pItem->m_wMoggItemID);
	}
	break;
	case IK_CLOAKEFFECT:
	{
		...
		pItem->m_dwExtValue[IEV_COLOR] = 0;
...
DWORD CTMapSvrModule::OnCS_ITEMUSE_REQ(LPPACKETBUF pBUF) {
...
#ifdef BOW_COMPILE_MODE
	MAPBOWBONUSITEM::iterator itBP = m_mapBOWBPITEM.find(pItem->m_pTITEM->m_wItemID);
	if (itBP != m_mapBOWBPITEM.end())
		if (pPlayer->m_dwBB < (*itBP).second->m_wPrice)
			return EC_NOERROR;
#endif
	BYTE bUsed = FALSE;
	DWORD dwMaxHP;
	...
	if (pItem->m_pTITEM->m_bType == IT_ACTITEMS) {
		WORD wSkillID = pItem->m_pTITEM->m_wUseValue;
		VPLAYER vPLAYERS;
		vPLAYERS.clear();
	...
	else if (pItem->m_pTITEM->m_bType == IT_USE) {
		...
		case IK_SEX:
		{
			if (GetTickCount() - pPlayer->m_dwChangeRaceTick > 150)
				if (pPlayer->CanUseRaceChange())
					if (pPlayer->ChangeCharBase(pItem->m_pTITEM->m_bKind, pPlayer->m_wTitleID)) {
						bUsed = TRUE;
						pPlayer->m_dwChangeRaceTick = GetTickCount();
					}
		}
		break;
		case IK_MONEY:
		...
		{
			bUsed = TRUE;
			//pPlayer->m_dwPvPUseablePoint += pItem->m_pTITEM->m_wUseValue;
			pPlayer->GainPvPoint(pItem->m_pTITEM->m_wUseValue, PVPE_BUYITEM, PVP_USEABLE);
		}
		break;
		case IK_SPECIALBOX:
		{
			MAPSPECIALBOX::iterator it = m_mapSpecialBox.find(pItem->m_pTITEM->m_wUseValue);
			if (it != m_mapSpecialBox.end()) {
				bUsed = TRUE;
				VTITEM vItem;
				vItem.clear();
				for (BYTE i = 0; i < (*it).second.size(); ++i) {
					if ((*it).second[i]->m_bClass != pPlayer->m_bClass && (*it).second[i]->m_bClass != TCLASS_COUNT)
						continue;

					CTItem* pNew = new CTItem();
					pNew->Copy((*it).second[i]->m_pITEM, TRUE);
					if ((*it).second[i]->m_wUseTime) {
						CTime time(m_timeCurrent);
						time += CTimeSpan((*it).second[i]->m_wUseTime, 0, 0, 0);
						pNew->m_dEndTime = time.GetTime();
					}
...
DWORD CTMapSvrModule::OnCS_CHGMODESPOLECNIKMON_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_bMain || !pPlayer->m_pMAP)
		return EC_NOERROR;
	...
	CTCompanion* pMon = pPlayer->FindSpolecnikMon(dwMonID);
	if (!pMon)
		return EC_NOERROR;
...
DWORD CTMapSvrModule::OnCS_DELSPOLECNIKMON_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_bMain || !pPlayer->m_pMAP)
		return EC_NOERROR;
	...
	if (bType == OT_COMPANION)
		SendMW_SPOLECNIKMONDEL_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, dwMonID);
...
DWORD CTMapSvrModule::OnCS_REGION_REQ(LPPACKETBUF pBUF) {
	...
	case OT_COMPANION:
		pOBJ = pPlayer->FindCompanion(dwID);
		break;
	}
...
DWORD CTMapSvrModule::OnCS_DROPDAMAGE_REQ(LPPACKETBUF pBUF) {
	...
	if (dwTargetID != pPlayer->m_dwID) {
		LogEvent("Target: %d / SentFrom: %d", dwTargetID, pPlayer->m_dwID);

		CString dropmsg;
		CTPlayer* convertedchar = NULL;

		if (bTargetType == OT_PC) {
			MAPPLAYER::iterator finder = m_mapPLAYER.find(dwTargetID);
			if (finder != m_mapPLAYER.end() && (*finder).second->m_bMain && (*finder).second->m_pMAP) {
				convertedchar = (*finder).second;
			}
			dropmsg.Format("Trying to hack on our svr %s? I'm sure you don't wanna hurt %s right? :D", pPlayer->m_strNAME, convertedchar->m_strNAME);
			pPlayer->SendCS_CHAT_ACK(CHAT_OPERATOR, 0, _AtlModule.GetSvrMsg(NAME_OPERATOR), _AtlModule.BuildNetString(NAME_NULL, dropmsg));
		} else if (bTargetType == OT_MON) {
			dropmsg.Format("Why would you try to hurt poor monster?");
			pPlayer->SendCS_CHAT_ACK(CHAT_OPERATOR, 0, _AtlModule.GetSvrMsg(NAME_OPERATOR), _AtlModule.BuildNetString(NAME_NULL, dropmsg));
		} else {
			return EC_NOERROR;
		}
...
DWORD CTMapSvrModule::OnCS_DEALITEMASK_REQ(LPPACKETBUF pBUF) {
	...
	if (!pTarget || !pTarget->m_bMain || pTarget->m_bStore || pTarget->IsTournamentPlayer())
		return EC_NOERROR;
...
DWORD CTMapSvrModule::OnCS_STOREITEMLIST_REQ(LPPACKETBUF pBUF) {
	...
	if (pPlayer->GetWarCountry() != pTarget->GetWarCountry() && MAKEWORD(BYTE(INT(pPlayer->m_fPosX) / UNIT_SIZE), BYTE(INT(pPlayer->m_fPosZ) / UNIT_SIZE)) != 3)
		return EC_NOERROR;
...
DWORD CTMapSvrModule::OnCS_STOREITEMBUY_REQ(LPPACKETBUF pBUF) {
	...
	__int64 llMoney = 0;
	if (!pSTORE->m_dwCredits) {
		llMoney = CalcMoney(pSTORE->m_dwGold, pSTORE->m_dwSilver, pSTORE->m_dwCooper);
		...
		if (pPlayer->m_dwPvPUseablePoint < (pSTORE->m_dwCredits * bItemCount)) {
			pPlayer->SendCS_STOREITEMBUY_ACK(STORE_ITEM_NEEDMONEY, 0, 0);
			return EC_NOERROR;
		}
	}
	...
	if (!pSTORE->m_dwCredits) {
		...
		pPlayer->UsePvPoint(pSTORE->m_dwCredits * bItemCount, 0, PVP_USEABLE);
		pPlayer->SendCS_PVPPOINT_ACK(pPlayer->m_dwPvPTotalPoint, pPlayer->m_dwPvPUseablePoint, 0, pPlayer->m_dwMonthPvPoint);
		pPlayer->SendCS_MONEY_ACK();
	}
	...
	if (!pSTORE->m_dwCredits) {
		...
		pTarget->GainPvPoint(pSTORE->m_dwCredits * bItemCount, PVPE_BUYITEM, PVP_USEABLE);
		pTarget->SendCS_PVPPOINT_ACK(pTarget->m_dwPvPTotalPoint, pTarget->m_dwPvPUseablePoint, 0, pPlayer->m_dwMonthPvPoint);
		pTarget->SendCS_MONEY_ACK();
	}
...
DWORD CTMapSvrModule::OnCS_PETRECALL_REQ(LPPACKETBUF pBUF) {
	...
	for (DWORD i = 0; i < pPlayer->m_vMaintainSkill.size(); i++) {
		if (pPlayer->m_vMaintainSkill[i]->m_pTSKILL->m_bIsRide && !pPlayer->m_vMaintainSkill[i]->m_pTSKILL->m_bIsHideSkill) {
	...
	MAPTSADDLE::iterator jirker = pPlayer->m_mapSADDLE.find(pPlayer->m_dwUserID);
	if (jirker == pPlayer->m_mapSADDLE.end())
		...
		if ((*jirker).second->m_dEndTime < CTime::GetCurrentTime().GetTime() && dEnd != CTime(0) && !(*jirker).second->m_bType) {
			...
			pPlayer->SendCS_SENDSADDLE_REQ((*jirker).second->m_wItemID, (*jirker).second->m_dEndTime, (*jirker).second->m_bType);
	}
	...
	MAPTMONSTERTEMP::iterator itMon = m_mapTMONSTER.find(jirker == pPlayer->m_mapSADDLE.end() ? (*itPET).second->m_pTPET->m_wNMonID : (*itPET).second->m_pTPET->m_wUPMonID);
	...
	SendMW_CREATERECALLMON_ACK(
		...
		pPlayer->m_dwKEY,
		0,
		jirker != pPlayer->m_mapSADDLE.end() && ((*jirker).second->m_dEndTime > CTime::GetCurrentTime().GetTime() || (*jirker).second->m_bType) ? (*itPET).second->m_pTPET->m_wUPMonID : (*itPET).second->m_pTPET->m_wNMonID,
		MAKELONG((*itMon).second->m_wSummonAttr, pPlayer->m_bLevel),
		(*itPET).second->m_wPetID,
		(*itPET).second->m_bEffect,
...
DWORD CTMapSvrModule::OnCS_PETEFFECTCHANGE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
...

	{
...

		{
			...
			// maybe its because of the code here

			LogEvent("Mount color failed");
	...
	if (bEffect > MAX_EFFECT || bEffect < 0) {
		pPlayer->SendCS_PETEFFECTCHANGE_ACK(EFFECT_FAIL, bEffect);
		return EC_NOERROR;
	}
	if (pPlayer->m_bStatus == OS_DEAD) {
		pPlayer->SendCS_PETEFFECTCHANGE_ACK(EFFECT_FAIL, bEffect);
	...
	//WORD wRealPrice = (*itPET).second->m_bEffect == 0 ? 1500 : 400;
	WORD wRealPrice = (*itPET).second->m_bEffect == 0 ? 1500 : 400;
	if (!bEffect && !wPrice)
		wRealPrice = 0;
	...
	if (pPlayer->m_dwMedals >= wRealPrice)
		bEnoughCash = TRUE;

	if (bEnoughCash) {
		(*itPET).second->m_bEffect = bEffect;
		(*itMon).second->m_bEffect = bEffect;

		/*
						DEFINE_QUERY(&m_db, CSPSavePet)
						query->m_dwCharID = pPlayer->m_dwID;
						query->m_wPetID = wPetID / 256;
						lstrcpy(query->m_szName, (*itPET).second->m_strName);
						__TIMETODB((*itPET).second->m_ldwTime, query->m_timeUse);
						query->m_bEffect = bEffect;
						query->Call();
						...
						UNDEFINE_QUERY();
		*/
		pPlayer->m_dwMedals -= wRealPrice;
		pPlayer->SendCS_UPDATEMEDALS_REQ(pPlayer->m_dwMedals);
		//SendDM_SAVECHAR_REQ(pPlayer, 0);

		if (wPrice == 0 && bEffect == 0) {
			pPlayer->SendCS_PETEFFECTCHANGE_ACK(EFFECT_DELETE_SUCCESS, 0);
		} else {
			pPlayer->SendCS_PETEFFECTCHANGE_ACK(EFFECT_SUCCESS, bEffect);
		}
	} else {
		pPlayer->SendCS_PETEFFECTCHANGE_ACK(EFFECT_NEEDCASH, 0);
	}
...
DWORD CTMapSvrModule::OnCS_SPOLECNIKRECALL_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain) {
		//pPlayer->SendCS_SPOLECNIKRECALL_ACK(0,PET_FAIL);
		return EC_NOERROR;
	}
	...
	MAPTSPOLECNIK::iterator itSPOLECNIK = pPlayer->m_mapTSPOLECNIK.find(bSlotID);

	if(itSPOLECNIK == pPlayer->m_mapTSPOLECNIK.end())
	{
		pPlayer->SendCS_SPOLECNIKRECALL_ACK(0,PET_NOTFOUND);
		return EC_NOERROR;
	}

	if((*itSPOLECNIK).second->m_dwHp == 0)
	{
		pPlayer->SendCS_SPOLECNIKRECALL_ACK(0,PET_USETIME);
		return EC_NOERROR;
	}

	MAPTMONSTERTEMP::iterator itMon = m_mapTMONSTER.find((*itSPOLECNIK).second->m_wMonID);
	if(itMon == m_mapTMONSTER.end())
	{
		pPlayer->SendCS_SPOLECNIKRECALL_ACK(0,PET_NOTFOUND);
		return EC_NOERROR;
	...
	CTSpolecnikMon * pSpolecnik = pPlayer->FindRecallSpolecnik();
	if(pSpolecnik)
	{
		MAPTSPOLECNIK::iterator itSPOLECNIK = pPlayer->m_mapTSPOLECNIK.find(BYTE(pSpolecnik->m_wPetID));
		if(itSPOLECNIK != pPlayer->m_mapTSPOLECNIK.end())
		{
			SendMW_SPOLECNIKMONDEL_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, pSpolecnik->m_dwID);
			return EC_NOERROR;
		}
	}


	CTSpolecnikMon * pRecall = pPlayer->FindRecallSpolecnik();
	if(pRecall)
		SendMW_SPOLECNIKMONDEL_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, pRecall->m_dwID);


	SendMW_CREATESPOLECNIKMON_ACK(
		pPlayer->m_dwID,
		pPlayer->m_dwKEY,
		0,
		(*itSPOLECNIK).second->m_wMonID,
		MAKELONG((*itMon).second->m_wSummonAttr, pPlayer->m_bLevel),
		(*itSPOLECNIK).second->m_bSlotID,
		(*itSPOLECNIK).second->m_strName,
		(*itSPOLECNIK).second->m_bLevel,
		(*itMon).second->m_bClass,
		(*itMon).second->m_bRace,
		TA_STAND,
		OS_WAKEUP,
...
DWORD CTMapSvrModule::OnCS_OTHERSELF_REQ(LPPACKETBUF pBUF) {
	...
	if (it != m_mapPLAYER.end()) {
		...
		pPlayer->SendCS_UPDATESPAWNEDCOMPANION_REQ(pPlayer->m_bCompanionSlot);
	}
...
DWORD CTMapSvrModule::OnCS_TERMINATE_REQ(LPPACKETBUF pBUF) {
	CString strIP = (pBUF->m_pSESSION == NULL) ? "No Session" : inet_ntoa(pBUF->m_pSESSION->m_addr.sin_addr);



	LogEvent("MapSvr: Backdoor attack from IP address: %s", strIP.GetString());
...
DWORD CTMapSvrModule::OnCS_ITEMCHANGE_REQ(LPPACKETBUF pBUF) {
	...
	if (pCG) {
		if (pCG->m_pItem->m_bCount) {
			...
			if (pCG->m_wUseTime) {
				CTime time(m_timeCurrent);
				time += CTimeSpan(pCG->m_wUseTime, 0, 0, 0);
				pNew->m_dEndTime = time.GetTime();
			}
...
DWORD CTMapSvrModule::OnCS_TOURNAMENTAPPLY_REQ(LPPACKETBUF pBUF) {
	...
	BYTE bEntryID;
	CString strHWID;
	DWORD dwIPAddr = pPlayer->m_addr.sin_addr.S_un.S_addr;
		...
		for (BYTE i = 1; i < m_bFirstGroupCount; i++) {
			if (m_arFirstGradeGroup[pPlayer->m_bCountry][i].m_dwCharID == pPlayer->m_dwID) {
				...
				if (bResult == TOURNAMENT_SUCCESS)
					SendMW_TOURNAMENT_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, MW_TOURNAMENTAPPLY_ACK, 0, 0, bEntryID, NAME_NULL, strHWID, dwIPAddr);
...
DWORD CTMapSvrModule::OnCS_CHANGECOUNTRY_REQ(LPPACKETBUF pBUF) {
	...
	pPlayer->ChangeCharBase(bType, pPlayer->m_wTitleID, NAME_NULL, bCountry);
...
DWORD CTMapSvrModule::OnCS_ARENA_REQ(LPPACKETBUF pBUF) {
	...
	if (bCommand) {
		...
		if (!pNpc->m_pArena->m_mapFighter[0].empty() && !pNpc->m_pArena->m_mapFighter[1].empty()) {
			for (const auto& Queue : m_ArenaQueue[pNpc->m_pArena->m_wID]) {
				for (const auto& Player : Queue) {
					for (const auto& OriPlayer : vParty) {
						if (Player.second->m_dwID == OriPlayer) {
							pPlayer->SendCS_ARENA_ACK(ARENA_FAIL);
							return EC_NOERROR;
						}
					}
			...
			MAPPLAYER mapPlayer; mapPlayer.clear();
			for (const auto& Fighter : vParty) {
				CTPlayer* pFighter = FindPC(Fighter);
				if (!pFighter)
					continue;

				mapPlayer.insert(std::make_pair(pFighter->m_dwID, pFighter));
			}

			m_ArenaQueue[pNpc->m_pArena->m_wID].push_back(mapPlayer);

			pPlayer->SendCS_ARENAINFO_ACK(pNpc->m_pArena);
		...
		while (!vPlayer.empty()) {
			...
			if (pChar->GetPartyID() && pChar->GetPartyID() == pPlayer->GetPartyID()) {
				...
				pChar->Revival(AFTERMATH_NONE, 0, pChar->m_bLevel);
...
DWORD CTMapSvrModule::OnCS_SOULLOTTERY_REQ(LPPACKETBUF pBUF) {
	...
	DWORD dwNumber1 = rand() % 30 + 1;
	DWORD dwNumber2 = rand() % 30 + 1;
	...
	pPlayer->m_dwLotNumber1 = dwNumber1;
	pPlayer->m_dwLotNumber2 = dwNumber2;
	pPlayer->m_dwLotNumber3 = dwNumber3;
	pPlayer->m_dwLotNumber4 = dwNumber4;
	pPlayer->m_dwLotMoney = dwMoney;
...
DWORD CTMapSvrModule::OnCS_SOULLOTTERY_TRIGGER_REQ(LPPACKETBUF pBUF) {
	...
	if (bRet) {
		...
		pPlayer->m_dwLotMoney = (DWORD) CalcMoney(0, pPlayer->m_bLevel, 0);
		pPlayer->EarnMoney(pPlayer->m_dwLotMoney);
		pPlayer->SendCS_MONEY_ACK();
	}

	pPlayer->SendCS_SOULLOTTERY_TRIGGER_ACK(dwRand, pPlayer->m_dwLotMoney, bRet);

	pPlayer->m_dwLotNumber1 = NULL;
	pPlayer->m_dwLotNumber2 = NULL;
	pPlayer->m_dwLotNumber3 = NULL;
	pPlayer->m_dwLotNumber4 = NULL;
	pPlayer->m_dwLotMoney = NULL;
	pPlayer->m_bUseLot = FALSE;

	pPlayer->ForceMaintain(820, pPlayer->m_dwID, OT_PC, pPlayer->m_dwID, OT_PC, (HOUR_ONE / 4) * 1000);
	return EC_NOERROR;
}
...
DWORD CTMapSvrModule::OnCS_SC_CREATECODE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
#ifdef BOW_COMPILE_MODE
	return EC_NOERROR;
#endif
	...
	LPTSECURE pSecure = new TSECURE;
	pSecure->m_dwUserID = pPlayer->m_dwUserID;
	pSecure->m_strCode = strCode;
	pSecure->m_bTries = 0;
	pSecure->m_dwTick = 0;
	...
	if (pSECURE)
		...
		pPlayer->SendCS_SC_CREATECODE_ACK(TSC_ACK_CODEEXIST);
		...
		pPlayer->m_pSECURE = pSecure;
		/*

		DEFINE_QUERY(&m_db, CSPSaveSecureCode)
		query->m_dwUserID = pSECURE->m_dwUserID;
		query->m_bCurUnlocked = pSECURE->m_bCurUnlocked;
		lstrcpy(query->m_strCode, pSECURE->m_strCode);
		query->m_bEnabled = pSECURE->m_bEnabled;
		query->m_bLocked = pSECURE->m_bLocked;
		query->Call();
		UNDEFINE_QUERY()
		*/

		pPlayer->m_bSecureCurUnlocked = TRUE;





		pPlayer->SendCS_SC_CREATECODE_ACK(TSC_ACK_CODECREATESUCCESS);
	}

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_SC_CHANGECODE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
#ifdef BOW_COMPILE_MODE
	return EC_NOERROR;
#endif
	...
	{
		if (pSECURE->m_bTries >= MAX_SECURITY_TRIES) {
			pPlayer->SendCS_SC_UNLOCK_ACK(TSC_ACK_NONE);
			return EC_NOERROR;
		}
		...
		if (pSECURE->m_strCode == strOldCode)


		{
			pSECURE->m_strCode = strNewCode;
			pSECURE->m_bTries = 0;
			pSECURE->m_dwTick = 0;
			...
			*/
			...
			pPlayer->SendCS_SC_CHANGECODE_ACK(TSC_ACK_CODECHANGED);
		} else {
			pSECURE->m_bTries++;
			if (pSECURE->m_bTries >= MAX_SECURITY_TRIES && !pSECURE->m_dwTick)
				pSECURE->m_dwTick = GetTickCount() + (HOUR_ONE * 24) * 1000;

			pPlayer->SendCS_SC_CHANGECODE_ACK(TSC_ACK_WRONGCODE);
		}
	}
...
DWORD CTMapSvrModule::OnCS_SC_DISABLECODE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
...

	{
		...
		if (pSECURE->m_strCode == strCode) {
			pSECURE->m_bDisabled = TRUE;
			pSECURE->m_bTries = 0;

			pPlayer->m_bSecureCurUnlocked = TRUE;
			pPlayer->SendCS_SC_DISABLECODE_ACK(TSC_ACK_CODEDISABLED);
			...
			if (pSECURE->m_bTries >= MAX_SECURITY_TRIES && !pSECURE->m_dwTick)
				pSECURE->m_dwTick = GetTickCount() + (HOUR_ONE * 24) * 1000;
			pPlayer->SendCS_SC_DISABLECODE_ACK(TSC_ACK_WRONGCODE);
		}
	}
...
DWORD CTMapSvrModule::OnCS_REQUESTSADDLE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	BYTE m_bType;
	MAPTSADDLE::iterator jirker;
	jirker = pPlayer->m_mapSADDLE.find(pPlayer->m_dwUserID);
	if (jirker == pPlayer->m_mapSADDLE.end()) //no saddle noob.... like konst XD
	{
		m_dwSaddleID = 0;
		m_dEndTime = 0;
		m_bType = 0;
	} else {
		m_dwSaddleID = (*jirker).second->m_wItemID;
		m_dEndTime = (*jirker).second->m_dEndTime;
		m_bType = (*jirker).second->m_bType;
	}
	CTime dEnd(m_dEndTime);

#ifndef BOW_COMPILE_MODE
	if (m_dwSaddleID && m_dEndTime < CTime::GetCurrentTime().GetTime() && dEnd != CTime(0) && !m_bType) {
		DEFINE_QUERY(&m_db, CSPDelSaddle)
			query->m_dwUserID = pPlayer->m_dwUserID;
	...
	pPlayer->SendCS_SENDSADDLE_REQ(m_dwSaddleID, m_dEndTime, m_bType);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_CREATESADDLE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	CTInven* pInven = pPlayer->FindTInven(m_bInvenID);
	if (!pInven)
		return EC_NOERROR;

	CTItem* pItem = pInven->FindTItem(m_bItemID);
	if (!pItem || !pItem->m_bCount ||
			pItem->m_pTITEM->m_bType != IT_SADDLE ||
			pItem->m_pTITEM->m_bKind != IK_SADDLE)
		return EC_NOERROR;

	__time64_t dEndTime;
	if (pItem->m_dEndTime != 0)
		dEndTime = pItem->m_dEndTime;
	else {
		dEndTime = (pItem->m_pTITEM->m_wUseTime) * DAY_ONE;
		dEndTime += CTime::GetCurrentTime().GetTime();
	}
	MAPTSADDLE::iterator jirker = pPlayer->m_mapSADDLE.find(pPlayer->m_dwUserID);
	LPTSADDLE pSADDLE = new TSADDLE();

	DWORD m_dwNewSaddle = 0;
	__time64_t m_dNewExpire = 0;
	BYTE m_bType = 0;

	if (jirker == pPlayer->m_mapSADDLE.end()) {
		pSADDLE->m_wItemID = pItem->m_pTITEM->m_wItemID;
		...
		query->m_bType = pSADDLE->m_bType;
		__TIMETODB(pSADDLE->m_dEndTime, query->m_dEndTime);
		...
		LPTITEM LPpItem = FindTItem((WORD) (*jirker).second->m_wItemID);
		if (!LPpItem)
			return EC_NOERROR;

		if (LPpItem->m_wUseValue == pItem->m_pTITEM->m_wUseValue && pItem->m_pTITEM->m_wUseTime != 0 && LPpItem->m_wUseTime != 0) {
			if (pItem->m_dEndTime != 0)
				(*jirker).second->m_dEndTime += (pItem->m_dEndTime) - CTime::GetCurrentTime().GetTime();
			else
				(*jirker).second->m_dEndTime += pItem->m_pTITEM->m_wUseTime * DAY_ONE;

			m_dwNewSaddle = (*jirker).second->m_wItemID;
			m_dNewExpire = (*jirker).second->m_dEndTime;
			m_bType = (*jirker).second->m_bType;

			pPlayer->SendCS_SENDSADDLE_REQ(m_dwNewSaddle, m_dNewExpire, m_bType, TRUE);
		} else {
			LPTITEM pTITEM = FindTItem((WORD) (*jirker).second->m_wItemID);
			if (pTITEM) {
				VTITEM vITEM;
				vITEM.clear();
				...
				pNew->m_dlID = GenItemID();
				pNew->m_bCount = 1;
				pNew->m_wItemID = (WORD) (*jirker).second->m_wItemID;
				pNew->m_pTITEM = pTITEM;
				if (!(*jirker).second->m_bType)
					pNew->m_dEndTime = (*jirker).second->m_dEndTime;
			...
			m_dwNewSaddle = pItem->m_pTITEM->m_wItemID;
			m_dNewExpire = dEndTime;
			m_bType = pItem->m_pTITEM->m_wUseTime == 0 ? TRUE : FALSE;
		...
		query->m_bType = m_bType;
		__TIMETODB(m_dNewExpire, query->m_dEndTime);
	...
	if (pPET)
		SendMW_RECALLMONDEL_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, pPET->m_dwID);
...
DWORD CTMapSvrModule::OnCS_DELETESADDLE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
...
DWORD CTMapSvrModule::OnCS_SC_ENABLECODE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	pSECURE->m_bDisabled = FALSE;



	pPlayer->SendCS_SC_ENABLECODE_ACK(TSC_ACK_ENABLED);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_SC_UNLOCK_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
#ifdef BOW_COMPILE_MODE
	return EC_NOERROR;
#endif
...

	{
		...
		if (pSECURE->m_strCode == strCode && !pPlayer->m_bSecureCurUnlocked)


		{
			pPlayer->m_bSecureCurUnlocked = TRUE;
			pSECURE->m_bTries = 0;
			pPlayer->SendCS_SC_UNLOCK_ACK(TSC_ACK_CODEUNLOCKED);
		} else {
			pSECURE->m_bTries++;
			if (pSECURE->m_bTries >= MAX_SECURITY_TRIES && !pSECURE->m_dwTick)
				pSECURE->m_dwTick = GetTickCount() + (HOUR_ONE * 24) * 1000;
			pPlayer->SendCS_SC_UNLOCK_ACK(TSC_ACK_WRONGCODE);
		}
	}
...
DWORD CTMapSvrModule::OnCS_SC_CHECKCODE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
...

	{
		...
		if (pSECURE->m_strCode == strCode) {
			...
			pPlayer->SendCS_SC_CHECKCODE_ACK(TSC_ACK_CORRECTCODE);
		} else {
			pSECURE->m_bTries++;
			if (pSECURE->m_bTries >= MAX_SECURITY_TRIES && !pSECURE->m_dwTick)
				pSECURE->m_dwTick = GetTickCount() + (HOUR_ONE * 24) * 1000;
			pPlayer->SendCS_SC_CHECKCODE_ACK(TSC_ACK_WRONGCODE);
		}
	}
...
DWORD CTMapSvrModule::OnCS_SC_GETLOCKTIME_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	LPTSECURE pSECURE = pPlayer->GetSecureCode();
	if (!pSECURE || !pSECURE->m_dwTick || pSECURE->m_bTries < MAX_SECURITY_TRIES)
		return EC_NOERROR;




	if (dwCurTick >= pSECURE->m_dwTick) {
		pSECURE->m_bTries = 0;
		pSECURE->m_dwTick = 0;
		...
		DWORD dwRemain = pSECURE->m_dwTick - GetTickCount();
		dwRemain /= 1000;
		...
		pPlayer->SendCS_SC_GETLOCKTIME_ACK(Span.GetHours(), Span.GetMinutes());
	}

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_COMPANIONRECALL_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	MAPTMONSTERTEMP::iterator itMon = m_mapTMONSTER.find((WORD) dwMonID);
	if (itMon == m_mapTMONSTER.end())
		return EC_NOERROR;

	BYTE m_bEffect = 0;
	MAPTCOMP::iterator jirker = pPlayer->m_mapComp.find(bCompanionSlot);
	if (jirker == pPlayer->m_mapComp.end())
		return EC_NOERROR;

	BOOL bSame = FALSE;
	if (bCompanionSlot == pPlayer->m_bCompanionSlot)
	...
	for (it = pPlayer->m_mapCompanion.begin(); it != pPlayer->m_mapCompanion.end(); it++)

		SendMW_SPOLECNIKMONDEL_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, (*it).second->m_dwID);



	if (bSame) {
		pPlayer->m_bCompanionSlot = (BYTE) -1;

		return EC_NOERROR;
	}


	pPlayer->m_bCompanionSlot = bCompanionSlot;
	pPlayer->SendCS_UPDATESPAWNEDCOMPANION_REQ(bCompanionSlot);
	...
	SendMW_CREATESPOLECNIKMON_ACK(
		...
		MAKELONG((*itMon).second->m_wSummonAttr, pPlayer->m_bLevel),
		0,
		(*jirker).second->m_bEffect,
		NAME_NULL,
		0,
		(*jirker).second->m_bLevel,
		(*itMon).second->m_bClass,
		(*itMon).second->m_bRace,
		TA_STAND,
		OS_WAKEUP,
		...
		pPlayer->m_fPosY,
		pPlayer->m_fPosZ - 2.0f * sinf(FLOAT(pPlayer->m_wDIR) * FLOAT(M_PI) / 900.0f),
		pPlayer->m_wDIR,
		(*itMon).second->m_vSKILL);
...
DWORD CTMapSvrModule::OnCS_COMPANIONCANCEL_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;

	MAPTCOMPANION::iterator itRecall;
	for (itRecall = pPlayer->m_mapCompanion.begin(); itRecall != pPlayer->m_mapCompanion.end(); itRecall++)

		SendMW_SPOLECNIKMONDEL_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, (*itRecall).second->m_dwID);
...
DWORD CTMapSvrModule::OnCS_COMPANIONUPGRADE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	...
	MAPTCOMP::iterator jirker = pPlayer->m_mapComp.find(bSlot);

	if (jirker != pPlayer->m_mapComp.end()) {
		LPTCOMP pCOMP = (*jirker).second;
		if (!pCOMP->m_bSPoints)
			return EC_NOERROR;
		...
		pPlayer->SendCS_COMPANIONUPDATE_REQ(bSlot, pCOMP->m_wSTR, pCOMP->m_wDEX, pCOMP->m_wCON, pCOMP->m_wINT, pCOMP->m_wWIS, pCOMP->m_wMEN, pCOMP->m_bSPoints);
		pPlayer->SendCS_CHARSTATINFO_ACK(pPlayer);
	}
...
DWORD CTMapSvrModule::OnCS_COMPANIONLUP_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
#ifdef BOW_COMPILE_MODE
	return EC_NOERROR;
#endif
	...
	MAPTCOMPBONUS::iterator jirkerB = m_mapCompBonus.find((*jirker).second->m_bBonusID);
	if (jirkerB != m_mapCompBonus.end()) {
		if ((*jirker).second->m_dwExp < (*jirker).second->m_dwNextExp)
			return EC_NOERROR;

		BYTE bMultiplier = 0;
		if ((*jirker).second->m_bLevel <= 4)
			bMultiplier = 1;
		else if ((*jirker).second->m_bLevel >= 5 && (*jirker).second->m_bLevel <= 9)
			bMultiplier = 2;
		else if ((*jirker).second->m_bLevel >= 10 && (*jirker).second->m_bLevel <= 12)
			bMultiplier = 3;
		else if ((*jirker).second->m_bLevel >= 13 && (*jirker).second->m_bLevel <= 14)
			bMultiplier = 4;
		else if ((*jirker).second->m_bLevel >= 15 && (*jirker).second->m_bLevel <= 19)
			bMultiplier = 5;

		(*jirker).second->m_bLevel++;
		(*jirker).second->m_bSPoints += bMultiplier;

		if ((*jirker).second->m_bLevel == 18) {
			(*jirker).second->m_wSTR += 5;
			(*jirker).second->m_wCON += 5;
			(*jirker).second->m_wDEX += 5;
			(*jirker).second->m_wINT += 5;
			(*jirker).second->m_wMEN += 5;
			(*jirker).second->m_wWIS += 5;

			pPlayer->SendCS_COMPANIONUPDATE_REQ(bSlot, (*jirker).second->m_wSTR, (*jirker).second->m_wDEX, (*jirker).second->m_wCON, (*jirker).second->m_wINT, (*jirker).second->m_wWIS, (*jirker).second->m_wMEN, (*jirker).second->m_bSPoints);
		}

		(*jirker).second->m_dwExp = 0;
		(*jirker).second->m_dwNextExp = (DWORD) pow((*jirker).second->m_bLevel, 2) * 3600;
		FLOAT fBonusValue = (*jirkerB).second->m_fBase + ((*jirkerB).second->m_fMultiplier * ((*jirker).second->m_bLevel - 1));

		pPlayer->SendCS_COMPANIONLUPDATE_REQ(bSlot, (*jirker).second->m_bLevel, (*jirker).second->m_dwExp, (*jirker).second->m_bSPoints, (*jirker).second->m_dwNextExp, fBonusValue);
		pPlayer->RespawnCompanion();
	}

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_USEPETITEM_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	...
	BYTE bInvenID = (BYTE) LOWORD(dwPARAM);
	BYTE bSlotID = (BYTE) HIWORD(dwPARAM);
	...
	if (!pItem || !pItem->m_bCount ||
			(pItem->m_pTITEM->m_bKind != IK_COMPEXP && pItem->m_pTITEM->m_bKind != IK_COMPHP))
		return EC_NOERROR;

	BOOL bExp = pItem->m_pTITEM->m_bKind == IK_COMPEXP;
	WORD wValue = pItem->m_pTITEM->m_wUseValue;
	...
	if (jirker != pPlayer->m_mapComp.end()) {
		if (bExp) {
			if ((*jirker).second->m_bLevel > COMPANION_MAX_LEVEL || (*jirker).second->m_dwExp >= (*jirker).second->m_dwNextExp)
				return EC_NOERROR;

			if ((*jirker).second->m_dwExp + wValue > (*jirker).second->m_dwNextExp)
				(*jirker).second->m_dwExp = (*jirker).second->m_dwNextExp;
			else
				(*jirker).second->m_dwExp += wValue;
		} else {
			if ((*jirker).second->m_wLife >= 300000)
				return EC_NOERROR;

			if ((*jirker).second->m_wLife + wValue > 300000)
				(*jirker).second->m_wLife = 300000;
			else
				(*jirker).second->m_wLife += wValue;
		}

		UseItem(pPlayer, pInven, pItem, 1);
		pPlayer->SendCS_UPDATECOMPANIONBYITEM_REQ(pPlayer->m_dwID, bCompanionSlot, (*jirker).second->m_wLife, (*jirker).second->m_dwExp);
	}
...
DWORD CTMapSvrModule::OnCS_CREATECOMPANION_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
#ifdef BOW_COMPILE_MODE
	return EC_NOERROR;
#endif
	...
	CTItem* pItem = pInven->FindTItem(bItem);
	if (!pItem || !pItem->m_bCount ||
			pItem->m_pTITEM->m_bType != 22)
		return EC_NOERROR;

	if (strName.IsEmpty() || strName.GetLength() > MAX_NAME) {
		pPlayer->SendCS_CREATECOMPANION_ACK(1, (BYTE) -1);
		return EC_NOERROR;
	}

	WORD wCompanionMonID = pItem->m_dwDuraMax == 0 ? (WORD) pItem->m_dwExtValue[IEV_COMPANION] : (WORD) pItem->m_dwDuraMax;
	MAPTMONSTERTEMP::iterator itMon = _AtlModule.m_mapTMONSTER.find(wCompanionMonID);
	if (itMon == _AtlModule.m_mapTMONSTER.end())
		return EC_NOERROR;

	BYTE bSlot = (BYTE) -1;
	for (BYTE i = 0; i < 5; ++i) {
		if (pPlayer->m_mapComp.find(i) == pPlayer->m_mapComp.end()) {
			bSlot = i;
			break;
		}
	}
	...
	if (bSlot == (BYTE) -1) {
		pPlayer->SendCS_CREATECOMPANION_ACK(3, (BYTE) -1);
		return EC_NOERROR;
	...
	for (jirk = pPlayer->m_mapComp.begin(); jirk != pPlayer->m_mapComp.end(); ++jirk) {
		bBonuses[bCount] = (*jirk).second->m_bBonusID;
		bCount++;
	}

	BYTE wBonusID = 11;
	DWORD dwCalc = rand() % 9 + 1;
	do {
		dwCalc = rand() % 9 + 1;
		switch (dwCalc) {
		case 1:
	...
	} while (bBonuses[0] == wBonusID || bBonuses[1] == wBonusID || bBonuses[2] == wBonusID || bBonuses[3] == wBonusID || bBonuses[4] == wBonusID);

	LPTCOMP pCOMPANION = new TCOMP();
	pCOMPANION->m_bSlot = bSlot;

	pCOMPANION->m_dwMonID = wCompanionMonID;
	pCOMPANION->m_strName = strName;
	pCOMPANION->m_bBonusID = wBonusID;
	pCOMPANION->m_bEffect = 0;
	pCOMPANION->m_bLevel = 1;
	pCOMPANION->m_bSPoints = 1;
	pCOMPANION->m_dwExp = 0;
	pCOMPANION->m_wSTR = 0;
	pCOMPANION->m_wDEX = 0;
	pCOMPANION->m_wCON = 0;
	pCOMPANION->m_wINT = 0;
	pCOMPANION->m_wMEN = 0;
	pCOMPANION->m_wWIS = 0;
	pCOMPANION->m_wLife = 11000;
	pCOMPANION->m_dwTick = 0;
	pCOMPANION->m_dwNextExp = 3600;
	pCOMPANION->m_wItemID[0] = 18084;
	pCOMPANION->m_wItemID[1] = 0;
	pCOMPANION->m_dEndTime[0] = _AtlModule.m_timeCurrent + HOUR_ONE;
	pCOMPANION->m_dEndTime[1] = 0;

	pPlayer->m_mapComp.insert(MAPTCOMP::value_type(bSlot, pCOMPANION));
	...
	pPlayer->m_bCompanionSlot = bSlot;
	...
	pPlayer->SendCS_COMPANIONLIST_ACK();
	pPlayer->SendCS_CREATECOMPANION_ACK(0, bSlot);
	pPlayer->SendCS_CHARSTATINFO_ACK(pPlayer);
...
DWORD CTMapSvrModule::OnCS_DELETECOMPANION_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	...
	MAPTCOMP::iterator itCompanion = pPlayer->m_mapComp.find(bSlot);
	if (itCompanion == pPlayer->m_mapComp.end())
		return EC_NOERROR;

	if (pPlayer->m_bCompanionSlot == bSlot) {
		MAPTCOMPANION::iterator it;
		for (it = pPlayer->m_mapCompanion.begin(); it != pPlayer->m_mapCompanion.end(); it++)
			SendMW_SPOLECNIKMONDEL_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, (*it).second->m_dwID);
	...
	pPlayer->m_mapComp.erase(itCompanion);

	DEFINE_QUERY(&m_db, CSPDeleteCompanion);
	query->m_dwCharID = pPlayer->m_dwID;
	query->m_bSlot = bSlot;
	query->Call();
	UNDEFINE_QUERY();
	...
	for (BYTE i = bSlot + 1; i < MAX_COMPANION_COUNT; ++i)


	{
		itCompanion = pPlayer->m_mapComp.find(i);
		if (itCompanion != pPlayer->m_mapComp.end())

		{
			LPTCOMP pCOMP = (*itCompanion).second;
			pCOMP->m_bSlot--;

			pPlayer->m_mapComp.erase(itCompanion);
			pPlayer->m_mapComp.insert(MAPTCOMP::value_type(pCOMP->m_bSlot, pCOMP));
...
DWORD CTMapSvrModule::OnCS_HIDECOMPANION_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_bMain || !pPlayer->m_pMAP)
		return EC_NOERROR;
...
DWORD CTMapSvrModule::OnCS_USECOMPANIONITEM_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	CTItem* pItem = pInven->FindTItem(bItemID);
	if (!pItem || !pItem->m_bCount ||
			pItem->m_pTITEM->m_bType != IT_COMPANIONITEM)
		return EC_NOERROR;


	LPTITEM pNewItem = FindTItem(pItem->m_wItemID);

	MAPTCOMP::iterator jirker = pPlayer->m_mapComp.find(bCompanionSlot);
	if (jirker != pPlayer->m_mapComp.end()) {
		LPTITEM pITEMS[PETITEMS_COUNT];
		for (BYTE i = 0; i < PETITEMS_COUNT; ++i) {
			pITEMS[i] = NULL;
			if ((*jirker).second->m_wItemID[i])
				pITEMS[i] = FindTItem((WORD) (*jirker).second->m_wItemID[i]);
		}

		BYTE bSlot = 0;
		if (!pITEMS[0] || pITEMS[0]->m_bKind == pNewItem->m_bKind &&
				(!pITEMS[1] || pITEMS[1]->m_bKind != pNewItem->m_bKind)) {
			bSlot = 0;
		} else
			bSlot = 1;

		if ((bSlot == 0 && pITEMS[1] && pITEMS[1]->m_bKind == pNewItem->m_bKind) ||
				(bSlot == 1 && pITEMS[0] && pITEMS[0]->m_bKind == pNewItem->m_bKind)) {
			bSlot = !bSlot;
		}

		if (pITEMS[bSlot]) {
			LPTITEM pTITEM = pITEMS[bSlot];
			if (pTITEM) {
				VTITEM vITEM;
				vITEM.clear();
				...
				pNew->m_dlID = GenItemID();
				pNew->m_bCount = 1;
				pNew->m_wItemID = (WORD) (*jirker).second->m_wItemID[bSlot];
				pNew->m_pTITEM = pTITEM;
				pNew->m_dEndTime = (*jirker).second->m_dEndTime[bSlot];
		...
		(*jirker).second->m_wItemID[bSlot] = pNewItem->m_wItemID;
		(*jirker).second->m_dEndTime[bSlot] = pItem->m_dEndTime;

		pPlayer->SendCS_UPDATECOMPANIONITEMS_REQ(bCompanionSlot, bSlot, (*jirker).second->m_wItemID[bSlot], (*jirker).second->m_dEndTime[bSlot]);
	}
...
DWORD CTMapSvrModule::OnCS_DELETECOMPITEMS_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	LPTITEM pTITEM = FindTItem((WORD) (*jirker).second->m_wItemID[bTargetItem]);
	if (pTITEM) {
		VTITEM vITEM;
		vITEM.clear();
		...
		pNew->m_dlID = GenItemID();
		pNew->m_bCount = 1;
		pNew->m_wItemID = (WORD) (*jirker).second->m_wItemID[bTargetItem];
		pNew->m_pTITEM = pTITEM;
		pNew->m_dEndTime = (*jirker).second->m_dEndTime[bTargetItem];
		...
		(*jirker).second->m_wItemID[bTargetItem] = 0;
		(*jirker).second->m_dEndTime[bTargetItem] = 0;



		pPlayer->SendCS_UPDATECOMPANIONITEMS_REQ(bCompanionSlot, bTargetItem, 0, 0);
	}

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_CHANGECOMPANIONEFFECT_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	...
	MAPTCOMP::iterator jirker = pPlayer->m_mapComp.find(bCompanionSlot);
	if (jirker != pPlayer->m_mapComp.end()) {
		DWORD dwPrice = (*jirker).second->m_bEffect == 0 ? 1500 : 400;
		if (pPlayer->m_dwMedals < dwPrice) {
			pPlayer->SendCS_PETEFFECTCHANGE_ACK(EFFECT_NEEDCASH, 0);
			return EC_NOERROR;
		}

		pPlayer->m_dwMedals -= dwPrice;
		pPlayer->SendCS_UPDATEMEDALS_REQ(pPlayer->m_dwMedals);

		BYTE bNewEffect = 0;
		while (TRUE) {
			bNewEffect = rand() % (MAX_EFFECT - MIN_EFFECT) + MIN_EFFECT;
			if ((*jirker).second->m_bEffect != bNewEffect)
				break;
		}



		(*jirker).second->m_bEffect = bNewEffect;
...
DWORD CTMapSvrModule::OnCS_USECOMPANIONPOWDER_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	if (!pItem || !pItem->m_bCount ||
			pItem->m_pTITEM->m_bType != IT_USE ||
			pItem->m_pTITEM->m_bKind != IK_POWDER)
		return EC_NOERROR;
	...
	if (jirker != pPlayer->m_mapComp.end()) {
		...
		(*jirker).second->m_bBonusID = wBonusID;
		pPlayer->SendCS_UPDATECOMPANIONBONUS_REQ(bCompanionSlot);
	}
...
DWORD CTMapSvrModule::OnCS_USECOMPRESET_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	if (!pItem || !pItem->m_bCount ||
			pItem->m_pTITEM->m_bType != IT_USE ||
			pItem->m_pTITEM->m_bKind != IK_COMPRESET)
		return EC_NOERROR;

	BYTE bPoints = 0;
	MAPTCOMP::iterator jirker = pPlayer->m_mapComp.find(bCompanionSlot);
	if (jirker != pPlayer->m_mapComp.end()) {
		BYTE* bAttr[TCLASS_COUNT] =
		{
			&(*jirker).second->m_wSTR,
			&(*jirker).second->m_wDEX,
			&(*jirker).second->m_wCON,
			&(*jirker).second->m_wINT,
			&(*jirker).second->m_wMEN,
			&(*jirker).second->m_wWIS,
		};
		...
		if ((*jirker).second->m_bLevel >= 18)
			bPoints -= 5 * TCLASS_COUNT;
		(*jirker).second->m_bSPoints += bPoints;
...
DWORD CTMapSvrModule::OnCS_USEMEDALITEM_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
#ifdef BOW_COMPILE_MODE
	return EC_NOERROR;
#endif
	...
	CTItem* pItem = pInven->FindTItem(bSlot);
	if (!pItem || !pItem->m_bCount || pItem->m_pTITEM->m_bType != IT_PACKAGE || pItem->m_pTITEM->m_bKind != 118)
		return EC_NOERROR;

	if (pItem->m_bCount < bCount)
		return EC_NOERROR;

	pPlayer->m_dwMedals += (pItem->m_pTITEM->m_wUseValue * bCount);
	pPlayer->SendCS_UPDATEMEDALS_REQ(pPlayer->m_dwMedals);
	UseItem(pPlayer, pInven, pItem, bCount);
...
DWORD CTMapSvrModule::OnCS_VERIFYSESSION_ACK(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	DWORD dwCharID;
...
DWORD CTMapSvrModule::OnCS_PINGMEASUREMENT_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP)
		return EC_NOERROR;
	...
	pPlayer->SendCS_PINGMEASUREMENT_ACK(dwTick);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_REGISTERBOW_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain || pPlayer->IsTournamentPlayer())
		return EC_NOERROR;

	SendMW_ADDTOBOWQUEUE_REQ(
		pPlayer->m_dwID,
		pPlayer->m_dwKEY);
...
DWORD CTMapSvrModule::OnCS_CANCELBOWQUEUE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;

	SendMW_CANCELBOWQUEUE_REQ(
		pPlayer->m_dwID,
		pPlayer->m_dwKEY);
...
DWORD CTMapSvrModule::OnCS_CASHBOWRESPAWN_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	pPlayer->Revival(AFTERMATH_NONE, NULL, 0);
...
DWORD CTMapSvrModule::OnCS_EXCHANGEBOWBP_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
...
#ifndef BOW_COMPILE_MODE
#else
	if (pPlayer->m_dwMedals < dwMedals)
		return EC_NOERROR;

	pPlayer->m_dwMedals -= dwMedals;
...
DWORD CTMapSvrModule::OnCS_LEAVEBATTLEFIELD_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;

#if defined(BR_COMPILE_MODE)
	LPBRPLAYER pMainPlayer = _AtlModule.GetBRPlayerByTeamID(pPlayer->m_dwTeamID, pPlayer->m_dwID);
	if (!pMainPlayer)
		return EC_NOERROR;
	...
	if (m_bBRStatus != BS_BODPEACE && !pMainPlayer->m_bDead)
		pPlayer->SendCS_LEAVEBATTLEFIELD_ACK(BOWLEAVE_FAIL);
	else
		SendMW_LEAVEBATTLEFIELD_REQ(pPlayer->m_dwID, pPlayer->m_dwKEY);
#endif
#if defined(BOW_COMPILE_MODE) && !defined(BR_COMPILE_MODE)
	if (m_bBOWStatus != BS_BODPEACE)
...
DWORD CTMapSvrModule::OnCS_BOWRANKING_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
...
#ifndef BOW_COMPILE_MODE
#else
	VBOWRANKING vPlayer; vPlayer.clear();
	MAPBOWGRANKING mapGuild; mapGuild.clear();
	MAPBOWGRANKING::iterator guildFinder;
	MAPPLAYER::iterator onlineFinder;
	MAPSAVEDPLAYER::iterator offlineFinder;
	DWORD dwGuildID = 0;

	for (MAPBOWPLAYER::iterator itPlayer = m_mapBOWPLAYER.begin(); itPlayer != m_mapBOWPLAYER.end(); ++itPlayer) {
		LPBOWRANKING pPLAYERRANK = new BOWRANKING();
		onlineFinder = m_mapPLAYER.find((*itPlayer).second->m_dwCharID);
		if (onlineFinder != m_mapPLAYER.end()) {
			pPLAYERRANK->m_dwCharID = (*onlineFinder).second->m_dwID;
			pPLAYERRANK->m_strName = (*onlineFinder).second->m_strNAME;
			pPLAYERRANK->m_strGuildName = (*onlineFinder).second->m_strGuildName;
			pPLAYERRANK->m_dwBP = (*onlineFinder).second->m_dwTotalPoints;
			pPLAYERRANK->m_dwSP = (*onlineFinder).second->m_dwPoints;
			pPLAYERRANK->m_bCountry = (*onlineFinder).second->m_bCountry;
			pPLAYERRANK->m_bGuildCountry = (*onlineFinder).second->m_bGuildCountry;
			dwGuildID = (*onlineFinder).second->m_dwGuildID;
		} else {
			offlineFinder = m_mapSAVEDPLAYER.find((*itPlayer).second->m_dwCharID);
			if (offlineFinder == m_mapSAVEDPLAYER.end())
				continue;

			pPLAYERRANK->m_dwCharID = (*itPlayer).second->m_dwCharID;
			pPLAYERRANK->m_bCountry = (*itPlayer).second->m_bCountry;
			pPLAYERRANK->m_strName = (*offlineFinder).second->m_strNAME;
			pPLAYERRANK->m_strGuildName = (*offlineFinder).second->m_strGuildName;
			pPLAYERRANK->m_dwBP = (*offlineFinder).second->m_dwTotalPoints;
			pPLAYERRANK->m_dwSP = (*offlineFinder).second->m_dwPoints;
			pPLAYERRANK->m_bGuildCountry = (*offlineFinder).second->m_bGuildCountry;
			dwGuildID = (*offlineFinder).second->m_dwGuildID;
		}

		if (dwGuildID) {
			guildFinder = mapGuild.find(dwGuildID);
			if (guildFinder == mapGuild.end()) {
				LPBOWGRANKING pGUILDRANK = new BOWGRANKING();
				pGUILDRANK->m_dwGuildID = dwGuildID;
				pGUILDRANK->m_bCountry = pPLAYERRANK->m_bGuildCountry;
				pGUILDRANK->m_strName = pPLAYERRANK->m_strGuildName;
				pGUILDRANK->m_dwBP = pPLAYERRANK->m_dwBP;
				pGUILDRANK->m_dwSP = pPLAYERRANK->m_dwSP;

				mapGuild.insert(MAPBOWGRANKING::value_type(dwGuildID, pGUILDRANK));
			} else {
				(*guildFinder).second->m_dwBP += pPLAYERRANK->m_dwBP;
				(*guildFinder).second->m_dwSP += pPLAYERRANK->m_dwSP;
			}
		}

		vPlayer.push_back(pPLAYERRANK);
	}

	pPlayer->SendCS_BOWRANKING_ACK(vPlayer, mapGuild);
#endif

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_ACTITEMSUSE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	DWORD dwDefendID;
	BYTE bDefendType;
	...
	CTSkillTemp* pSkill = FindTSkill(wSkillID);
	if (!pSkill) {
		pPlayer->SendCS_SKILLUSE_ACK(
			SKILL_NOTFOUND,
			dwAttackID,
	...
	if (pSkill->m_wMapID != INVALID_MAPID &&
			pSkill->m_wMapID != pPlayer->m_wMapID) {
		pPlayer->SendCS_SKILLUSE_ACK(
			SKILL_WRONGREGION,
			dwAttackID,
			bAttackType,
	...
	if (IsTournamentMap(pATTACK->m_wMapID)) {
		LPTOURNAMENTPLAYER pTP = FindTournamentPlayer(pPlayer->m_dwID, TRUE);
		if (!pTP || pSkill->m_wID == TOURNAMENT_CANTUSESKILL) {
			pPlayer->SendCS_SKILLUSE_ACK(
				SKILL_NOTFOUND,
				dwAttackID,
				bAttackType,
	...
	if (pPlayer->m_wArenaID && pSkill->m_wID == TOURNAMENT_CANTUSESKILL) {
		pPlayer->SendCS_SKILLUSE_ACK(
			SKILL_NOTFOUND,
			dwAttackID,
			bAttackType,
	...
	if (!pATTACK->CheckPrevAct(pSkill->m_wPrevActiveID)) {
		pPlayer->SendCS_SKILLUSE_ACK(
			SKILL_NEEDPREVACT,
			dwAttackID,
			bAttackType,
	...
	if (bAttackType != OT_MON) {
		...
		pATTACK->EraseBuffByAttack(pSkill);
		pATTACK->CheckEternalBuff();

		CTPlayer* pAttacker = static_cast<CTPlayer*>(pATTACK);
		if (!pAttacker) {
			pPlayer->SendCS_SKILLUSE_ACK(
				SKILL_NOTFOUND,
				dwAttackID,
		...
		for (BYTE i = 0; i < pAttacker->m_vUsedSkill.size(); ++i) {
			if (pAttacker->m_vUsedSkill[i]->m_wSkillID == pSkill->m_wID) {
				pAttacker->m_vUsedSkill.erase(pAttacker->m_vUsedSkill.begin() + i);
			}
		}

		LPUSEDSKILL UsedSkill = new USEDSKILL();
		UsedSkill->m_wSkillID = pSkill->m_wID;
		UsedSkill->m_dwTick = m_dwTick;
		UsedSkill->m_bRepeatCount = 0;

		pAttacker->m_wLastSkillID = pSkill->m_wID;
		pAttacker->m_vUsedSkill.push_back(UsedSkill);

		LPTITEM pITEM = FindTItem(pSkill->m_wUseItem);
		if (pITEM) {
			MAPDWORD::iterator it;
			it = pAttacker->m_mapItemCoolTime.find(pITEM->m_wDelayGroupID);
			if (it != pAttacker->m_mapItemCoolTime.end()) {
				DWORD dwTick = (*it).second;
				if (dwTick < _AtlModule.m_dwTick)
					(*it).second = _AtlModule.m_dwTick + pITEM->m_dwDelay;
				else {
					pPlayer->SendCS_SKILLUSE_ACK(
						SKILL_SPEEDYUSE,
						dwAttackID,
				...
				pAttacker->m_mapItemCoolTime.insert(MAPDWORD::value_type(pITEM->m_wDelayGroupID, _AtlModule.m_dwTick + pITEM->m_dwDelay));

			pAttacker->SendCS_ITEMUSE_ACK(IU_SUCCESS, pITEM->m_wDelayGroupID, pITEM->m_bKind, pITEM->m_dwDelay);
		}

		if (!pAttacker->HaveItem(pSkill->m_wUseItem, 1)) {
			pPlayer->SendCS_SKILLUSE_ACK(
				SKILL_NEEDITEM,
				dwAttackID,
				bAttackType,
		...
		pAttacker->UseItem(pSkill->m_wUseItem, 1);
		...
		VDWORD vDEFENDID; vDEFENDID.clear();
		VBYTE vDEFENDTYPE; vDEFENDTYPE.clear();

		vDEFENDID.push_back(dwDefendID);
		vDEFENDTYPE.push_back(bDefendType);
...
DWORD CTMapSvrModule::OnCS_FINISHSKILL_ACK(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP)
		return EC_NOERROR;
	...
	DWORD dwMgMaxPower;
	WORD wAttackLevel;
	WORD wAttackPartyID = 0;
	BYTE bDefendCount;
	FLOAT fPosX, fPosY, fPosZ;
	BOOL IsLinked;
	BYTE bHitType = HT_NORMAL;
	BOOL IsFake;
	DWORD dwID;
	...
	VDWORD vDefendID; vDefendID.clear();
	VBYTE vDefendType; vDefendType.clear();
	for (BYTE i = 0; i < bDefendCount; ++i) {
		pBUF->m_packet
			>> dwTargetID
			>> bTargetType;

		vDefendID.push_back(dwTargetID);
		vDefendType.push_back(bTargetType);
	}
	...
	if (pTemp->m_bRunFromServer == 1 ||
			(pTemp->m_wMapID != INVALID_MAPID &&
			 pTemp->m_wMapID != pPlayer->m_wMapID)) {
		return EC_NOERROR;
	}

	CTObjBase* pATTACK = NULL;
	MAPPLAYER::iterator itPl = m_mapPLAYER.find(dwAttackID);
	if (itPl != m_mapPLAYER.end())
		pATTACK = (*itPl).second;

	if (bType != OT_PC) {
		pATTACK = FindTarget(pPlayer, bType, dwID);
		if (!pATTACK)
			return EC_NOERROR;
	}

	if (pATTACK && bType == OT_PC) {
		CTSkill* pSkill = pATTACK->FindTSkill(pTemp->m_wID);
		if (pTemp->m_bRepeatCount) {
			for (BYTE i = 0; i <= 2; ++i) {
				if (!pSkill)
					pSkill = pATTACK->FindTSkill(pTemp->m_wID - i);
				else
					break;
			}
		}

		if (pTemp->m_bKind != 21 && pSkill) {
			BOOL bFound = FALSE;
			CString strMessage = NAME_NULL;
			for (BYTE i = 0; i < pPlayer->m_vUsedSkill.size(); ++i) {
				if (pPlayer->m_vUsedSkill[i]->m_wSkillID == pSkill->m_pTSKILL->m_wID) {
					bFound = TRUE;
					if (pPlayer->m_vUsedSkill[i]->m_bRepeatCount > (pTemp->m_bRepeatCount + 1) && (pTemp->m_wID < 31 && pTemp->m_wID > 34 && pTemp->m_wID != 38)) {
						strMessage.Format("Limit exceeded. Closing Player : %d", pPlayer->m_dwID);
						LogSmartly(strMessage, TRUE);
						CloseSession(pPlayer);
						return EC_NOERROR;
					}

					pPlayer->m_vUsedSkill[i]->m_bRepeatCount++;
					break;
				}
			}

			if (!bFound) {
				strMessage.Format("Not found skill. Closing Player : %d", pPlayer->m_dwID);
				LogSmartly(strMessage, TRUE);
				//	CloseSession(pPlayer);
				//	return EC_NOERROR;
			}

			if (pSkill->CanUse(m_dwTick) && pTemp->m_dwReuseDelay && (pTemp->m_wID < 31 && pTemp->m_wID > 34 && pTemp->m_wID != 38)) {
				strMessage.Format("Cant use. Player : %d", pPlayer->m_dwID);
				LogSmartly(strMessage, TRUE);
				return EC_NOERROR;
			}

			if (pTemp->m_dwActionTime && pATTACK->m_wLastChargeSkill == wSkillID) {
				if (pTemp->m_dwActionTime > m_dwTick - pSkill->m_dwChargeTick) {
					strMessage.Format("NoCast. Player : %d", pPlayer->m_dwID);
					LogSmartly(strMessage, TRUE);
					return EC_NOERROR;
				}
			}
		} else if (pTemp->m_bKind == 21) {
			BOOL bFound = FALSE;
			CString strMessage = NAME_NULL;
			for (BYTE i = 0; i < pPlayer->m_vUsedSkill.size(); ++i) {
				if (pPlayer->m_vUsedSkill[i]->m_wSkillID == pTemp->m_wID) {
					bFound = TRUE;
					if (pPlayer->m_vUsedSkill[i]->m_bRepeatCount > (pTemp->m_bRepeatCount + 1)) {
						strMessage.Format("Limit exceede. Closing Player : %d", pPlayer->m_dwID);
						LogSmartly(strMessage, TRUE);
						CloseSession(pPlayer);
						return EC_NOERROR;
					}
			...
			if (!bFound) {
				strMessage.Format("(acts)Not found skill. Closing Player : %d", pPlayer->m_dwID);
				LogSmartly(strMessage, TRUE);
				CloseSession(pPlayer);
				return EC_NOERROR;
			}
	...
	//	if (pATTACK)
			//wAttackPartyID = pATTACK->GetPartyID();

	if (!pTemp->m_bRunFromServer) {
		if (pATTACK) {
			CTSkill* pAtkSkill = pATTACK->FindTSkill(pTemp->m_wTriggerID);
			if (pTemp->m_bKind != 21) {
				if (pTemp->m_wTriggerID == wSkillID && pAtkSkill)
					pATTACK->m_pInstanceSkill = pAtkSkill;
			}
			...
			bSkillLevel = pAtkSkill ? pAtkSkill->m_bLevel : 0;
			bCP = pAtkSkill ? pATTACK->GetCritical(pAtkSkill) : 0;
			bAttackCountry = pATTACK->m_bCountry;
			bAttackAidCountry = pATTACK->m_bAidCountry;

			BYTE bIsPysLong = pTemp->IsLongAttack();
			if (bType == OT_SELF && ((CTSelfObj*) pATTACK)->m_bRecallType == TRECALLTYPE_SKILL) {
				if (pPlayer->m_bMain) {
					dwPysMinPower = pPlayer->GetMinAP(bIsPysLong);
					dwPysMaxPower = pPlayer->GetMaxAP(bIsPysLong);
					dwMgMinPower = pPlayer->GetMinMagicAP();
					dwMgMaxPower = pPlayer->GetMaxMagicAP();
					wAttackLevel = pTemp->GetAttackType() == SAT_PHYSIC ? pPlayer->GetAttackLevel() : pPlayer->GetMagicAtkLevel();
				}
			} else {
				if (pTemp->m_wTriggerID == wSkillID && pAtkSkill)
					pATTACK->m_pInstanceSkill = pAtkSkill;
				...
				dwMgMinPower = pATTACK->GetMinMagicAP();
				dwMgMaxPower = pATTACK->GetMaxMagicAP();
				wAttackLevel = pTemp->GetAttackType() == SAT_PHYSIC ? pATTACK->GetAttackLevel() : pATTACK->GetMagicAtkLevel();
				pATTACK->m_pInstanceSkill = NULL;
			}
		} else {
			if (bTargetType == OT_MON)
			...
			if (!pATTACK && pTemp->m_bCheckAttacker)
				return EC_NOERROR;
		}
	}
	...
	BYTE bAttackClass = TCLASS_COUNT;
	if (bType == OT_PC)
		bAttackClass = pPlayer->m_bClass;

	if ((BYTE) vDefendID.size() < bDefendCount ||
			(BYTE) vDefendType.size() < bDefendCount) {
		return EC_NOERROR;
	}
	...
	if (GuildSkill != m_mapGuildSkill.end()) {
		...
		if (pPlayer->m_bGuildDuty < GuildSkill->second.m_bType)
			return EC_NOERROR;
	}
	for (BYTE i = 0; i < bDefendCount; ++i) {
		CTObjBase* pDEFEND = FindTarget(pPlayer, vDefendType[i], vDefendID[i]);
		if (vDefendType[i] == OT_PC)
			pDEFEND = FindChar(vDefendID[i]);
		else if (vDefendType[i] == OT_MON)
			pDEFEND = FindTarget(pPlayer, vDefendType[i], vDefendID[i]);
		else {
			if (!pATTACK || !pATTACK->m_pMAP)
				return EC_NOERROR;

			MAPTCELL mapCells; mapCells.clear();
			pATTACK->m_pMAP->GetNeighbor(&mapCells, pATTACK->m_fPosX, pATTACK->m_fPosZ);
			for (MAPTCELL::iterator itCell = mapCells.begin(); itCell != mapCells.end(); ++itCell) {
				if ((*itCell).second->IsMainCell(pATTACK->m_bChannel)) {
					CTCell* pCell = (*itCell).second;
					switch (vDefendType[i]) {
					case OT_RECALL:
					{
						MAPTRECALLMON::iterator it = pCell->m_mapRECALLMON.find(vDefendID[i]);
						if (it == pCell->m_mapRECALLMON.end())
							continue;
					...
					case OT_SELF:
					{
						MAPTSELFOBJ::iterator it = pCell->m_mapSELFOBJ.find(vDefendID[i]);
						if (it == pCell->m_mapSELFOBJ.end())
							continue;
		...
		if (pDEFEND && vDefendType[i] == OT_PC) {
			if (!((CTPlayer*) pDEFEND)->CanDuel(pPlayer))
				continue;
		}

		if (pDEFEND) {
			if (vDefendType[i] == OT_MON &&
					pDEFEND->m_bCountry != TCONTRY_N &&
					(GetAttackCountry(bAttackCountry, bAttackAidCountry) == TCONTRY_B ||
					 GetAttackCountry(bAttackCountry, bAttackAidCountry) == pDEFEND->m_bCountry) &&
					pTemp->IsNegative()) {
			...
			if (pDEFEND->m_bType != OT_MON &&
					pDEFEND->m_pLocal &&
					pTemp->IsNegative() &&
					!pDEFEND->m_pLocal->m_bCanBattle &&
					(OT_PC != pDEFEND->m_bType ||
					 dwAttackID != pDEFEND->m_dwID)) {
				continue;
			}

			BYTE bMirror = FALSE;
			if (bType == OT_RECALL && IsFake)
				bHitType = HT_MISS;
			else
				bHitType = pDEFEND->GetAtkHitType(pTemp, bCP, wAttackLevel, bSkillLevel, pATTACK ? pATTACK->m_bLevel : 1, pATTACK ? pATTACK->m_bEquipSpecial : FALSE);
			...
			/*
						...
						{	// ��a��Lzd
							...
							if(pATTACK)
								pDEFEND = FindTarget(pPlayer, OT_PC, dwAttackID);
							else
								pDEFEND = NULL;

							if(pDEFEND)
							{
								dwAttackID = vDefendID[i];
								bHitType = HT_NORMAL;
			...
			*/
			if (pDEFEND) {
				if (pTemp->m_wID == 229 || pTemp->m_wID == 128) {
					if (pDEFEND->FindMaintainSkill(pTemp->m_wID + 1))
						pDEFEND->EraseMaintainSkill(pTemp->m_wID + 1);
				}

				if (wSkillID == 3604) {
					auto Skill = pPlayer->FindTSkill(3603);
					if (!Skill)
						return EC_NOERROR;

					bSkillLevel = Skill->m_bLevel;
				}
				pDEFEND->Defend(
					pTemp,
					bSkillLevel,
					...
					dwMgMinPower,
					dwMgMaxPower,
					pTemp->GetTransHPMPFromType(SCT_HPTRANS),
					pTemp->GetTransHPMPFromType(SCT_MPTRANS),
					0,
					0,
					bType == OT_PC ? TRUE : FALSE,
					wAttackLevel,
					pATTACK ? pATTACK->m_bLevel : 1,
					bCP,
					bHitType,
					0,
					fPosX, fPosY, fPosZ,
					pDEFEND->m_fPosX, pDEFEND->m_fPosY, pDEFEND->m_fPosZ,
					bMirror);
			}
		}
	}
	...
	if (pATTACK) {
		for (BYTE i = 0; i < pTemp->m_vData.size(); ++i) {
			if (pTemp->m_vData[i]->m_bExec == SDT_STATUS_LINK) {
				CTSkillTemp* pTempLink = FindTSkill(pTemp->m_vData[i]->m_wValue);
				if (pTempLink)
					if (!pATTACK->FindMaintainSkill(pTempLink->m_wID) && !pTempLink->m_bTargetRange && bHitType != HT_MISS)
						pATTACK->ForceMaintain(pTempLink->m_wID, pATTACK->m_dwID, OT_PC, pATTACK->m_dwID, OT_PC, pTempLink->m_dwDuration);
			}
		}
	}
...
DWORD CTMapSvrModule::OnCS_REGISTERBR_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain || pPlayer->IsTournamentPlayer())
		return EC_NOERROR;
...
DWORD CTMapSvrModule::OnCS_BRTEAMMATEADD_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	SendMW_BRTEAMMATEADD_REQ(pPlayer->m_dwID, pPlayer->m_dwKEY, strNAME);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_BRTEAMMATEDEL_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	SendMW_BRTEAMMATEDEL_REQ(pPlayer->m_dwID, pPlayer->m_dwKEY, strNAME);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_BRTEAMMATEADDRESULT_ACK(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	SendMW_BRTEAMMATEADDRESULT_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, bResult, strNAME);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_VOTEFORBRMAP_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	SendMW_VOTEFORBRMAP_REQ(pPlayer->m_dwID, pPlayer->m_dwKEY, strMap, Mode);

	return EC_NOERROR;
}

#ifdef BR_COMPILE_MODE
DWORD CTMapSvrModule::OnCS_DONATELIFE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	if (!pBRPLAYER || !pBRPLAYER->m_bLife || pBRPLAYER->m_bDead) {
		//ack handler
		return EC_NOERROR;
	}

	LPBRTEAM pTEAM = NULL;
	LPBRPLAYER pMATE = NULL;
	...
	for (MAPBRPLAYER::iterator it = pTEAM->m_mapBRPLAYER.begin(); it != pTEAM->m_mapBRPLAYER.end(); ++it) {
		if (nID == bIndex) {
			pMATE = (*it).second;
			break;
		}
	...
	if (!pMATE || pMATE->m_bDead)
		return EC_NOERROR;

	pBRPLAYER->m_bLife--;
	pMATE->m_bLife++;

	for (MAPBRPLAYER::iterator itPlayer = pTEAM->m_mapBRPLAYER.begin(); itPlayer != pTEAM->m_mapBRPLAYER.end(); ++itPlayer) {
		CTPlayer* pMate = _AtlModule.FindPlayer((*itPlayer).second->m_dwCharID, (*itPlayer).second->m_dwKEY);
		if (pMate)
			pMate->SendCS_UPDATEBRMATE_ACK(pTEAM->m_mapBRPLAYER);
	}
...
DWORD CTMapSvrModule::OnCS_FINISHCOMPANIONTRANSFER_ACK(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	if (!pItem || !pItem->m_bCount ||
			pItem->m_pTITEM->m_bType != IT_USE ||
			pItem->m_pTITEM->m_bKind != IK_PETTRANSFORM)
		return EC_NOERROR;

	MAPTCOMP::iterator it = pPlayer->m_mapComp.find(bSlot);
	if (it == pPlayer->m_mapComp.end())
		return EC_NOERROR;

	if ((*it).second->m_dwMonID == pItem->m_dwExtValue[IEV_COMPANION])
		return EC_NOERROR;

	UseItem(pPlayer, pInven, pItem, 1);

	(*it).second->m_dwMonID = pItem->m_dwExtValue[IEV_COMPANION];
	if (bSlot == pPlayer->m_bCompanionSlot)
		pPlayer->RespawnCompanion();
...
DWORD CTMapSvrModule::OnCS_CMTELEPORTBATTLEMODE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	SendMW_CMTELEPORTBATTLEMODE_REQ(pPlayer->m_dwID, pPlayer->m_dwKEY, bType);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_CUSTOMCLOAK_LIST_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;


	for (auto itPtr : pPlayer->m_mapTCUSTOMCLOAK)
		delete itPtr.second;
	pPlayer->m_mapTCUSTOMCLOAK.clear();
	...
	DEFINE_QUERY(&m_db, CTBLCustomCloakTable)
		query->m_dwUserID = pPlayer->m_dwUserID;
	if (query->Open()) {
		while (query->Fetch()) {
			LPTCUSTOMCLOAK pCloak = new TCUSTOMCLOAK();

			pCloak->m_wID = query->m_wID;
			pCloak->m_dDateCreate = __DBTOTIME(query->m_dDateCreate);
			pPlayer->m_mapTCUSTOMCLOAK.insert(MAPTCUSTOMCLOAK::value_type(pCloak->m_wID, pCloak));
		}
	...
	UNDEFINE_QUERY()
		...
		pPlayer->SendCS_CUSTOMCLOAK_LIST_ACK();

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_CUSTOMCLOAK_CHANGE_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	CTInven* pInven = pPlayer->FindTInven(INVEN_EQUIP);
	if (!pInven)
		return EC_NOERROR;

	CTItem* pItem = pInven->FindTItem(ES_BACK);
	if (!pItem)
		return EC_NOERROR;
	...
	if (bEnoughCash) {
		DEFINE_QUERY(&m_db, CSPGetCustomCloak);
		query->m_dwUserID = pPlayer->m_dwUserID;
		query->m_wID = wID;
		if (query->Call()) {
			bRET = query->m_nRET;
			if (bRET || wID == 0) {
				pItem->m_dwExtValue[IEV_CUSTOMTEX] = wID;
				if (wID == 0)
					bRET = 1;
			}
		} else {
		...
		if (bRET == 0)
			pPlayer->SendCS_CUSTOMCLOAK_CHANGE_ACK(pPlayer->m_dwID, 0, bRET);
		else {
			pPlayer->SendCS_CUSTOMCLOAK_CHANGE_ACK(pPlayer->m_dwID, wID, bRET);
			VPLAYER vPLAYERS;
			vPLAYERS.clear();
			...
			while (!vPLAYERS.empty()) {
				CTPlayer* pChar = vPLAYERS.back();
				if (pChar->m_dwID != pPlayer->m_dwID)
					pChar->SendCS_CUSTOMCLOAK_CHANGE_ACK(pPlayer->m_dwID, wID, bRET);
...
DWORD CTMapSvrModule::OnCS_QUESTPOSEXEC_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	auto Quest = pPlayer->m_mapQUEST.find(dwQuestID);
	if (Quest == pPlayer->m_mapQUEST.end())
		return EC_NOERROR;



	auto Term = Quest->second->FindTerm(dwTermID, QTT_HUNT);
	if (!Term)
		return EC_NOERROR;
	Quest->second->FinishTerm(dwTermID);
	Quest->second->m_bSave = TRUE;
	pPlayer->SendCS_QUESTUPDATE_ACK(dwQuestID, dwTermID, QTT_HUNT, 1, QTS_SUCCESS);


	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_GUILDSKILLACTION_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	wParam = new WORD[ParamSize];
	WORD* ParamNew = nullptr;

	for (BYTE i = 0; i < ParamSize; ++i)
		pBUF->m_packet >> wParam[i];

	switch (Action) {
	case GS_BUY:
	{
		auto GuildSkill = m_mapGuildSkill.find(wParam[0]);
		if (GuildSkill == m_mapGuildSkill.end())
			return EC_NOERROR;

		if (!pPlayer->CheckGuildDuty(GuildSkill->second.m_bType))
			return EC_NOERROR;

		auto Skill = pPlayer->m_mapTSKILL.find(wParam[0]);
		if (Skill != pPlayer->m_mapTSKILL.end())
			if (Skill->second->m_bLevel >= Skill->second->m_pTSKILL->m_bMaxLevel)
				return EC_NOERROR;

		ParamNew = new WORD[ParamSize + 1];
		ParamNew[0] = wParam[0];
		ParamNew[1] = GuildSkill->second.m_bType;
	}
	break;
	}

	SendMW_GUILDSKILLACTION_REQ(pPlayer->m_dwID, pPlayer->m_dwKEY, Action, ParamNew ? ParamNew : wParam);
...
DWORD CTMapSvrModule::OnCS_ARENAINFO_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	auto itArena = m_mapArena.find(Arena);
	if (itArena == m_mapArena.end())
		return EC_NOERROR;

	pPlayer->SendCS_ARENAINFO_ACK((*itArena).second);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnCS_ARENARANKING_REQ(LPPACKETBUF pBUF) {
	CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;
	if (!pPlayer->m_pMAP || !pPlayer->m_bMain)
		return EC_NOERROR;
	...
	pPlayer->SendCS_ARENARANKING_ACK();
