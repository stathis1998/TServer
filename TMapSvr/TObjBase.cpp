...
void CTObjBase::CalcDamage(DWORD dwAttackID,
	...
	try {
		...
		for (DWORD i = 0; i < pSkill->m_pTSKILL->m_vData.size(); i++) {
			...
			case SATT_LONG:
			{
				...
				case MTYPE_DAMAGE:
				{
					...
					CTObjBase* pOBJ = m_pMAP->FindNeighbor(bAttackType, dwAttackID, m_fPosX, m_fPosZ);

					if (pOBJ) {
						if (pOBJ->m_bType == OT_RECALL) {
							if (((CTRecallMon*) pOBJ)->m_bRecallType == TRECALLTYPE_AUTOAI)
								dwValue *= 3;
						}
					}
		...
		CString x;
		x.Format("Exception : 0x02 ; %s", ex.what());
		_AtlModule.LogSmartly(x);
	}
}
...
void CTObjBase::Defend(CTSkillTemp* pTemp,
	...
	if (bAtkHit != HT_MISS &&
		...
		if (pTemp->m_wID >= 3000) {
			...
			CTPlayer* Attack = _AtlModule.FindPC(dwAttackID);
			if (!Attack)
				return;

			switch (pTemp->m_wID) {
			case 3604:
				Dmg = GetMaxHP() * (bSkillLevel / 100.0f);
				break;
			case 3301:
				Dmg = Attack->GetMaxHP() * ((10 * bSkillLevel) / 100.0f);
				break;
			case 3600:
			case 3601:
				Dmg = Attack->GetMaxHP() * ((5 + (5 * bSkillLevel)) / 100.0f);
				break;
			default:
				CalcDamage(
					dwAttackID,
			...
			if (Dmg) {
				...
				auto Damage = mapDamage.begin();
				if (Damage == mapDamage.end())
					return;
		...
		if (bAttackType == OT_PC) {
			CTPlayer* Attack = _AtlModule.FindPC(dwAttackID);
			if (Attack) {
				CTSkill* LifeSteal = Attack->FindMaintainSkill(3302);
				if (LifeSteal) {
					if (Attack->m_dwHP) {
						FLOAT Add = (nDamageHP * ((3 + LifeSteal->m_bLevel) / 100.0f));
						Attack->m_dwHP += (DWORD) Add;
						Attack->SendCS_HPMP_ACK(Attack->m_dwID, Attack->m_bType, Attack->m_bLevel, Attack->GetMaxHP(), Attack->m_dwHP, Attack->GetMaxMP(), Attack->m_dwMP);
					}
				}
			}
		}

		DWORD LeftHP = OnDamage(
			dwHostID,
			dwAttackID,
			bAttackType,
			bAttackCountry,
...
FLOAT CTObjBase::GetBaseStatValue(BYTE bType, WORD wBaseMIN) {
	...
	try {
		...
		fValue = max(wBaseMIN, wBase) * pow(GetFormulaRateX(FTYPE_1ST), m_bLevel - 1);
	} catch (...) {
		_AtlModule.LogSmartly("Error in GetBaseStatusValue");
		return 0.0f;
	}
...
DWORD CTObjBase::GetMaxMP() {
	...
	if (m_bType == OT_PC) {
		CTPlayer* pPlayer = (CTPlayer*) this;
		if (pPlayer)
			dwMaxMP += (DWORD) pPlayer->CalcPetBonus(MTYPE_MMP);
	}
...
WORD CTObjBase::GetAttackLevel() {
	...
	if (m_bType == OT_PC) {
		CTPlayer* pPlayer = (CTPlayer*) this;
		if (pPlayer) {
			dwAL += (DWORD) pPlayer->CalcPetBonus(MTYPE_AL);
			if (pPlayer->m_dwGuildID && pPlayer->m_StatLevel)
				dwAL += CALCULATE_DAMAGE(pPlayer->m_StatLevel);
		}
	}
...
WORD CTObjBase::GetDefendLevel() {
	...
	if (m_bType == OT_PC) {
		CTPlayer* pPlayer = (CTPlayer*) this;
		if (pPlayer) {
			dwDL += (DWORD) pPlayer->CalcPetBonus(MTYPE_DL);
			if (pPlayer->m_dwGuildID && pPlayer->m_StatLevel)
				dwDL += CALCULATE_ARMOR(pPlayer->m_StatLevel);
		}
	}
...
WORD CTObjBase::GetMagicAtkLevel() {
	...
	if (m_bType == OT_PC) {
		CTPlayer* pPlayer = (CTPlayer*) this;
		if (pPlayer) {
			dwAL += (DWORD) pPlayer->CalcPetBonus(MTYPE_MAL);
			if (pPlayer->m_dwGuildID && pPlayer->m_StatLevel)
				dwAL += CALCULATE_DAMAGE(pPlayer->m_StatLevel);
		}
	}
...
WORD CTObjBase::GetMagicDefLevel() {
	...
	if (m_bType == OT_PC) {
		CTPlayer* pPlayer = (CTPlayer*) this;
		if (pPlayer) {
			dwDL += (DWORD) pPlayer->CalcPetBonus(MTYPE_MDL);
			if (pPlayer->m_dwGuildID && pPlayer->m_StatLevel)
				dwDL += CALCULATE_ARMOR(pPlayer->m_StatLevel);
		}
	}
...
BYTE CTObjBase::GetCriticalPysProb() {
	...
	if (m_bType == OT_PC) {
		CTPlayer* pPlayer = (CTPlayer*) this;
		if (pPlayer)
			dwCR += (DWORD) pPlayer->CalcPetBonus(MTYPE_CR);
	}
...
BYTE CTObjBase::GetCriticalMagicProb() {
	...
	if (m_bType == OT_PC) {
		CTPlayer* pPlayer = (CTPlayer*) this;
		if (pPlayer)
			dwCR += (BYTE) pPlayer->CalcPetBonus(MTYPE_MCR);
	}
...
BYTE CTObjBase::GetChargeProb() {
	...
	if (m_bType == OT_PC) {
		CTPlayer* pPlayer = (CTPlayer*) this;
		if (pPlayer)
			dwCP += (BYTE) pPlayer->CalcPetBonus(MTYPE_CMP);
	}
...
BYTE CTObjBase::GetAtkHitType(CTSkillTemp* pSkill,
															...
															BYTE bAttackerLevel,
															BYTE bEquipSpecial) {
	if (bCR == 0xFF || m_bMode == MT_GOHOME)
		return HT_MISS;

	if (_AtlModule.FindPremiumSkill(pSkill->m_wID))
		return HT_NORMAL;

	if (pSkill->m_bKind == 21)
		return HT_NORMAL;

	if (m_bIsSpecial && !bEquipSpecial)
		return HT_MISS;

	auto GSkill = _AtlModule.m_mapGuildSkill.find(pSkill->m_wID);
	if (GSkill != _AtlModule.m_mapGuildSkill.end())
		return HT_NORMAL;
	DWORD dwAtk = 0;
	BYTE bRand = 0;
	...
	if (pSkill->m_bHitTest == 1) {
		bRand = rand() % 100;
		BYTE bChance = pSkill->m_bHitInit + ((bSkillLevel - 1) * pSkill->m_bHitInc);
		...
		if (m_bType == OT_PC) {
			...
			return (BYTE(rand() % 100) < bCR) ? HT_CRITICAL : HT_NORMAL;
		}
...
void CTObjBase::OnDie(DWORD dwAttackID, BYTE bObjectType, WORD wTempMonID) {
	...
	if (m_bType == OT_PC) {
		...
		if (pPlayer) {
			...
			if (pMainPlayer) {
				if (pMainPlayer->m_bLife == 0) {
					pMainPlayer->m_bDead = TRUE;
					_AtlModule.OnBRPlayerDie(pMainPlayer);
				} else
					pMainPlayer->m_bLife--;

				MAPBRTEAM::iterator finder = _AtlModule.m_mapBRTEAM.find(pPlayer->m_dwTeamID);
				if (finder != _AtlModule.m_mapBRTEAM.end()) {
					for (MAPBRPLAYER::iterator itPlayer = (*finder).second->m_mapBRPLAYER.begin(); itPlayer != (*finder).second->m_mapBRPLAYER.end(); ++itPlayer) {
						...
						if (pMate)
							pMate->SendCS_UPDATEBRMATE_ACK((*finder).second->m_mapBRPLAYER);
					}

					for (MAPBRTEAM::iterator it = _AtlModule.m_mapBRTEAM.begin(); it != _AtlModule.m_mapBRTEAM.end(); ++it) {
						for (MAPBRPLAYER::iterator itPlay = (*it).second->m_mapBRPLAYER.begin(); itPlay != (*it).second->m_mapBRPLAYER.end(); ++itPlay) {
							CTPlayer* pBRPlayer = _AtlModule.FindPlayer((*itPlay).second->m_dwCharID, (*itPlay).second->m_dwKEY);
							if (pBRPlayer)
								pBRPlayer->SendCS_UPDATEBRTPLAYER_ACK(pPlayer->m_dwTeamID, pPlayer->m_dwID, pMainPlayer->m_wKills, pMainPlayer->m_bLife);
						}
					}
				}
			}
			...
			if (dwAttackID != m_dwID) {
				LPBRPLAYER pAttack = _AtlModule.GetBRPlayer(dwAttackID);
				if (pAttack) {
					pAttack->m_wKills++;
					...
					if (pAttackTeam) {
						for (MAPBRPLAYER::iterator itPlayer = pAttackTeam->m_mapBRPLAYER.begin(); itPlayer != pAttackTeam->m_mapBRPLAYER.end(); ++itPlayer) {
							CTPlayer* pMate = _AtlModule.FindPlayer((*itPlayer).second->m_dwCharID, (*itPlayer).second->m_dwKEY);
							if (pMate)
								pMate->SendCS_UPDATEBRMATE_ACK(pAttackTeam->m_mapBRPLAYER);
						}
						...
						for (MAPBRTEAM::iterator it = _AtlModule.m_mapBRTEAM.begin(); it != _AtlModule.m_mapBRTEAM.end(); ++it) {
							for (MAPBRPLAYER::iterator itPlay = (*it).second->m_mapBRPLAYER.begin(); itPlay != (*it).second->m_mapBRPLAYER.end(); ++itPlay) {
								CTPlayer* pBRPlayer = _AtlModule.FindPlayer((*itPlay).second->m_dwCharID, (*itPlay).second->m_dwKEY);
								if (pBRPlayer)
									pBRPlayer->SendCS_UPDATEBRTPLAYER_ACK(pAttackTeam->m_dwTeamID, dwAttackID, pAttack->m_wKills, pAttack->m_bLife);
							}
						}
					}
				}
...
BYTE CTObjBase::UseSkillItem(CTSkill* pSkill, BYTE bAddtional) {
	...
	if (pSkill->m_pTSKILL->m_dwWeapon == 96) {
		...
		for (MAPTITEM::iterator itEquip = pTEquip->m_mapTITEM.begin(); itEquip != pTEquip->m_mapTITEM.end(); ++itEquip) {
			if ((*itEquip).second->m_pTITEM->m_bUseItemKind == 24) {
				bUseArrows = TRUE;
				break;
			}
		}
		...
		for (itInven = m_mapTINVEN.begin(); itInven != m_mapTINVEN.end(); ++itInven) {
			...
			for (MAPTITEM::iterator itItem = pInven->m_mapTITEM.begin(); itItem != pInven->m_mapTITEM.end(); ++itItem) {
				if (((*itItem).second->m_wItemID == CASH_BOLT && !bUseArrows) ||
						((*itItem).second->m_wItemID == CASH_ARROW && bUseArrows)) {
					return TRUE;
				}
			}
		}
	}
	if (pSkill->m_pTSKILL->m_wUseItem == THROWING_AXE_ITEMID) {
		for (MAPTINVEN::iterator itInven = m_mapTINVEN.begin(); itInven != m_mapTINVEN.end(); ++itInven) {
			...
			for (MAPTITEM::iterator itItem = pInven->m_mapTITEM.begin(); itItem != pInven->m_mapTITEM.end(); ++itItem) {
				if ((*itItem).second->m_pTITEM->m_wItemID == CASH_AXE)
					return TRUE;
			}
		}
	}
					...
					for (itInv = m_mapTINVEN.begin(); itInv != m_mapTINVEN.end(); itInv++) {
						...
						if (bCount >= bNeedCount) {
							...
							for (itUI = mapUItem.begin(); itUI != mapUItem.end(); itUI++) {
								...
								//	if((*itUI).second->m_pTITEM->m_wItemID == 25020 || (*itUI).second->m_pTITEM->m_wItemID == 25021 || (*itUI).second->m_pTITEM->m_wItemID == 25022)
									//	break;
								//	else
								_AtlModule.UseItem(
									pPlayer,
	...
	for (DWORD i = 0; i < pTemp->m_vData.size(); i++) {
		...
		case SDT_RECALL:
		{
			if (m_bType == OT_MON ||
					...
					(m_bType == OT_PC &&
					 dwHostID == dwAttack &&
					 pData->m_bExec == SER_MONSTER)) {
				_AtlModule.SendMW_MONTEMPTEVO_ACK(dwAttack, m_dwID, m_bType);
				return PERFORM_FAIL;
			}
			...
			BYTE bCheckRecall = TRUE;
			int nEvocateLoop = EVOCATE_NORMAL;
			...
			FLOAT fDeg[3] = { 0.0f, 90.0f, 180.0f };
			FLOAT fRadius = 2.0f;

			for (int i = 0; i < nEvocateLoop; i++) {
				MAPTMONSTERTEMP::iterator itMon = _AtlModule.m_mapTMONSTER.find(wRecMon);
				if (itMon != _AtlModule.m_mapTMONSTER.end()) {
					DWORD dwATTR = MAKELONG((*itMon).second->m_wSummonAttr, min(bAttackerLevel, 140));
					...
					if (!bCheckRecall) {
						CTObjBase* pATTACK = pMain->m_pMAP->FindNeighbor(bAtkerType, dwAttack, m_fPosX, m_fPosZ);
						if (pATTACK) {
							fPosX = pATTACK->m_fPosX + fRadius * cosf(fDeg[i]);
							fPosY = pATTACK->m_fPosY;
							fPosZ = pATTACK->m_fPosZ + fRadius * sinf(fDeg[i]);
						} else {
							fPosX = m_fPosX + fRadius * cosf(fDeg[i]);
							fPosY = m_fPosY;
							fPosZ = m_fPosZ + fRadius * sinf(fDeg[i]);
						}
					} else if (pTemp->m_bTargetRange == TSKILLRANGE_POINT) {
						fPosX = fAtkPosX;
						fPosY = fAtkPosY;
						fPosZ = fAtkPosZ;
					} else {
		...
		case SDT_STATUS:
			...
			case SDT_STATUS_RETURN:
			{
				if (m_bType == OT_PC) {
					...
					if (pTarget->m_wSpawnID != BOW_DEFAULT_SPAWNID &&
							pTarget->m_wSpawnID != BOW_DEFAULT_SPAWNID + TCONTRY_C) {
						return PERFORM_MISS;
					}
				#else
					if (pTarget->m_wSpawnID == BOW_DEFAULT_SPAWNID ||
			...
			case SDT_STATUS_WARP:
			{
				...
				if (m_bType == OT_PC) {
					...
					if (((CTMonster*) this)->m_pMON->m_wID >= 30756 &&
							((CTMonster*) this)->m_pMON->m_wID <= 30768) {
						return PERFORM_FAIL;
					}
				}
		...
		case SDT_AI:
		{
			...
			break;
			case SDT_STATUS_MAXHP_PERCENT_DECREASE:
			{
				if (!m_dwHP || !GetMaxHP())
					return PERFORM_FAIL;

				FLOAT NewHP = GetMaxHP() * ((pData->m_wValue + (pData->m_wValueInc * bSkillLevel)) / 100.0f);
				m_dwHP -= (DWORD) NewHP;
			}
			break;
			case SDT_AGGROTRANS:
...
void CTObjBase::CancelBlockBuff() {
	for (DWORD i = 0; i < m_vMaintainSkill.size();) {
		if (m_vMaintainSkill[i]->m_pTSKILL->IsBlockType())
...
BYTE CTObjBase::IsInSkyGarden() {
	if (!m_pMAP || !m_pMAP->m_pSkyGarden)
		return FALSE;
