...
string mapName() {
	std::string hitler;
	cout << "MAP SERVICE NAME(DEFAULT - " << "TMap" << "): ";
...
CTMapSvrModule::CTMapSvrModule()
	...
	: TServerSystem("TBr", TMAP_VERSION)
#elif defined(BOW_COMPILE_MODE)
	: TServerSystem("TBoW", TMAP_VERSION)
#else
	: TServerSystem(mapName(), TMAP_VERSION)
#endif
{
	...
	m_bGroupID = 0;
	m_bRelayOn = FALSE;
	m_bHasAlreadyCTRLServer = FALSE;

	m_pDebugSocket = new CDebugSocket(m_szServiceName);
	...
	m_mapBOWBPITEM.clear();

	for (BYTE i = 0; i < TCLASS_COUNT; ++i)
		m_vBOWITEM[i].clear();

	m_mapBOWPLAYER.clear();
#endif
...
#ifdef CRASH_DEBUG_LOG
	m_setChars.insert(13095);
	m_setChars.insert(6429);
	m_setChars.insert(13680);

	m_pDebugLog = new CLog(_T("MapSvr"));

	if (!m_pDebugLog->Open())
		LogEvent(_T("Could not open debug log!"));
	else
		m_pDebugLog->WriteMessage(_T("Let's go! :)"));
#endif
	...
	m_dwToggleableSkills = 0;
	static WORD wSkillIDs[] =
	{
		221,
		536,
		526,
	...
	BYTE bSkillSize = sizeof(wSkillIDs) / sizeof(WORD);
	m_dwToggleableSkills = new DWORD[bSkillSize];

	for (BYTE i = 0; i < bSkillSize; ++i)
		m_dwToggleableSkills[i] = wSkillIDs[i];


#if defined(BR_COMPILE_MODE)
	m_bRemainingPlayers = 0;
	m_bBRStatus = BS_PEACE;
	m_wBRMapID = 0;
	m_Mode = BR_3V3;
	m_mapLeftPlayer.clear();
#elif defined(BOW_COMPILE_MODE)
	m_bBOWStatus = BS_PEACE;
#endif
...
void CTMapSvrModule::InitEnvironment() {
	...
	ClearBRPlayers();
#elif defined(BOW_COMPILE_MODE)
	DEFINE_QUERY(&m_db, CSPClearBOWPlayers)
		query->Call();
	UNDEFINE_QUERY()
	#endif
...
#ifdef SKYGARDEN
	for (DWORD mk = 0; mk < m_vSkygarden.size(); mk++) {
		LPTLOCAL pSkygarden = m_vSkygarden[mk];
...
DWORD CTMapSvrModule::LoadConfig() {
	...
	LoadIntFromIni("WorldPort", 3816, m_wWorldPort);
	...
	//#endif
		...
		globalvar.Format(valdefinition, m_szGamePasswd, m_szDBUserID, m_szWorldIP);
...
BYTE CTMapSvrModule::Accept() {
	...
	if (pPlayer->m_addr.sin_addr.s_addr == m_addrCtrl.sin_addr.s_addr) {
		if (!m_bHasAlreadyCTRLServer) {
			m_bHasAlreadyCTRLServer = TRUE;
			pPlayer->m_bSessionType = SESSION_SERVER;
		} else {
			LogSmartly("Trying to crash server where dev is Jirkus? Good luck then.");
			return FALSE;
		}
	} else {
		pPlayer->m_bUseCrypt = TRUE;
		pPlayer->m_bCheckedSession = FALSE;
...
void CTMapSvrModule::LogSmartly(CString strLogMessage, BOOL IsHackLog) {
	TCHAR path[MAX_PATH] = "";
	CString newpath;
	GetModuleFileName(NULL, path, MAX_PATH);
	PathRemoveFileSpec(path);
	newpath.Format("%s\\%s", path, IsHackLog ? "HackLog.log" : "MapLog.log");

	FILE* pDebug = fopen(newpath, "a+");
	CTime tTime(m_timeCurrent);
	if (!pDebug)
		pDebug = fopen(newpath, "w+");

	if (!pDebug)
		LogEvent(newpath);

	CString strWhoreNewMessage;
	strWhoreNewMessage.Format("MAP : %d %s - %s\n", m_bServerID, tTime.Format("%c"), strLogMessage);
	fprintf(pDebug, strWhoreNewMessage);
	fclose(pDebug);
}
...
void CTMapSvrModule::UpdateData() {
	...
	MAPTTNMTINVEN::iterator itTNMTINVEN;
	MAPTITEM::iterator itTNMTITEM;
	...
	for (itTNMTINVEN = m_mapTTNMTINVEN.begin(); itTNMTINVEN != m_mapTTNMTINVEN.end(); itTNMTINVEN++)

		delete (*itTNMTINVEN).second;
	m_mapTTNMTINVEN.clear();
...
#ifdef BR_COMPILE_MODE
	MAPBRSPAWNPOS::iterator itPos;
	for (itPos = m_mapBRSPAWNPOS.begin(); itPos != m_mapBRSPAWNPOS.end(); ++itPos)
		delete (*itPos).second;
	m_mapBRSPAWNPOS.clear();

	while (!m_vBRSUPPLIES.empty()) {
		delete m_vBRSUPPLIES.back();
		m_vBRSUPPLIES.pop_back();
	}
#endif
	...
	for (BYTE i = 0; i < TCLASS_COUNT; i++) {
		while (!m_vTTNMTITEM[i].empty()) {
			delete m_vTTNMTITEM[i].back()->m_pItem;
			delete m_vTTNMTITEM[i].back();
			m_vTTNMTITEM[i].pop_back();
		}
...
#ifdef BOW_COMPILE_MODE
	for (BYTE i = 0; i < TCLASS_COUNT; i++) {
		while (!m_vBOWITEM[i].empty()) {
			delete m_vBOWITEM[i].back();
			m_vBOWITEM[i].pop_back();
		}

		m_vBOWITEM[i].clear();
	}

	for (MAPBOWBONUSITEM::iterator itBPItem = m_mapBOWBPITEM.begin(); itBPItem != m_mapBOWBPITEM.end(); ++itBPItem)
		delete (*itBPItem).second;
	m_mapBOWBPITEM.clear();
...
#ifdef BR_COMPILE_MODE
	for (MAPBRTEAM::iterator itTeam = m_mapBRTEAM.begin(); itTeam != m_mapBRTEAM.end(); ++itTeam) {
		for (MAPBRPLAYER::iterator itPlayer = (*itTeam).second->m_mapBRPLAYER.begin(); itPlayer != (*itTeam).second->m_mapBRPLAYER.end(); ++itPlayer)
			delete (*itPlayer).second;
...
void CTMapSvrModule::SetEventCloseSession(CTMapSession* pSession, BYTE bSave) {
	if (pSession->m_bSessionType == SESSION_CLIENT) {
		...
		if (pPlayer->m_dwID) {
			...
			/*	MAPTSECURE::iterator finderS = m_mapTSECURE.find(pPlayer->m_dwUserID);
				if (finderS != m_mapTSECURE.end())
				{
					m_mapTSECURE.erase(finderS);
					delete (*finderS).second;
				}*/
			...
			for (BYTE i = 0; i < ARENA_COUNT; ++i) {
				for (BYTE a = 0; a < m_ArenaQueue[i].size(); a++) {
					for (const auto& Player : m_ArenaQueue[i][a]) {
						if (Player.second->m_dwID == pPlayer->m_dwID) {
							m_ArenaQueue[i].erase(m_ArenaQueue[i].begin() + a);
							break;
						}
					}
				}
			...
			pPlayer->ClearSpolecnikMon(FALSE);
...
void CTMapSvrModule::CloseSession(CTMapSession* pSession) {
	if (pSession->m_bSessionType == SESSION_SERVER &&
			pSession->m_addr.sin_addr.s_addr == m_addrCtrl.sin_addr.s_addr) {
		LogSmartly("Deleted Session CL");
		m_bHasAlreadyCTRLServer = FALSE;
	}
...
DWORD CTMapSvrModule::LoadData() {
	...
	UNDEFINE_QUERY()
		...
		if (query->Open()) {
			while (query->Fetch()) {
				m_gemgrade[query->m_bGem].m_bGem = query->m_bGem;
				m_gemgrade[query->m_bGem].m_bProb = query->m_bProb;
			}
			query->Close();
		}
	UNDEFINE_QUERY()
	...
	UNDEFINE_QUERY()
		...
		if (query->Open()) {
			while (query->Fetch()) {
				LPTTITLETEMP pTITLE = new TTITLETEMP();

				pTITLE->m_wTitleID = query->m_wTitleID;
				pTITLE->m_wCategory = query->m_wCategory;
				pTITLE->m_bKind = query->m_bKind;
				pTITLE->m_dwRequirement = query->m_dwRequirement;

				m_mapTTITLE.insert(MAPTTITLETEMP::value_type(pTITLE->m_wTitleID, pTITLE));
			}
	...
	if (query->Open()) {
		while (query->Fetch()) {
			...
			pSkill->m_bRepeatCount = query->m_bRepeatCount;
	...
	UNDEFINE_QUERY()
...

		{
			...
			while (query->Fetch()) {
				...
				pPET->m_bID = query->m_wID;



				pPET->m_wNMonID = query->m_wDefMonID;
				pPET->m_wUPMonID = query->m_wUpgMonID;





				m_mapTPET.insert(MAPTPETTEMP::value_type(pPET->m_bID, pPET));
			}
			query->Close();
		}
	UNDEFINE_QUERY()
	...
	UNDEFINE_QUERY()
		...
		if (query->Open()) {
			while (query->Fetch()) {
				...
				pZone->m_wValorianBossDefend = query->m_wValorianBossDefend;
				pZone->m_wValorianBossAttack = query->m_wValorianBossAttack;
				pZone->m_wDerionBossDefned = query->m_wDerionBossDefned;
				pZone->m_wDerionBossAttack = query->m_wDerionBossAttack;
				pZone->m_wMiddleSpawnID = query->m_wMiddleSpawnID;
				pZone->m_wRightSpawnID = query->m_wRightSpawnID;
				pZone->m_wLeftSpawnID = query->m_wLeftSpawnID;
	...
	DEFINE_QUERY(&m_db, CTBLMissionTable)
		if (query->Open()) {
			while (query->Fetch()) {
				...
				pLocal->m_mapKills.clear();

				pLocal->m_bMiddleOwner = NULL;
				pLocal->m_bLeftOwner = NULL;
				pLocal->m_bRightOwner = NULL;
	...
	UNDEFINE_QUERY()
		...
		if (query->Open()) {
			while (query->Fetch()) {
				MAPTBATTLEZONE::iterator itBz = m_mapTBATTLEZONE.find(query->m_wID);
				if (itBz == m_mapTBATTLEZONE.end())
					continue;

				LPTLOCAL pLocal = new TLOCAL();
				pLocal->m_bType = LOCAL_SKYGARDEN;
				pLocal->m_wLocal = query->m_wID;
				pLocal->m_bStatus = BS_NORMAL;
				pLocal->m_bCountry = query->m_bCountry;
				pLocal->m_dwGuild = 0;
				pLocal->m_wDPoint = 0;
				...
				pLocal->m_bMiddleOwner = CAMP_DEFEND;
				pLocal->m_bLeftOwner = CAMP_DEFEND;
				pLocal->m_bRightOwner = CAMP_DEFEND;
				...
				pLocal->m_timeNextDefend = __DBTOTIME(query->m_dateWarTime);
				pLocal->m_strHero = NAME_NULL;
				pLocal->m_timeHero = NULL;

				m_vSkygarden.push_back(pLocal);
				m_mapTLOCAL.insert(MAPTLOCAL::value_type(pLocal->m_wLocal, pLocal));
			}
			query->Close();
		}
	...
	DEFINE_QUERY(&m_db, CTBLArena)
		if (query->Open()) {
			while (query->Fetch()) {
				...
				pArena->m_MaxWinCount = 0;
				pArena->m_TopPlayer.clear();
	...
	UNDEFINE_QUERY()
		...
		if (query->Open()) {
			while (query->Fetch()) {
				LPTCOMPBONUS pBONUS = new TCOMPBONUS();

				pBONUS->m_bBonusID = query->m_bBonusID;
				pBONUS->m_fBase = query->m_fBase;
	...
	UNDEFINE_QUERY()
		...
		DEFINE_QUERY(&m_db, CTBLInvenTournametChart)
		if (query->Open()) {
			while (query->Fetch()) {
				LPTTNMTINVEN pTINVEN = new TTNMTINVEN();

				pTINVEN->m_bInven = query->m_bInvenID;
				pTINVEN->m_wItemID = query->m_wItemID;
				pTINVEN->m_bELD = query->m_bELD;
				pTINVEN->m_dEndTime = NULL;

				m_mapTTNMTINVEN.insert(MAPTTNMTINVEN::value_type(query->m_bInvenID, pTINVEN));
			}
	...
	UNDEFINE_QUERY()
		...
		if (query->Open()) {
			while (query->Fetch()) {
				LPTTNMTITEM pTITEM = new TTNMTITEM();
				pTITEM->m_pItem = CreateItem(query->m_item);

				if (!pTITEM->m_pItem)
					continue;

				pTITEM->m_bInvenID = query->m_bInvenID;
				pTITEM->m_pItem->m_bItemID = query->m_item.m_bItemID;
				pTITEM->m_pItem->m_dEndTime = NULL;
				pTITEM->m_pItem->m_dwDuraCur = query->m_item.m_dwDuraMax;
				pTITEM->m_pItem->m_bRefineCur = NULL;

				m_vTTNMTITEM[query->m_bClass].push_back(pTITEM);
			}
	...
	DEFINE_QUERY(&m_db, CTBLPremiumSkillChart)
		if (query->Open()) {
			while (query->Fetch()) {
			#ifdef BR_COMPILE_MODE
				if (query->m_wSkillID == 1507)
					continue;
			#endif
				LPPREMIUMSKILL pSKILL = new PREMIUMSKILL();
				pSKILL->m_wSkillID = query->m_wSkillID;
				pSKILL->m_wMedals = query->m_wMedals;
				pSKILL->m_bType = query->m_bType;

			#ifdef BOW_COMPILE_MODE
				m_vPREMIUMSKILL[query->m_bType].push_back(pSKILL);
			#else
				//if (query->m_bType == PREMIUMSKILL_BOW)
				//{
				MAPTSKILLTEMP::iterator itSkill = m_mapTSKILL.find(query->m_wSkillID);
		...
		#ifdef BR_COMPILE_MODE
			LPPREMIUMSKILL pSKILL = new PREMIUMSKILL();
			pSKILL->m_wSkillID = 1599;
			pSKILL->m_wMedals = 0;
			pSKILL->m_bType = PREMIUMSKILL_BOW;

			m_vPREMIUMSKILL[PREMIUMSKILL_BOW].push_back(pSKILL);
		#endif
			query->Close();
		}
	UNDEFINE_QUERY();
	...
	if (query->Open()) {
		while (query->Fetch()) {
			LPBATTLERANK pRANK = new BATTLERANK();
			pRANK->m_bRank = query->m_bRank;
			pRANK->m_dwPoints = query->m_dwPoints;

			m_vBATTLERANK.push_back(pRANK);
		}
	...
	if (query->Open()) {
		while (query->Fetch()) {
			LPSPECIALBOX pBOX = new SPECIALBOX();
			pBOX->m_bGroup = (BYTE) query->m_wID;
			pBOX->m_wID = query->m_wGroup;
			pBOX->m_pITEM = CreateItem(query->m_item);
			pBOX->m_wUseTime = query->m_wUseTime;
			pBOX->m_bClass = query->m_bClass;
			if (!pBOX->m_pITEM) {
				delete pBOX;
				continue;
			}

			MAPSPECIALBOX::iterator it = m_mapSpecialBox.find(pBOX->m_wID);
			if (it == m_mapSpecialBox.end()) {
				VSBITEM vItems;
				vItems.push_back(pBOX);
				m_mapSpecialBox.insert(MAPSPECIALBOX::value_type(pBOX->m_wID, vItems));
			} else
	...
	if (query->Open()) {
		while (query->Fetch()) {
			...
			m_mapGuildSkill.insert(std::make_pair(query->m_wSkillID, GuildSkill));
		}
	...
	if (query->Open()) {
		while (query->Fetch()) {
			...
			m_vAccessoryMagic.push_back(Magic);
		}
...
void CTMapSvrModule::LoadBOWData() {
	DEFINE_QUERY(&m_db, CTBLBOWItemChart)
		if (query->Open()) {
			while (query->Fetch()) {
				LPBOWITEM pTITEM = new BOWITEM();
				pTITEM->m_bInvenID = query->m_bInvenID;
				pTITEM->m_wMinItemID = query->m_wMinItemID;
				pTITEM->m_wMaxItemID = query->m_wMaxItemID;
				pTITEM->m_wDependOnItemID = query->m_wDependOnItemID;
				pTITEM->m_bCount = query->m_bCount;

				m_vBOWITEM[query->m_bClass].push_back(pTITEM);
			}
	...
	DEFINE_QUERY(&m_db, CTBLBOWBonusItems)
		if (query->Open()) {
			while (query->Fetch()) {
				LPBOWBONUSITEM pBPITEM = new BOWBONUSITEM();
				pBPITEM->m_bItemID = query->m_bItemID;
				pBPITEM->m_wItemID = query->m_wItemID;
				pBPITEM->m_wPrice = query->m_wPrice;

				m_mapBOWBPITEM.insert(MAPBOWBONUSITEM::value_type(pBPITEM->m_wItemID, pBPITEM));
			}
...
void CTMapSvrModule::LoadBRData() {
	DEFINE_QUERY(&m_db, CTBLBRSpawnPos)
		if (query->Open()) {
			while (query->Fetch()) {
				MAPBRSPAWNPOS::iterator finder = m_mapBRSPAWNPOS.find(query->m_wMapID);
				if (finder == m_mapBRSPAWNPOS.end()) {
					LPBRSPAWNPOS pPOS = new BRSPAWNPOS();
					pPOS->m_wMapID = query->m_wMapID;

					BRRAWPOS pRAWPOS;
					pRAWPOS.m_fPosX = query->m_fPosX;
					pRAWPOS.m_fPosY = query->m_fPosY;
					pRAWPOS.m_fPosZ = query->m_fPosZ;

					pPOS->m_vRAWPOS.push_back(pRAWPOS);
					m_mapBRSPAWNPOS.insert(MAPBRSPAWNPOS::value_type(query->m_wMapID, pPOS));
				} else {
					BRRAWPOS pRAWPOS;
					pRAWPOS.m_fPosX = query->m_fPosX;
					pRAWPOS.m_fPosY = query->m_fPosY;
					...
					(*finder).second->m_vRAWPOS.push_back(pRAWPOS);
				}
			}
	...
	DEFINE_QUERY(&m_db, CTBLBRSuppliesChart)
		if (query->Open()) {
			while (query->Fetch()) {
				LPBRSUPPLIES pSUPPLY = new BRSUPPLIES();
				pSUPPLY->m_wItemID = query->m_wItemID;
				pSUPPLY->m_dwTick = query->m_dwTick;

				m_vBRSUPPLIES.push_back(pSUPPLY);
			}
	...
	UNDEFINE_QUERY()
		/*
			...
			if(query->Open())
			{
				...
				while(query->Fetch())
				{
					LPTCUSTOMTICKS pTicks = new TCUSTOMTICKS();
					pTicks->m_wFirstSkill = query->m_wSkill;
					pTicks->m_wSkill = query->m_wAffectedSkill;
					pTicks->m_dwTick = query->m_dwTick;

					m_mapCUSTOMTICKS.insert(MAPTCUSTOMTICKS::value_type(++bIndex, pTicks));
				}
			}
			UNDEFINE_QUERY()
			*/
...
DWORD CTMapSvrModule::BatchThread() {
	...
	int nCurMinute;
	CTime curtime_start(CTime::GetCurrentTime().GetTime());
	nCurMinute = curtime_start.GetMinute();
...
void CTMapSvrModule::ProcessSession(CTMapSession* pSession, DWORD dwIoBytes) {
	...
	while (bContinue) {
		...
		if (pSession) {
			if (pSession->m_bSessionType == SESSION_CLIENT) {
				CTPlayer* pPlayer = (CTPlayer*) pSession;
				if (pPlayer->m_bLock && wMsgID != CS_CONNECT_REQ && wMsgID != CS_KICKOUT_REQ) {
					LogSmartly("Player locked, can't send any another stuff.");
...
DWORD CTMapSvrModule::OnReceive(LPPACKETBUF pBUF) {
...
#ifdef CRASH_DEBUG_LOG
	if (pBUF->m_pSESSION && pBUF->m_pSESSION->m_bSessionType == SESSION_CLIENT) {
		CTPlayer* pPlayer = (CTPlayer*) pBUF->m_pSESSION;

		if (m_setChars.find(pPlayer->m_dwID) != m_setChars.end()) {
			CString message;

			message.Format(_T("Received packet %hx from \"%s\" (%d) - %s"),
										 pBUF->m_packet.m_pHeader->m_wID, pPlayer->m_strNAME, pPlayer->m_dwID, inet_ntoa(pPlayer->m_addr.sin_addr));

			m_pDebugLog->WriteMessage(message);
			m_pDebugLog->WritePacket(&pBUF->m_packet);
		}
	}
#endif
	...
	switch (wMsgID) {
		if (pBUF->m_pSESSION) {
			if (pBUF->m_pSESSION->m_bSessionType == SESSION_SERVER) {
				...
				ON_RECEIVE(CT_SERVICEMONITOR_ACK)
					...
					ON_RECEIVE(CT_USERMOVE_ACK) // ���·� ���� ��ġ�̵�
			...
			} else if (pBUF->m_pSESSION->m_bSessionType == SESSION_SERVER && pBUF->m_pSESSION->m_addr.sin_port == m_wGamePort) {	// AI request message handler (Execute on AI thread) - Implemented on SSHandler.cpp
					ON_RECEIVE(SM_AICMD_REQ)
						...
						ON_RECEIVE(SM_VALIDMAPSESSION_REQ)
						...
						ON_RECEIVE(DM_CLEARTITLE_REQ)
						ON_RECEIVE(DM_SAVETITLE_REQ)
					...
					#ifdef SKYGARDEN
						ON_RECEIVE(DM_SKYGARDENOCCUPY_REQ)
					#endif
					...
					#ifdef SKYGARDEN
						ON_RECEIVE(DM_SKYGARDENOCCUPY_ACK)
					#endif
				...
				} else if (pBUF->m_pSESSION->m_bSessionType == SESSION_SERVER && pBUF->m_pSESSION->m_addr.sin_port == m_wWorldPort) {
						...
						ON_RECEIVE(MW_MONTHRANKRESETCHAR_REQ)
							...
							ON_RECEIVE(MW_BOWTIMEUPDATE_ACK)
							ON_RECEIVE(MW_ADDTOBOWQUEUE_ACK)
							ON_RECEIVE(MW_CANCELBOWQUEUE_ACK)
							ON_RECEIVE(MW_NOTIFYNONQUEUEDPLAYER_ACK)
							ON_RECEIVE(MW_PREPAREFORBOW_REQ)

							//Apex
							ON_RECEIVE(MW_APEXDATA_REQ)

						#ifdef BOW_COMPILE_MODE
							ON_RECEIVE(MW_BOWCOMMANDEXEC_REQ)
							ON_RECEIVE(MW_ENDBOWWAR_REQ)
							ON_RECEIVE(MW_ADDBOWPLAYERS_ACK)
							ON_RECEIVE(MW_RELEASESINGLEBOWPLAYER_REQ)
						#endif
							ON_RECEIVE(MW_BRTIMEUPDATE_ACK)
							ON_RECEIVE(MW_BRTEAMMATEADD_ACK)
							ON_RECEIVE(MW_UPDATEBRTEAM_ACK)
						#ifdef BR_COMPILE_MODE
							ON_RECEIVE(MW_ADDBRTEAMS_ACK)
							ON_RECEIVE(MW_BRCOMMANDEXEC_REQ)
							ON_RECEIVE(MW_ENDBRWAR_REQ)
							ON_RECEIVE(MW_RELEASESINGLEBRPLAYER_REQ)
						#endif
							ON_RECEIVE(MW_PREPAREFORBR_REQ)
							ON_RECEIVE(MW_ADDTOBRQUEUE_ACK)
							ON_RECEIVE(MW_RECALCCOUNTRYBALANCE_ACK)

							ON_RECEIVE(MW_BATTLEMODESTATUS_ACK)

							ON_RECEIVE(MW_GUILDSKILLACTION_ACK)
							ON_RECEIVE(MW_UPDATEGUILDCOOLDOWN_REQ)

							ON_RECEIVE(MW_ADDCOOLDOWN_REQ)
					}
		} else
			LogSmartly("Packet sent from wrong port!");
		...
		ON_RECEIVE(CS_HACKSHIELD_REQ)
			...
			ON_RECEIVE(CS_PETEFFECTCHANGE_REQ)
			ON_RECEIVE(CS_PETRIDING_REQ)
			ON_RECEIVE(CS_SPOLECNIKRECALL_REQ)
			...
			ON_RECEIVE(CS_CREATESADDLE_REQ)
			ON_RECEIVE(CS_CHANGENAME_REQ)
			ON_RECEIVE(CS_CHANGETITLE_REQ)
			...
			ON_RECEIVE(CS_DELETESADDLE_REQ)
			...
			ON_RECEIVE(CS_REQUESTSADDLE_REQ)
			...
			ON_RECEIVE(CS_GETREWARDS_REQ)

			//ON_RECEIVE(CS_HACKINFO_REQ)
			ON_RECEIVE(CS_WINLDIC_REQ)
			ON_RECEIVE(CS_SOULLOTTERY_REQ)
			ON_RECEIVE(CS_SOULLOTTERY_TRIGGER_REQ)

			//Securitysystem
			ON_RECEIVE(CS_SC_CREATECODE_REQ)
			ON_RECEIVE(CS_SC_CHANGECODE_REQ)
			ON_RECEIVE(CS_SC_DISABLECODE_REQ)
			ON_RECEIVE(CS_SC_ENABLECODE_REQ)
			ON_RECEIVE(CS_SC_UNLOCK_REQ)
			ON_RECEIVE(CS_SC_CHECKCODE_REQ)
			ON_RECEIVE(CS_SC_GETLOCKTIME_REQ)
			//Pet system
			ON_RECEIVE(CS_COMPANIONLUP_REQ)
			ON_RECEIVE(CS_USEPETITEM_REQ)
			ON_RECEIVE(CS_COMPANIONUPGRADE_REQ)
			ON_RECEIVE(CS_COMPANIONRECALL_REQ)
			ON_RECEIVE(CS_COMPANIONCANCEL_REQ)
			ON_RECEIVE(CS_CREATECOMPANION_REQ)
			ON_RECEIVE(CS_DELETECOMPANION_REQ)
			ON_RECEIVE(CS_HIDECOMPANION_REQ)
			ON_RECEIVE(CS_USECOMPANIONITEM_REQ)
			ON_RECEIVE(CS_DELETECOMPITEMS_REQ)
			ON_RECEIVE(CS_CHANGECOMPANIONEFFECT_REQ)
			ON_RECEIVE(CS_USECOMPANIONPOWDER_REQ)
			ON_RECEIVE(CS_USEMEDALITEM_REQ)
			ON_RECEIVE(CS_VERIFYSESSION_ACK)

			ON_RECEIVE(CS_DELSPOLECNIKMON_REQ)
			ON_RECEIVE(CS_CHGMODESPOLECNIKMON_REQ)
			ON_RECEIVE(CS_REGISTERBOW_REQ)
			ON_RECEIVE(CS_CANCELBOWQUEUE_REQ)
			ON_RECEIVE(CS_CASHBOWRESPAWN_REQ)
			ON_RECEIVE(CS_EXCHANGEBOWBP_REQ)
			ON_RECEIVE(CS_BOWRANKING_REQ)

			ON_RECEIVE(CS_PINGMEASUREMENT_REQ)
			ON_RECEIVE(CS_ACTITEMSUSE_REQ)
			ON_RECEIVE(CS_FINISHSKILL_ACK)
			ON_RECEIVE(CS_LEAVEBATTLEFIELD_REQ)

			ON_RECEIVE(CS_REGISTERBR_REQ)
			ON_RECEIVE(CS_BRTEAMMATEADD_REQ)
			ON_RECEIVE(CS_BRTEAMMATEADDRESULT_ACK)
			ON_RECEIVE(CS_BRTEAMMATEDEL_REQ)
			ON_RECEIVE(CS_VOTEFORBRMAP_REQ)
		#ifdef BR_COMPILE_MODE
			ON_RECEIVE(CS_DONATELIFE_REQ)
		#endif
			ON_RECEIVE(CS_USECOMPRESET_REQ)
			ON_RECEIVE(CS_FINISHCOMPANIONTRANSFER_ACK)
			ON_RECEIVE(CS_CMTELEPORTBATTLEMODE_REQ)
			ON_RECEIVE(CS_CUSTOMCLOAK_LIST_REQ)
			ON_RECEIVE(CS_CUSTOMCLOAK_CHANGE_REQ)
			ON_RECEIVE(CS_QUESTPOSEXEC_REQ)
			ON_RECEIVE(CS_GUILDSKILLACTION_REQ)
			ON_RECEIVE(CS_ARENAINFO_REQ)
			ON_RECEIVE(CS_ARENARANKING_REQ)
	}
...
void CTMapSvrModule::OnTimer(DWORD dwTick) {
	...
	for (itMRT = m_mapMeetingRoomTime.begin(); itMRT != m_mapMeetingRoomTime.end();) {
		if (m_dwTick - itMRT->second > 1000 * HOUR_ONE) {
			...
			itMRT = m_mapMeetingRoomTime.erase(itMRT);
			break;
		} else
			++itMRT;
	}
...
CTObjBase* CTMapSvrModule::FindTarget(CTPlayer* pHost,
	...
	case OT_COMPANION:
	{
		MAPTCOMPANION::iterator itComp = pHost->m_mapCompanion.find(dwTargetID);
		if (itComp != pHost->m_mapCompanion.end() &&
				(*itComp).second->m_bMain)
			pTARGET = (*itComp).second;
	}
	break;
	case OT_SELF:
...
void CTMapSvrModule::CheckQuest(CTPlayer* pPlayer,
																...
																BYTE bTriggerType,
																BYTE bCount) {
	pPlayer->CheckQuest(m_dwTick, dwTermID, bTermType, dwTargetID, bCount);
...
void CTMapSvrModule::SetItemAttr(CTItem* pItem, BYTE bLevel) {
	MAPTITEMATTR::iterator it = m_mapTItemAttr.find(pItem->m_pTITEM->m_wAttrID + m_itemgrade[bLevel].m_bGrade + pItem->m_bGem);
...
BYTE CTMapSvrModule::Teleport(CTPlayer* pPlayer,
	...
	/*
		...
		if( pPlayer->m_dwRiding )
		{
			...
			for( itRecall = pPlayer->m_mapRecallMon.begin(); itRecall != pPlayer->m_mapRecallMon.end(); ++itRecall )
			{
				if( (*itRecall).second->m_bType == OT_RECALL &&
					(*itRecall).second->m_wPetID )
				{
					pPlayer->DeleteRecallMon( (*itRecall).first );
				}
			}
		}
	*/
	pPlayer->PetRiding(0);
	pPlayer->EraseMaintainSkill(526);
	pPlayer->EraseMaintainSkill(527);
	pPlayer->EraseMaintainSkill(331);



#if defined(BR_COMPILE_MODE)
	if (bChannel != BR_SERVER_ID)
		return FALSE;
...
void CTMapSvrModule::SafeTeleport(CTPlayer* pPlayer,
																	BYTE bChannel,
																	WORD wMapID,
																	FLOAT fPosX,
																	FLOAT fPosY,
	...
	if (bChannel == 0) {
		DEFINE_QUERY(&m_db, CSPGetPosition);
		{
			query->m_dwCharID = pPlayer->m_dwID;

			query->Call();

			bChannel = DEFAULT_CHANNEL;
			wMapID = query->m_wMapID;
			fPosX = query->m_fPosX;
			fPosY = query->m_fPosY;
			fPosZ = query->m_fPosZ;
		}
...
void CTMapSvrModule::ExitMAP(CTPlayer* pPlayer, BYTE bTeleport, WORD wGoMapID) {
...
#ifdef SKYGARDEN
	if (pPlayer->IsInSkyGarden())
		pPlayer->SendCS_LEAVESKYGARDEN_ACK();
#endif
	...
	if (IsTournamentMap(pPlayer->m_wMapID)) {
		...
		if (pPlayer->IsTournamentPlayer())
			pPlayer->ExitTournamentCharClear();
	}
...
CTRecallMon* CTMapSvrModule::CreateRecallMon(CTPlayer* pPlayer,
	...
	if (wCompanionID) {
		pMon = new CTCompanion();
		((CTCompanion*) pMon)->m_bType = OT_COMPANION;
		((CTCompanion*) pMon)->m_bRecallType = TRECALLTYPE_NONE;
		pMon->m_bRecallType = TRECALLTYPE_NONE;

		CTCompanion* pComp = static_cast<CTCompanion*>(pMon);
		pPlayer->m_mapCompanion.insert(MAPTCOMPANION::value_type(dwID, pComp));
	} else if (pTemp->m_bIsSelf) {
		pMon = new CTSelfObj();
		dwID = LockSelfMonID();
	...
	if (bRecallAuto) {
		...
		pMon->m_dwTargetID = dwTargetID;
		pMon->m_bTargetType = bTargetType;
	} else
		pMon->m_bRecallType = pTemp->m_bRecallType;
...
#ifdef SKYGARDEN
LPTLOCAL CTMapSvrModule::FindSkygarden(WORD wLocalID) {
	MAPTLOCAL::iterator it = m_mapTLOCAL.find(wLocalID);
	if (it != m_mapTLOCAL.end() &&
			(*it).second->m_bType == LOCAL_SKYGARDEN)
		return (*it).second;
	else
		return NULL;
}
...
void CTMapSvrModule::InitMap(CTPlayer* pPlayer) {
	...
	if (IsCastleMap(pTMAP)) {
...
#ifdef SKYGARDEN
	else if (IsSkyGarden(pTMAP)) {
		LPTLOCAL pSkyGarden = pTMAP->m_pSkyGarden;
		if (!pSkyGarden)
			return;

		if (pTMAP->m_pSkyGarden->m_bCountry == pPlayer->m_bCountry || pTMAP->m_pSkyGarden->m_bCountry == pPlayer->m_bAidCountry)
			pPlayer->m_bCamp = CAMP_DEFEND;
		else
			pPlayer->m_bCamp = CAMP_ATTACK;

		pPlayer->m_wCastle = pSkyGarden->m_wLocal;
		BYTE bDefendCountry = pSkyGarden->m_bCountry;
		BYTE bAttackCountry = (bDefendCountry == TCONTRY_C) ? TCONTRY_D : TCONTRY_C;

		pPlayer->SendCS_ENTERSKYGARDEN_ACK(pTMAP->m_pSkyGarden->m_pZone->m_wID, pPlayer->m_bCamp, bDefendCountry,
																			 pSkyGarden->m_bLeftOwner, pSkyGarden->m_bMiddleOwner,
																			 pSkyGarden->m_bRightOwner, bAttackCountry);
	}
#endif
		...
		if (pRecall && pRecall->m_bRecallType == TRECALLTYPE_PET && !pPlayer->m_dwRiding && pRecall->m_wPetID) {
			SendMW_RECALLMONDEL_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, pRecall->m_dwID, TRUE);
		}

		pPlayer->SendCS_TOURNAMENTENTER_ACK(pTMAP);

		if (FindTournamentPlayer(pPlayer->m_dwID, TRUE))
			SetTournamentChar(pPlayer);
	}
	...
	for (itTMON = pPlayer->m_mapRecallMon.begin(); itTMON != pPlayer->m_mapRecallMon.end(); itTMON++) {
		...
		if ((*itTMON).second->m_bRecallType != TRECALLTYPE_PET || (pPlayer->m_dwRiding && (*itTMON).second->m_wPetID))
			pTMAP->EnterMAP((*itTMON).second);
	}

#if defined(BR_COMPILE_MODE)
	LPBRTEAM pTEAM = GetBRTeam(pPlayer->m_dwID);
	if (pTEAM)
		pPlayer->SendCS_UPDATEBONUSPOINT_ACK(pPlayer->m_dwBB);
#elif defined(BOW_COMPILE_MODE)
	MAPBOWPLAYER::iterator finderPlayer = m_mapBOWPLAYER.find(pPlayer->m_dwID);
	if (finderPlayer != m_mapBOWPLAYER.end()) {
		pPlayer->SendCS_UPDATEBONUSPOINT_ACK(pPlayer->m_dwBB);
		pPlayer->m_bAidCountry = TCONTRY_N;
		if (pPlayer->m_bCountry != (*finderPlayer).second->m_bCountry) {
			pPlayer->m_bCountry = (*finderPlayer).second->m_bCountry;
			pPlayer->SendCS_CHANGECHARBASE_ACK(IU_SUCCESS, pPlayer->m_dwID, IK_COUNTRY, pPlayer->m_bCountry, pPlayer->m_strNAME, 0);
		}
	}
#endif
	...
	MAPTCOMP::iterator jirker = pPlayer->m_mapComp.find(pPlayer->m_bCompanionSlot);
	if (jirker != pPlayer->m_mapComp.end()) {
		MAPTMONSTERTEMP::iterator itMon = _AtlModule.m_mapTMONSTER.find((WORD) (*jirker).second->m_dwMonID);
		if (itMon != _AtlModule.m_mapTMONSTER.end()) {
			_AtlModule.SendMW_CREATESPOLECNIKMON_ACK(
				pPlayer->m_dwID,
				pPlayer->m_dwKEY,
				0,
				(*jirker).second->m_dwMonID,
				MAKELONG((*itMon).second->m_wSummonAttr, (*jirker).second->m_bLevel),
				0,
...
BYTE CTMapSvrModule::MonMoneyTake(CTPlayer* pPlayer, DWORD dwMonID) {
	try {
		...
		CTMonster* pMon = NULL;
		try {
			pMon = pPlayer->m_pMAP->FindMonster(dwMonID);
		} catch (...) {
			_AtlModule.LogEvent("Catch");

			return FALSE;
		}

		if (!pMon) {
			_AtlModule.LogEvent("No MON");
			return FALSE;
		}
		...
		_AtlModule.LogEvent("It used to crash.");
		return FALSE;
	}
...
void CTMapSvrModule::WrapItem(CPacket* pPacket,
	...
	(*pPacket)
		...
		<< query.m_bGem
		<< query.m_wMoggItemID
		<< query.m_dwExtValue[IEV_COMPANION]
		<< query.m_bCount
		<< query.m_bGLevel
		<< query.m_dwDuraMax
		<< query.m_dwDuraCur
		...
		<< query.m_dwExtValue[IEV_CUSTOMTEX]
		<< BYTE(TMAGIC_MAX);
...
void CTMapSvrModule::WrapItemQuery(CPacket* pPacket,
	...
	(*pPacket)
		...
		>> item.m_bGem
		>> item.m_wMoggItemID
		>> item.m_dwExtValue[IEV_COMPANION]
		>> item.m_bCount
		>> item.m_bGLevel
		>> item.m_dwDuraMax
		>> item.m_dwDuraCur
		...
		>> item.m_dwExtValue[IEV_CUSTOMTEX]
		>> bMagicCount;
...
CTItem* CTMapSvrModule::CreateItem(CPacket* pPacket) {
	...
	(*pPacket)
		...
		>> pTITEM->m_bLevel
		>> pTITEM->m_bGem
		>> pTITEM->m_wMoggItemID
		>> pTITEM->m_dwExtValue[IEV_COMPANION]
		>> pTITEM->m_bCount
		>> pTITEM->m_bGLevel
		>> pTITEM->m_dwDuraMax
		>> pTITEM->m_dwDuraCur
		...
		>> pTITEM->m_dwExtValue[IEV_CUSTOMTEX]
		>> bMagicCount;
...
CTItem* CTMapSvrModule::CreateItem(TITEMQUERY& item) {
	...
	if (pTITEM->m_pTITEM) {
		...
		pTITEM->m_bGem = item.m_bGem;
		pTITEM->m_wMoggItemID = item.m_wMoggItemID;
		...
		pTITEM->m_dwExtValue[IEV_COMPANION] = item.m_dwExtValue[IEV_COMPANION];
		pTITEM->m_dwExtValue[IEV_CUSTOMTEX] = item.m_dwExtValue[IEV_CUSTOMTEX];
...
BYTE CTMapSvrModule::CalcProb(CTPlayer* pPlayer, CTNpc* pNpc, BYTE bType, BYTE bBaseProb) {
	...
	break;
	case PROB_ITEMGUARD_MEDIUM:
	{
		wAddProb = pPlayer->HaveItemProbBuff(SDT_STATUS_MEDIUM_ITEMGUARD);
		pPlayer->EraseItemProbBuff(SDT_STATUS_MEDIUM_ITEMGUARD);
	}
	break;
	case PROB_ITEMGUARD_GREATER:
	{
		wAddProb = pPlayer->HaveItemProbBuff(SDT_STATUS_GREATER_ITEMGUARD);
		pPlayer->EraseItemProbBuff(SDT_STATUS_GREATER_ITEMGUARD);
	}
	break;
	case PROB_RAREMAGIC:
	{
	...
	for (size_t ll = 0; ll < m_vOccupation.size(); ll++) {
		...
		for (itChar = mapPlayer[pLocal->m_pZone->m_bLine].begin(); itChar != mapPlayer[pLocal->m_pZone->m_bLine].end(); itChar++) {
			...
			if (pChar->m_bGuildDuty == GUILD_DUTY_CHIEF &&
					...
					pLocal->m_dwGuild == pChar->GetGuild()) {
	...
	if (bType == OCCUPY_ACCEPT) {
		...
		for (DWORD i = 0; i < vPlayers.size(); i++) {
			...
			if (pChar->GetWarCountry() == bCountry) {
				...
				if (pChar)
					pChar->GainPvPoint(200, PVPE_GUILD, PVP_USEABLE);

			}
		}

		if (pReward)
			delete pReward; //rewhor'd? XD
	}
...
#ifdef SKYGARDEN
void CTMapSvrModule::SkygardenReward(LPTLOCAL pLocal, BYTE bType, BYTE bCountry) {
	CTMap* pTMAP = FindTMap(DEFAULT_CHANNEL, 0, pLocal->m_pZone->m_wMapID);
		...
		for (DWORD i = 0; i < vPlayers.size(); i++) {
			...
			if (pChar->GetWarCountry() == bCountry) {
				if (pReward) {
					...
					SendDM_POSTRECV_REQ(
						0, pChar->m_dwID, GetSvrMsg(NAME_OPERATOR), pChar->m_strNAME,
						BuildNetString(NAME_NULL, GetSvrMsg(MSG_SKYGARDEN_OCCUPY)), NAME_NULL, POST_PACKATE,
						0, 0, 0, pReward);
				}
				pChar->GainPvPoint(360, PVPE_WIN, PVP_USEABLE);
			} else
				pChar->GainPvPoint(360, PVPE_DEFEND, PVP_USEABLE);
...
void CTMapSvrModule::PvPEvent(BYTE bEvent, void* pObject, DWORD dwTargetID) {
	...
	case PVPE_KILL_L:
	{
		...
		if (itTeam != m_mapBRTEAM.end()) {
			for (MAPBRPLAYER::iterator itPlayer = (*itTeam).second->m_mapBRPLAYER.begin(); itPlayer != (*itTeam).second->m_mapBRPLAYER.end(); ++itPlayer) {
				if ((*itPlayer).second->m_dwCharID == pAttack->m_dwID)
					continue;

				if ((*itPlayer).second->m_bClass >= TCLASS_PRIEST)
					(*itPlayer).second->m_wBonus += 5;
			}
		}
		...
		return;
	#endif
		if (pChar->GetPartyID() && pChar->m_pMAP) {
			VPLAYER vPlayer;
			vPlayer.clear();
		...
		//	if (pChar->GetWarCountry() == m_bBonusCountry && m_bStage >= 2)
			//	dwDecPoint = 0;
		...
		if (vParty.empty()) {
			...
			while (!vParty.empty()) {
				...
				if (pLocal)
					LocalRecord(pLocal, pParty, bEvent, 12, TRUE);

				if (pAttack->m_dwID == pParty->m_dwID)
					pAttack->GainPvPoint(dwIncPoint, bEvent, PVP_TOTAL | PVP_USEABLE, &setChar);

				if (pParty->m_bMain && pParty->m_dwID != pAttack->m_dwID)

					pParty->GainPvPoint(12, bEvent, PVP_TOTAL | PVP_USEABLE, &setChar);
			}
		}
	}
...
void CTMapSvrModule::EndWar(LPTLOCAL pCastle, CTMap* pMap, BYTE bType) {
	...
	for (const auto& LocalPlayer : m_mapPLAYER) {
		if (LocalPlayer.second->m_wLocalID != pCastle->m_wLocal)
			continue;

		if (LocalPlayer.second->m_dwGuildID != dwWinGuild)
			continue;

		LocalPlayer.second->IncreaseStatExp(CASTLEOCCUPY_STATEXP / 2);
	}
}
...
#ifdef SKYGARDEN
BYTE CTMapSvrModule::IsSkyGarden(CTMap* pMap) {
	if (!pMap || !pMap->m_pSkyGarden)
		return FALSE;
...
int CTMapSvrModule::Timer(int Intervall) {
	while (1) {
		int begTime = clock();
		if (clock() - begTime > Intervall) {
			return 1;
...
void CTMapSvrModule::SetTournamentChar(CTPlayer* pPlayer) {
	if (!IsTournamentMap(pPlayer->m_wMapID) || !FindTournamentPlayer(pPlayer->m_dwID, TRUE))
		return;

	pPlayer->TournamentCharClear();
	pPlayer->m_bLevel = m_bMaxLevel;
	...
	MAPTTNMTINVEN::iterator itINVEN;
	for (itINVEN = m_mapTTNMTINVEN.begin(); itINVEN != m_mapTTNMTINVEN.end(); itINVEN++) {
		CTInven* pInven = new CTInven();

		pInven->m_bELD = (*itINVEN).second->m_bELD;
		pInven->m_bInvenID = (*itINVEN).second->m_bInven;
		pInven->m_dEndTime = (*itINVEN).second->m_dEndTime;
		pInven->m_wItemID = (*itINVEN).second->m_wItemID;
		pInven->m_pTITEM = FindTItem((*itINVEN).second->m_wItemID);

		pPlayer->m_mapTINVEN.insert(MAPTINVEN::value_type(pInven->m_bInvenID, pInven));

		if (pInven->m_bInvenID != INVEN_DEFAULT && pInven->m_bInvenID != INVEN_EQUIP)
			pPlayer->SendCS_INVENADD_ACK(INVEN_SUCCESS, pInven->m_bInvenID, pInven->m_wItemID, pInven->m_dEndTime, FALSE);
	}

	for (DWORD i = 0; i < m_vTTNMTITEM[pPlayer->m_bClass].size(); i++) {
		LPTTNMTITEM pTITEM = m_vTTNMTITEM[pPlayer->m_bClass][i];
		CTInven* pInven = pPlayer->FindTInven(pTITEM->m_bInvenID);

		if (!pInven)
			continue;

		CTItem* pItem = new CTItem();
		pItem->Copy(pTITEM->m_pItem, TRUE);


		if (pItem->m_pTITEM->m_bKind >= IK_1HAND &&
				pItem->m_pTITEM->m_bKind <= IK_BACK) {
			MAPTINVEN::iterator finder = pPlayer->m_mapTINVEN_bak.find(INVEN_EQUIP);

			if (finder != pPlayer->m_mapTINVEN_bak.end()) {

				CTItem* pActItem = NULL;
				MAPTITEM::iterator itITEM;

				bool bIsWeapon = (pItem->m_pTITEM->m_bKind >= IK_1HAND && pItem->m_pTITEM->m_bKind <= IK_SHIELD);

				for (itITEM = (*finder).second->m_mapTITEM.begin(); itITEM != (*finder).second->m_mapTITEM.end(); ++itITEM) {
					if (pItem->m_pTITEM->m_dwSlotID == (*itITEM).second->m_pTITEM->m_dwSlotID) {
						if (bIsWeapon) {
							if (pItem->m_pTITEM->m_bKind != (*itITEM).second->m_pTITEM->m_bKind)
								continue;
						}

						pActItem = (*itITEM).second;
						break;
					}
				}

				if (pActItem) {
					pItem->m_wMoggItemID = (pActItem->m_wMoggItemID) ? pActItem->m_wMoggItemID : pActItem->m_wItemID;
					pItem->m_bGradeEffect = pActItem->m_bGradeEffect;
				}
			}
		}

		pInven->m_mapTITEM.insert(MAPTITEM::value_type(pItem->m_bItemID, pItem));
		pPlayer->SendCS_ADDITEM_ACK(pItem, pTITEM->m_bInvenID);
	}

	CTInven* pInven = pPlayer->FindTInven(INVEN_EQUIP);
	MAPTINVEN::iterator finderInv = pPlayer->m_mapTINVEN_bak.find(INVEN_EQUIP);
	if (finderInv != pPlayer->m_mapTINVEN_bak.end()) {
		for (MAPTITEM::iterator itItem = (*finderInv).second->m_mapTITEM.begin(); itItem != (*finderInv).second->m_mapTITEM.end(); ++itItem) {
			if ((*itItem).second->m_bItemID >= ES_COSTUME_HAT && (*itItem).second->m_bItemID <= ES_COSTUME_RACE) {
				if (!pInven)
					continue;
				...
				pInven->m_mapTITEM.insert(MAPTITEM::value_type(pItem->m_bItemID, pItem));
				pPlayer->SendCS_ADDITEM_ACK(pItem, INVEN_EQUIP);
			}
		}
	}
	...
	pPlayer->SendCS_LEVEL_ACK(pPlayer->m_dwID, pPlayer->m_bLevel, FALSE);
	pPlayer->SendCS_HPMP_ACK(pPlayer->m_dwID, pPlayer->m_bType, pPlayer->m_bLevel, pPlayer->GetMaxHP(), pPlayer->GetMaxHP(), pPlayer->GetMaxMP(), pPlayer->GetMaxMP());
	pPlayer->SendCS_EQUIP_ACK(pPlayer);
	pPlayer->SendCS_CHARSTATINFO_ACK(pPlayer);
}
...
#if defined(BR_COMPILE_MODE)
	if (m_bBRStatus != BS_BATTLE)
		return TRUE;

	return FALSE;
#elif defined(BOW_COMPILE_MODE)
	if (m_bBOWStatus != BS_BATTLE)
		return TRUE;
...
#ifdef SKYGARDEN
	if (pPlayer->IsInSkyGarden() && !pPlayer->m_pMAP->m_pSkyGarden->m_bCanBattle)
		return TRUE;
#endif
...
void CTMapSvrModule::ArenaEnd(LPTARENA pArena, CTPlayer* pPlayer) {
	...
	for (itP = pArena->m_mapFighter[0].begin(); itP != pArena->m_mapFighter[0].end(); itP++) {
		...
		itP->second->Revival(AFTERMATH_NONE, 0, itP->second->m_bLevel);
	...
	if (!pWin && !pLose) {
		...
		if (pWin->GetPartyID() == pArena->m_wWinPartyID) {
			pArena->m_bWinCount++;
			if (pArena->m_bWinCount > pArena->m_MaxWinCount) {
				pArena->m_TopPlayer.clear();
				for (const auto& Player : pArena->m_mapFighter[pWin->m_bArenaTeam - 1]) {
					ArenaPlayer APlayer = { Player.second->m_strNAME, Player.second->m_bLevel, Player.second->m_bClass };
					pArena->m_TopPlayer.push_back(APlayer);
				}

				pArena->m_MaxWinCount = pArena->m_bWinCount;
			}
		} else
	...
	if (pArena->m_bStatus == BS_NORMAL) {
		if (m_ArenaQueue[pArena->m_wID].size()) {
			BYTE bTeam = 0;
			DWORD dwChiefID = 0;
			DWORD dwChiefKEY = 0;

			if (pArena->m_mapFighter[0].empty())
				bTeam = 1;
			else if (pArena->m_mapFighter[1].empty())
				bTeam = 2;
			else
				return;

			VDWORD vParty; vParty.clear();
			for (const auto& Member : m_ArenaQueue[pArena->m_wID].front()) {
				if (Member.second->GetPartyChiefID() == Member.second->m_dwID) {
					dwChiefID = Member.second->m_dwID;
					dwChiefKEY = Member.second->m_dwKEY;
				}

				vParty.push_back(Member.second->m_dwID);
			}

			CTPlayer* Chief = FindPlayer(dwChiefID, dwChiefKEY);
			if (!Chief) {
				m_ArenaQueue[pArena->m_wID].erase(m_ArenaQueue[pArena->m_wID].begin());
				return;
			}

			Chief->UseMoney(pArena->m_dwFee, TRUE);
			Chief->SendCS_MONEY_ACK();

			BYTE bStart = FALSE;
			if ((pArena->m_mapFighter[0].size() && bTeam == 2) || (pArena->m_mapFighter[1].size() && bTeam == 1))
				bStart = TRUE;
			...
			while (!vPlayer.empty()) {
				CTPlayer* pChar = vPlayer.back();
				vPlayer.pop_back();
				pChar->SendCS_ARENATEAM_ACK(pArena->m_wID, bTeam, vParty);

				if (pChar->GetPartyID() && pChar->GetPartyID() == Chief->GetPartyID()) {
					pChar->m_wArenaID = pArena->m_wID;
					pChar->m_bArenaTeam = bTeam;
					pArena->m_mapFighter[bTeam - 1].insert(MAPPLAYER::value_type(pChar->m_dwID, pChar));
					...
					Teleport(pChar, pArena->m_pInPos->m_wID);
				}
			}

			SendMW_ARENAJOIN_ACK(Chief->m_dwID, Chief->m_dwKEY, TRUE, vParty);
			m_ArenaQueue[pArena->m_wID].erase(m_ArenaQueue[pArena->m_wID].begin());
		}
	}
	for (DWORD i = 0; i < (DWORD) vPlayer.size(); i++) {
...
void CTMapSvrModule::UpdateCSModule() {
	...
	if (m_dwTick - m_dwNPCSModuleTick > NPROTECT_CS_UPDATETICK)	//	CS��� ������Ʈ �ð�
	{
		GGAuthUpdateTimer();
...
void CTMapSvrModule::SaveCharKill(LPTRECORDSET pRec, DWORD dwKillerID) {
	if (pRec->m_bWin) {
		DEFINE_QUERY(&m_db, CSPSaveCharKill)
			DWORD dwTargetID = FindPlayer(pRec->m_strName)->m_dwID;

		query->m_dwKillerID = dwKillerID;
		query->m_dwTargetID = dwTargetID;
		query->Call();
		UNDEFINE_QUERY()
	}
}

void CTMapSvrModule::SaveSecurecodeStats(CTPlayer* pPlayer, BYTE bType) {
	/*
		MAPTSECURE::iterator finder = m_mapTSECURE.find(pPlayer->m_dwUserID);
		if( finder != m_mapTSECURE.end() )
		{
			switch(bType)
			{
			case SC_ENABLE:
				{
					(*finder).second->m_bEnabled = TRUE;
					pPlayer->m_bSecureEnabled = TRUE;
				}
				break;
			case SC_DISABLE:
				{
					(*finder).second->m_bEnabled = FALSE;
					pPlayer->m_bSecureEnabled = FALSE;
				}
				break;
			case SC_CURUNLOCK:
				{
					(*finder).second->m_bCurUnlocked = TRUE;
					pPlayer->m_bSecureCurUnlocked = TRUE;
				}
				break;
			case SC_CURLOCK:
				{
					(*finder).second->m_bCurUnlocked = FALSE;
					pPlayer->m_bSecureCurUnlocked = FALSE;
				}
				break;
			case SC_LOCKSECURE:
				{
					if( !(*finder).second->m_bLocked )
						(*finder).second->m_dLockTime = CTime::GetCurrentTime().GetTime();

					(*finder).second->m_bLocked = TRUE;
					pPlayer->m_bSecureLocked = TRUE;
				}
				break;
			case SC_UNLOCK:
				{
					(*finder).second->m_bLocked = FALSE;
					pPlayer->m_bSecureLocked = FALSE;
				}
				break;
			}

			// Keep calm and love Syntax

			DEFINE_QUERY(&m_db, CSPSecureCodeStatus)
...
LPPREMIUMSKILL CTMapSvrModule::FindPremiumSkill(WORD wSkillID) {



	for (BYTE i = 0; i < PREMIUMSKILL_COUNT; ++i) {
		for (BYTE s = 0; s < m_vPREMIUMSKILL[i].size(); ++s) {
			LPPREMIUMSKILL pSKILL = m_vPREMIUMSKILL[i].at(s);
			if (pSKILL->m_wSkillID == wSkillID)
				return pSKILL;
		}
	}
...
#if defined(BOW_COMPILE_MODE) || defined(BR_COMPILE_MODE)
void CTMapSvrModule::BOWNotify(const CString& strMSG) {
	MAPPLAYER::iterator it;
	for (it = m_mapPLAYER.begin(); it != m_mapPLAYER.end(); it++) {
		CTPlayer* pPlayer = (*it).second;
		if (pPlayer->m_bMain && pPlayer->m_pMAP)

			pPlayer->SendCS_BOWNOTIFY_ACK(strMSG);
	}
}

void CTMapSvrModule::ClearSavedPlayers() {
	for (MAPBOWPLAYER::iterator it = m_mapBOWPLAYER.begin(); it != m_mapBOWPLAYER.end(); ++it)
		delete (*it).second;
	m_mapBOWPLAYER.clear();
	for (MAPSAVEDPLAYER::iterator itPlayer = m_mapSAVEDPLAYER.begin(); itPlayer != m_mapSAVEDPLAYER.end(); ++itPlayer)
		delete (*itPlayer).second;
	m_mapSAVEDPLAYER.clear();
}
#ifdef BR_COMPILE_MODE
void CTMapSvrModule::ClearBRTeam() {
	for (MAPBRTEAM::iterator itTeam = m_mapBRTEAM.begin(); itTeam != m_mapBRTEAM.end(); ++itTeam) {
		for (MAPBRPLAYER::iterator itPlayer = (*itTeam).second->m_mapBRPLAYER.begin(); itPlayer != (*itTeam).second->m_mapBRPLAYER.end(); ++itPlayer)
			delete (*itPlayer).second;
		(*itTeam).second->m_mapBRPLAYER.clear();
...
#endif
void CTMapSvrModule::ReceiveBOWTitle() {
	MAPDWORD mapPoints; mapPoints.clear();
	...
	for (MAPBOWPLAYER::iterator itPlayer = m_mapBOWPLAYER.begin(); itPlayer != m_mapBOWPLAYER.end(); ++itPlayer) {
		...
		if (onlineFinder != m_mapPLAYER.end())
			mapPoints.insert(MAPDWORD::value_type((*itPlayer).second->m_dwCharID, (*onlineFinder).second->m_dwPoints));
		else {
			offlineFinder = m_mapSAVEDPLAYER.find((*itPlayer).second->m_dwCharID);
			if (offlineFinder == m_mapSAVEDPLAYER.end())
				continue;

			mapPoints.insert(MAPDWORD::value_type((*itPlayer).second->m_dwCharID, (*offlineFinder).second->m_dwPoints));
		}
	}

	DWORD dwCharID = 0;
	DWORD dwMaxPoints = 0;
	for (MAPDWORD::iterator itPoints = mapPoints.begin(); itPoints != mapPoints.end(); ++itPoints) {
		if ((*itPoints).second > dwMaxPoints) {
			dwCharID = (*itPoints).first;
			dwMaxPoints = (*itPoints).second;
		}
	}
...
BOOL CTMapSvrModule::HasPlayerNoTarget(CTObjBase* pDEFEND) {
	for (BYTE m = 0; m < pDEFEND->m_vMaintainSkill.size(); ++m) {
		CTSkill* pSkill = pDEFEND->m_vMaintainSkill[m];
		for (BYTE d = 0; d < pSkill->m_pTSKILL->m_vData.size(); d++) {
			LPTSKILLDATA pData = pSkill->m_pTSKILL->m_vData[d];
...
#ifdef BR_COMPILE_MODE
LPBRPLAYER CTMapSvrModule::GetBRPlayer(DWORD dwCharID) {
	for (MAPBRTEAM::iterator itTeam = m_mapBRTEAM.begin(); itTeam != m_mapBRTEAM.end(); ++itTeam) {
		MAPBRPLAYER::iterator finder = (*itTeam).second->m_mapBRPLAYER.find(dwCharID);
		if (finder != (*itTeam).second->m_mapBRPLAYER.end())
			return (*finder).second;
	}
...
LPBRTEAM CTMapSvrModule::GetBRTeam(DWORD dwCharID) {
	for (MAPBRTEAM::iterator itTeam = m_mapBRTEAM.begin(); itTeam != m_mapBRTEAM.end(); ++itTeam) {
		MAPBRPLAYER::iterator finder = (*itTeam).second->m_mapBRPLAYER.find(dwCharID);
		if (finder != (*itTeam).second->m_mapBRPLAYER.end())
...
LPBRPLAYER CTMapSvrModule::GetBRPlayerByTeamID(DWORD dwTeamID, DWORD dwCharID) {
	MAPBRTEAM::iterator finder = m_mapBRTEAM.find(dwTeamID);
	if (finder != m_mapBRTEAM.end()) {
		MAPBRPLAYER::iterator itPlayer = (*finder).second->m_mapBRPLAYER.find(dwCharID);
		if (itPlayer != (*finder).second->m_mapBRPLAYER.end())
...
BRRAWPOS CTMapSvrModule::GetBRRandomSpawnPos() {
	BRRAWPOS pPOS;
	MAPBRSPAWNPOS::iterator finder = m_mapBRSPAWNPOS.find(m_wBRMapID);
	if (finder != m_mapBRSPAWNPOS.end()) {
		BYTE bMax = (BYTE) (*finder).second->m_vRAWPOS.size();
		BYTE bRand = rand() % bMax;

		pPOS = (*finder).second->m_vRAWPOS[bRand];
	}

	return pPOS;
}

void CTMapSvrModule::DeleteBRPlayer(DWORD dwCharID) {
	DEFINE_QUERY(&m_db, CSPDeleteBRPlayer);
	query->m_dwCharID = dwCharID;
	query->Call();
	UNDEFINE_QUERY();
}

void CTMapSvrModule::ClearBRPlayers() {
	DEFINE_QUERY(&m_db, CSPClearBRPlayers);
	query->Call();
	UNDEFINE_QUERY();
}

void CTMapSvrModule::OnBRPlayerDie(LPBRPLAYER pBRPLAYER) {
	if (!pBRPLAYER)
		return;

	CTPlayer* pPlayer = FindPlayer(pBRPLAYER->m_strNAME);
	if (!pPlayer)
		return;

	DeleteBRPlayer(pBRPLAYER->m_dwCharID);
	SendMW_LEAVEBATTLEFIELD_REQ(pPlayer->m_dwID, pPlayer->m_dwKEY);
}

void CTMapSvrModule::ReceiveBRTitle() {
	MAPDWORD mapKills; mapKills.clear();
	for (MAPBRTEAM::iterator itTeam = m_mapBRTEAM.begin(); itTeam != m_mapBRTEAM.end(); ++itTeam) {
		for (MAPBRPLAYER::iterator itPlayer = (*itTeam).second->m_mapBRPLAYER.begin(); itPlayer != (*itTeam).second->m_mapBRPLAYER.end(); ++itPlayer) {
			if (mapKills.find((*itPlayer).second->m_dwCharID) == mapKills.end())
				mapKills.insert(MAPDWORD::value_type((*itPlayer).second->m_dwCharID, (*itPlayer).second->m_wKills));
		}
	}

	DWORD dwCharID = 0;
	WORD wMaxKills = 0;
	for (MAPDWORD::iterator itKills = mapKills.begin(); itKills != mapKills.end(); ++itKills) {
		if ((*itKills).second > wMaxKills) {
			dwCharID = (*itKills).first;
			wMaxKills = (WORD) (*itKills).second;
		}
	}
	...
	UNDEFINE_QUERY()
		...
		DWORD dwTeamID = CalcWinningTeam();
	MAPBRTEAM::iterator finder = m_mapBRTEAM.find(dwTeamID);
	if (finder != m_mapBRTEAM.end()) {
		for (MAPBRPLAYER::iterator itPlayer = (*finder).second->m_mapBRPLAYER.begin(); itPlayer != (*finder).second->m_mapBRPLAYER.end(); ++itPlayer) {
...
DWORD CTMapSvrModule::CalcWinningTeam() {
	DWORD dwTeamID = 0;
	WORD wMaxKills = 0;
	WORD wTotalMaxKills = 0;

	for (MAPBRTEAM::iterator itTeam = m_mapBRTEAM.begin(); itTeam != m_mapBRTEAM.end(); ++itTeam) {
		wMaxKills = 0;
		for (MAPBRPLAYER::iterator itPlayer = (*itTeam).second->m_mapBRPLAYER.begin(); itPlayer != (*itTeam).second->m_mapBRPLAYER.end(); ++itPlayer)
			wMaxKills += (*itPlayer).second->m_wKills;

		if (wMaxKills > wTotalMaxKills) {
			wTotalMaxKills = wMaxKills;
			dwTeamID = (*itTeam).first;
		}
	}
...
void CTMapSvrModule::OnFinishSkill(CPacket* buf) {
	LPPACKETBUF pBUF = new PACKETBUF();
	pBUF->m_packet.CopyData(buf, 0);
	pBUF->m_packet.SetID(CS_FINISHSKILL_ACK);
	pBUF->m_packet.Rewind(FALSE);

	SayToBATCH(pBUF);
}

BYTE CTMapSvrModule::GetRankID(DWORD dwPoint) {
	BYTE bMaxRank = 0;
	for (VBATTLERANK::iterator it = m_vBATTLERANK.begin(); it != m_vBATTLERANK.end(); ++it) {
		if (dwPoint >= (*it)->m_dwPoints)
			bMaxRank = (*it)->m_bRank;
		else
			return (*it)->m_bRank - 1;
	}
...
void CTMapSvrModule::SaveGuildSkill(const CTPlayer* pPlayer) {
	if (!pPlayer)
		return;

	for (const auto& Skill : pPlayer->m_mapGuildSkill) {
		DEFINE_QUERY(&m_db, CSPSaveGuildSkill);

		auto itSkill = m_mapGuildSkill.find(Skill.second.m_wSkillID);
		if (itSkill == m_mapGuildSkill.end())
			continue;

		query->m_Type = itSkill->second.m_bType;
		query->m_CharID = pPlayer->m_dwID;
		query->m_GuildID = pPlayer->m_dwGuildID;
		query->m_SkillID = Skill.second.m_wSkillID;
		query->m_Level = Skill.second.m_bLevel;
		__TIMETODB(Skill.second.m_EndTime, query->m_EndTime);
...
CTPlayer* CTMapSvrModule::FindPC(DWORD ID) //Use only for trusted sources handled by the map.
{
	auto Player = m_mapPLAYER.find(ID);
	if (Player == m_mapPLAYER.end())
		return nullptr;
