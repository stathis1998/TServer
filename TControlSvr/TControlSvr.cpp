// TControlSvr.cpp : WinMainŔÇ ±¸ÇöŔÔ´Ď´Ů.
...
vector<size_t> MapServiceVector;
...
void main()
{
	...
	{
		string fieldstr;
		cout << "MAP SERVICE PORT(DEFAULT - " << "5816" << "): ";
		getline(cin, fieldstr);
		if (fieldstr.empty())
		{
			MapServiceVector.push_back(5816);
		}
		else
		{ 
			istringstream is(fieldstr);
			int i;
			while (is >> i)
			{
				MapServiceVector.push_back(i);
			}
		}
...
CTControlSvrModule::CTControlSvrModule()
	:TServerSystem("TControl", TCONTROL_VERSION)
{
...
DWORD CTControlSvrModule::LoadConfig()
{
	...
	LoadStringFromIni("DBPasswd", string("as123654"), m_szDBPasswd);
	LoadIntFromIni("ControlPort", 3616, m_wPort);
...
void CTControlSvrModule::UpdateData()
{
	...
	MAPTMANAGER::iterator itCMANAGER; // Çö˝Â·ć CTManager
	MAPTMACHINE::iterator itMACHINE;
	MAPTGROUP::iterator itGROUP;
	MAPTSVRTYPE::iterator itSVRTYPE;
	MAPPLATFORM::iterator itPLATFORM; // Çö˝Â·ć CPlatformUsage
...
{
	BYTE bManager = pSession->m_bManager; // Çö˝Â·ć CTManager
...
{
	// pSessionżˇ ´ëÇŃ ĆĐĹ¶Ăł¸®°ˇ żĎ·áµÇ´Â ˝ĂÁˇŔ» ľË¸˛
	// pSessionżˇ ´ëÇŃ żŔąö·¦ żŔĆŰ·ˇŔĚĽÇŔĚ żĎ·áµČ °ÍŔ» Č®ŔÎČÄ ČŁĂâ ÇĎż©ľß ÇÔ.
	EnterCriticalSection(&m_csBATCH);
	MAPTMANAGER::iterator finder = m_mapSESSION.find((DWORD_PTR) pSession); // Çö˝Â·ć CTManager
...
void CTControlSvrModule::QueryStatus()//new solution by agnares
{
	...
	for (it = m_mapTSVRTEMP.begin(); it != m_mapTSVRTEMP.end(); it++)
	{
		...
		if (MapServiceVector.size() > 1)
		{
			if (pService->m_pSvrType->m_bType == SVRGRP_WORLDSVR || pService->m_pSvrType->m_bType == SVRGRP_LOGINSVR)
				CT_NEWCONNECT_SEND();

			for (size_t i = 0; i < MapServiceVector.size(); i++)
			{
				if (pService->m_pSvrType->m_bType == SVRGRP_MAPSVR && pService->m_wPort == MapServiceVector[i])
				{
					CT_NEWCONNECT_SEND();
				}
			}
		...
		{
			if (pService->m_pSvrType->m_bType == SVRGRP_WORLDSVR || pService->m_pSvrType->m_bType == SVRGRP_LOGINSVR || (pService->m_pSvrType->m_bType == SVRGRP_MAPSVR && pService->m_wPort == MapServiceVector[0]))
			{
				CT_NEWCONNECT_SEND();
			}
		}
			...
			case COMP_SESSION	:
				if(pOV)
				{
					...
					case TOV_SSN_RECV	: 
						...
						break;
						// ***** IOCP »çżëąý Áß ľËľĆł»±â Čűµç ĂąąřÂ° ±¸ą® (Ľ­ąöĂř ĽĽĽÇ Áľ·á) *****
						//
						// Ľ­ąö°ˇ ¸ŐŔú closesocket()Ŕ» ČŁĂâÇĎż© ĽĽĽÇŔ» Áľ·áÇŃ °ćżěŔĚ¸ç
						// WSARecv()°ˇ ČŁĂâµČ »óĹÂżˇĽ­¸¸ ŔĚ ÄÚµĺ·Î µéľîżŔ¸ç
						// ¸đµç żŔąö·¦ żŔĆŰ·ˇŔĚĽÇŔĚ Áľ·áµČ »óĹÂŔĚ±â ¶§ą®żˇ
						// ŔĚ ĽŇÄĎ ÇÚµé°ú °ü·ĂµČ µĄŔĚĹ¸´Â IOCPĹĄżˇ ł˛ľĆŔÖÁö ľĘ´Ů.
						// µű¶óĽ­ ŔĚ ˝ş·ąµĺżˇĽ­´Â ÇŘ´ç ĽĽĽÇżˇ °ü·ĂµČ ŔŰľ÷ ¸í·ÉŔ» ´őŔĚ»ó ĽöÇŕÇĎÁö ľĘ±â ¶§ą®żˇ
						// ´Ů¸Ą ˝ş·ąµĺ°ˇ Çă¶ôÇŃ´Ů¸é ŔĚ ±¸ą®żˇĽ­ ĽĽĽÇ Ć÷ŔÎĹÍ¸¦ »čÁ¦ÇŘµµ ą«ąćÇĎ´Ů.
						// ĽĽĽÇŔ» »čÁ¦ ÇĎ´ÂµĄ °ˇŔĺ ÁÁŔş ÁöÁˇŔĚąÇ·Î ŔüĂĽ ˝Ă˝şĹŰ Ľł°č˝Ă
						// Á¤»óŔűŔÎ ĽĽĽÇ Áľ·á´Â Ľ­ąöĂřżˇĽ­ ¸ŐŔú ĽĽĽÇŔ» Áľ·á˝ĂĹ°µµ·Ď Ľł°čÇĎ´Â °ÍŔĚ ľČŔüÇĎ´Ů.
...
void CTControlSvrModule::ProcessSession( CTControlSession *pSession, DWORD dwIoBytes)
{
	...
	{
		// ***** IOCP »çżëąý Áß ľËľĆł»±â Čűµç µÎąřÂ° ±¸ą® (Ĺ¬¶óŔĚľđĆ®Ăř ĽĽĽÇ Áľ·á) *****
		//
		// Ĺ¬¶óŔĚľđĆ®°ˇ ¸ŐŔú closesocket()Ŕ» ČŁĂâÇĎż© ĽĽĽÇŔ» Áľ·áÇŃ °ćżěŔĚ¸ç
		// WSARecv()°ˇ ČŁĂâµČ »óĹÂżˇĽ­¸¸ ŔĚ ÄÚµĺ·Î µéľîżŔ¸ç
		// ¸đµç żŔąö·¦ żŔĆŰ·ˇŔĚĽÇŔĚ Áľ·á µÇľú´Ů°í şĽ Ľö ľř´Ů.
		// µű¶óĽ­ ŔĚČÄżˇ ŔĚ ˝ş·ąµĺżˇĽ­ ŔĚ ĽĽĽÇ°ú °ü·ĂµČ ŔŰľ÷¸í·ÉŔĚ ˝ÇÇŕ µÉ Ľö ŔÖŔ¸ąÇ·Î
		// ż©±âĽ­ ĽĽĽÇ Ć÷ŔÎĹÍ¸¦ »čÁ¦ÇĎ¸é Ľ­ąö°ˇ ´ŮżîµÉ Ľö ŔÖ´Ů.
		// ReceiveżÍ °ü·ĂµČ żŔąö·¦ żŔĆŰ·ąŔĚĽÇŔş Č®˝ÇČ÷ Áľ·á µÇľúŔ¸ąÇ·Î
		// SendżÍ °ü·ĂµČ żŔąö·¦ żŔĆŰ·ąŔĚĽÇŔĚ Áľ·áµÇľú´ÂÁö¸¦
		// Č®ŔÎÇŃ ČÄ ´Ů¸Ą ˝ş·ąµĺŔÇ ĽĽĽÇ »čÁ¦ Ľö¶ô°úÁ¤Ŕ» °ĹÄˇ°í ĽĽĽÇŔ» »čÁ¦ ÇŘľß ÇŃ´Ů.
	...
	while(bContinue)
	{
		...
		default					: 
			...
			OnInvalidSession(pSession); 
			return;
			// ***** IOCP »çżëąý Áß ľËľĆł»±â Čűµç ĽĽąřÂ° ±¸ą® (şń Á¤»óŔűŔÎ ĽĽĽÇ Áľ·á) *****
			//
			// Ĺ¬¶óŔĚľđĆ®°ˇ ĆĐĹ¶Ŕ» şŻÁ¶ÇŘĽ­ ş¸ł˝´Ů°ĹłŞ ł×Ć®źp żŔ·ů·Î ŔÎÇŘ ĽĽĽÇŔĚ şń Á¤»óŔűŔÎ »óĹÂ°ˇ µČ °ćżěŔĚ¸ç
			// WSARecv()°ˇ ČŁĂâµČ »óĹÂżˇĽ­¸¸ ŔĚ ÄÚµĺ·Î µéľîżŔ¸ç
			// ¸đµç żŔąö·¦ żŔĆŰ·ˇŔĚĽÇŔĚ Áľ·á µÇľú´Ů°í şĽ Ľö ľř´Ů.
			// µű¶óĽ­ ŔĚČÄżˇ ŔĚ ˝ş·ąµĺżˇĽ­ ŔĚ ĽĽĽÇ°ú °ü·ĂµČ ŔŰľ÷¸í·ÉŔĚ ˝ÇÇŕ µÉ Ľö ŔÖŔ¸ąÇ·Î
			// ż©±âĽ­ ĽĽĽÇ Ć÷ŔÎĹÍ¸¦ »čÁ¦ÇĎ¸é Ľ­ąö°ˇ ´ŮżîµÉ Ľö ŔÖ´Ů.
			...
			//
			// *** ±ÇŔĺÇĎÁö ľĘ´Â Ćíąý ***
			// Č¤˝ĂłŞ ż©±âĽ­ closesocket()Ŕ» ČŁĂâ ÇĎż©
			// Ľ­ąöĂř ĽĽĽÇÁľ·á ÇÁ·ÎĽĽ˝ş·Î µéľî°ˇ·Á´Â ˝Ăµµ´Â ÇĎÁö ľĘ´Â °ÍŔĚ ÁÁ´Ů.
			// closesocket()Ŕ» ČŁĂâÇŘµµ WSARecv()°ˇ ČŁĂâµÇÁö ľĘŔş »óĹÂŔĚ±â ¶§ą®żˇ
			// Ľ­ąöĂř ĽĽĽÇÁľ·á ÇÁ·ÎĽĽ˝ş·Î µéľî°ˇÁö ¸řÇŃ´Ů. ¸¸ľŕ WSARecv()¸¦ ¸ŐŔú
			// ČŁĂâÇĎ°í ąŮ·Î closesocket()Ŕ» ČŁĂâÇĎ¸é ÇÁ·ÎĽĽ˝ş·ÎŔÇ ÁřŔÔŔş °ˇ´É ÇŇ Ľöµµ ŔÖŔ¸łŞ
			// şń Á¤»óŔűŔÎ ĽĽĽÇŔ» ´ë»óŔ¸·Î ±×·± ľ×ĽÇŔ» ÇĎ´Â °ÍŔş Ŕ§ÇčÇĎ´Ů.
		}
	}

	if(!pSession->WaitForMessage())
	{
		// ***** IOCP »çżëąý Áß ľËľĆł»±â Čűµç ł×ąřÂ° ±¸ą® (şń Á¤»óŔűŔÎ ĽĽĽÇ Áľ·á) *****
		//
		// ł×Ć®źp żŔ·ů·Î ŔÎÇŘ ĽĽĽÇŔĚ şń Á¤»óŔűŔÎ »óĹÂżˇĽ­ WSARecv()ÇÔĽö ČŁĂâŔĚ ˝ÇĆĐÇŃ °ćżěŔĚ¸ç
		// WSARecv()°ˇ ČŁĂâµČ »óĹÂżˇĽ­¸¸ ŔĚ ÄÚµĺ·Î µéľîżŔ¸ç
		// ¸đµç żŔąö·¦ żŔĆŰ·ˇŔĚĽÇŔĚ Áľ·á µÇľú´Ů°í şĽ Ľö ľř´Ů.
		// µű¶óĽ­ ŔĚČÄżˇ ŔĚ ˝ş·ąµĺżˇĽ­ ŔĚ ĽĽĽÇ°ú °ü·ĂµČ ŔŰľ÷¸í·ÉŔĚ ˝ÇÇŕ µÉ Ľö ŔÖŔ¸ąÇ·Î
		// ż©±âĽ­ ĽĽĽÇ Ć÷ŔÎĹÍ¸¦ »čÁ¦ÇĎ¸é Ľ­ąö°ˇ ´ŮżîµÉ Ľö ŔÖ´Ů.
...
DWORD CTControlSvrModule::OnReceive( LPPACKETBUF pBUF)
{
	switch(pBUF->m_packet.GetID())
	{
        ...
        ON_RECEIVE(CT_UPDATEPATCH_REQ) // Çö˝Â·ć CT_UPDATEPATCH_REQ
		ON_RECEIVE(CT_ANNOUNCEMENT_REQ) // Çö˝Â·ć CT_ANNOUNCEMENT_REQ
		ON_RECEIVE(CT_USERKICKOUT_REQ) // Çö˝Â·ć CT_USERKICKOUT_REQ
		ON_RECEIVE(CT_USERMOVE_REQ) // Çö˝Â·ć CT_USERMOVE_REQ
		ON_RECEIVE(CT_USERPOSITION_REQ) // Çö˝Â·ć CT_USERPOSITION_REQ
		ON_RECEIVE(CT_MONSPAWNFIND_REQ) // Çö˝Â·ć CT_MONSPAWNFIND_REQ
		ON_RECEIVE(CT_MONSPAWNFIND_ACK) // Çö˝Â·ć CT_MONSPAWNFIND_ACK
		ON_RECEIVE(CT_MONACTION_REQ) // Çö˝Â·ć CT_MONACTION_REQ
		ON_RECEIVE(CT_STLOGIN_REQ) // Çö˝Â·ć CT_STLOGIN_REQ
		ON_RECEIVE(CT_PLATFORM_REQ) // Çö˝Â·ć CT_PLATFORM_REQ
		ON_RECEIVE(CT_USERPROTECTED_REQ) // Çö˝Â·ć CT_USERPROTECTED_REQ
		ON_RECEIVE(CT_CHARMSG_REQ) // Çö˝Â·ć CT_CHARMSG_REQ
...
void CTControlSvrModule::ParseStrValue(EVENTINFO& stEVENTINFO,CString str,BYTE bEventID)
{
	...
	else if( bEventID == EVENT_LOTTERY)
	{
		...
		//	ex) 1203x1-10;55x1-1;|´çĂ·|ĂŕÇĎµĺ¸ł´Ď´Ů!
