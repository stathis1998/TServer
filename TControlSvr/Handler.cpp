...
DWORD CTControlSvrModule::OnCT_OPLOGIN_REQ( LPPACKETBUF pBUF)
{
	...
	else if( !pAlreadyM )
	{
		...
		//OnCT_NEWCONNECT_REQ(pBUF);
	}
	else
	{
		SSLogEvent("Close session");
...
DWORD CTControlSvrModule::OnCT_SERVICESTAT_REQ(LPPACKETBUF pBUF)
{
	...
	//SSLogEvent(6, "TempMap_Map established");
	return EC_NOERROR;
}
...
DWORD CTControlSvrModule::OnCT_NEWCONNECT_REQ(LPPACKETBUF pBUF)//brb
{
	DWORD dwID;
	...
	if(pService && !pService->m_pConn)
	{
		...
		{
			SSLogEvent(6, "Connect Error %s, %d, %d",pService->m_pMachine->m_vPriAddr.front(), pService->m_wPort, GetLastError());
		...
		if(!hIocpWork)
		{
			SSLogEvent(6, "Iocp Error %s, %d, %d",pService->m_pMachine->m_vPriAddr.front(), pService->m_wPort, GetLastError());
			delete pService->m_pConn;
			pService->m_pConn = NULL;
		...
		{
			SSLogEvent(6, "WaitForMessage Error %s, %d, %d",pService->m_pMachine->m_vPriAddr.front(), pService->m_wPort, GetLastError());
			delete pService->m_pConn;
			pService->m_pConn = NULL;
			CloseHandle(hIocpWork);			
			return EC_NOERROR;
		...
		//SSLogEvent("New Connect %d",pService->m_dwID);
	}

	SSLogEvent(13, "CT_NEWCONNECT_REQ SUCCESS ( %d, %s )", pService->m_wPort, (LPCSTR)pService->m_strName);
...
DWORD CTControlSvrModule::OnCT_UPDATEPATCH_REQ(LPPACKETBUF pBUF)
{
	...
	LogEvent("New Patch Count: %d", wCount);
...
DWORD CTControlSvrModule::OnCT_USERKICKOUT_REQ(LPPACKETBUF pBUF)
{
	...
	SSLogEvent(8, "TCONTROL: %s kicked %s", pManager->m_strID, strUser);
	m_pDebugSocket->LogAction(strLOG);

	MAPTSVRTEMP::iterator itSvr;
	for(itSvr=m_mapTSVRTEMP.begin(); itSvr!=m_mapTSVRTEMP.end(); itSvr++)
	{
		if ((*itSvr).second->m_pConn && ((*itSvr).second->m_pSvrType->m_bType == SVRGRP_MAPSVR))
		{
			SSLogEvent(9, "Kick sent to a server!");
...
DWORD CTControlSvrModule::OnCT_USERMOVE_REQ(LPPACKETBUF pBUF)
{
	...
	for(WORD i = 0; i < wCount; i++)
	{
		...
		strLOG.Format("TCONTROL: %s moved %s", pManager->m_strID, strUser);
		m_pDebugSocket->LogAction(strLOG);
	}
...
DWORD CTControlSvrModule::OnCT_USERPOSITION_REQ(LPPACKETBUF pBUF)
{
	...
	for(WORD i = 0; i < wCount; i++)
	{
		...
		strLOG.Format("TCONTROL: %s moved %s to %s", pManager->m_strID, strMoveUser, strTargetName);
		m_pDebugSocket->LogAction(strLOG);
	}
...
DWORD CTControlSvrModule::OnCT_USERPROTECTED_REQ(LPPACKETBUF pBUF)
{
	...
	strLOG.Format("TCONTROL: %s banned %s for %d days. Permanent = %d | Reason: %s", pManager->m_strID, strUserID, dwDuration, bPermanent, strReason);
	m_pDebugSocket->LogAction(strLOG);
	
	DEFINE_QUERY(&m_db, CSPUserProtectedAdd)
	lstrcpy(query->m_szUserID, strUserID);
	lstrcpy(query->m_szReason, strReason);
	lstrcpy(query->m_szOperator, pManager->m_strID);
	query->m_dwDuration = dwDuration;
	query->m_bPermanent = bPermanent;
	query->Call();
	bRet = query->m_nRET;
	UNDEFINE_QUERY()
...
DWORD CTControlSvrModule::OnCT_CHARMSG_REQ(LPPACKETBUF pBUF)
{
	...
	strLOG.Format("TCONTROL: %s wrote %s to %s", pManager->m_strID, strMsg, strName);
	m_pDebugSocket->LogAction(strLOG);
...
DWORD CTControlSvrModule::OnCT_CHATBAN_REQ(LPPACKETBUF pBUF)
{
	...
	strLOG.Format("TCONTROL: %s banned %s 's chat for %d minutes. Reason: %s", pManager->m_strID, strName, wMin, strReason);
	m_pDebugSocket->LogAction(strLOG);
...
DWORD CTControlSvrModule::OnCT_HELPMESSAGE_REQ(LPPACKETBUF pBUF)
{
	...
	SSLogEvent(3, "CTRL MESSAGE RECV - bGroup: %d", bGroupID);
	SSLogEvent(3, "m_mapTSVRTEMP size: %d", m_mapTSVRTEMP.size());

	for(itSvr=m_mapTSVRTEMP.begin(); itSvr!=m_mapTSVRTEMP.end(); itSvr++)
	{	
		//SSLogEvent(3, "m_bType: ", sizeof((*itSvr).second->m_pSvrType->m_bType));
		if ((*itSvr).second->m_pConn == NULL)
		{
			SSLogEvent(8, "CONNECTION = NULL");
		}
		if( (*itSvr).second->m_pConn && (*itSvr).second->m_pSvrType->m_bType == (BYTE)0x03/*SVRGRP_WORLDSVR*/)
		{
			if ((*itSvr).second->m_pGroup->m_bGroupID == bGroupID || !bGroupID) 
			{
				(*itSvr).second->m_pConn->SendCT_HELPMESSAGE_REQ(&(pBUF->m_packet));
				SSLogEvent(6, "HELP MESSAGE SENT");
			}
		}
	}
