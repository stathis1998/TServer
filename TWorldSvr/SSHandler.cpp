...
DWORD CTWorldSvrModule::OnCT_CHATBAN_REQ(LPPACKETBUF pBUF)  
{
	...
	pTarget->m_nChatBanTime = CTime::GetCurrentTime().GetTime() + wMin * 60;
...
DWORD CTWorldSvrModule::OnCT_HELPMESSAGE_REQ(LPPACKETBUF pBUF)
{
	...
	SSLogEvent(3, "Message received: %d", bID);
...
DWORD CTWorldSvrModule::OnMW_BATTLEMODESTATUS_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	...
	if (pMap)
		pMap->SendMW_BATTLEMODESTATUS_ACK(dwCharID, dwKEY, m_pBOWModule, m_pBRModule);
...
DWORD CTWorldSvrModule::OnMW_CONNECT_ACK( LPPACKETBUF pBUF)
{
	...
	case BR_SERVER_ID:
		{
			if (m_pBRModule)
				m_pBRModule->ConnectMap(pSERVER);
			//LogEvent("ServerID: %d", pSERVER->m_wID);
		}
		break;
	default:
		break;
...
DWORD CTWorldSvrModule::OnMW_ADDCHAR_ACK( LPPACKETBUF pBUF)
{
	...
	if( itCHAR == m_mapTCHAR.end())
	{
		...
		pTCHAR->m_dwRewardTick = GetTickCount();
		pTCHAR->m_bRewardIndex = 0;

		if (m_pBOWModule)
		{
			LPBOWPLAYER pBOWPLAYER = m_pBOWModule->FindPlayer(dwCharID);
			if (pBOWPLAYER)
				pBOWPLAYER->m_dwKEY = dwKEY;
		}
		
		if (m_pBRModule)
		{
			LPTBRPLAYERS pBRPlayer = m_pBRModule->FindPlayerPointerInTeam(dwCharID);
			if (pBRPlayer)
...
DWORD CTWorldSvrModule::OnMW_ENTERSVR_ACK( LPPACKETBUF pBUF)
{
	...
	{
		LPBOWPLAYER pBOWPlayer = m_pBOWModule->FindPlayer(dwCharID);
		pTCHAR->m_bCountry = pBOWPlayer->m_bCountry;
		pTCHAR->m_bAidCountry = TCONTRY_N;
	}
	else
	{
	...
	pTCHAR->m_dwRankPoint = dwRankPoint;
		...
		pTCHAR->m_bCHGMainID != BR_SERVER_ID )
	{
		...
		if (pTCHAR->m_bMainID == BOW_SERVER_ID || pTCHAR->m_bMainID == BR_SERVER_ID)
		{
...
DWORD CTWorldSvrModule::OnMW_TELEPORT_ACK( LPPACKETBUF pBUF)
{
	...
	if(!pTCHAR)
	{
		...
		CTBowSystem::Log("No Char > ServerID");
	...
	if(!pTMAP)
	{
		...
		pSERVER->SendMW_TELEPORT_REQ(
			...
			TPR_NODESTINATION);
		CloseChar(pTCHAR);
		CTBowSystem::Log("No Map > ServerID");
...
DWORD CTWorldSvrModule::OnMW_CASTLEOCCUPY_ACK(LPPACKETBUF pBUF)
{
	...
	if (pGuild) {
		...
		pGuild->m_dwStatExp += CASTLEOCCUPY_STATEXP;
		SaveGuildStats(pGuild);
	}
...
DWORD CTWorldSvrModule::OnMW_LOCALOCCUPY_ACK(LPPACKETBUF pBUF)
{
	...
	if (pGuild)
	{
		pGuild->m_dwStatExp += LOCALOCCUPY_STATEXP;
		SaveGuildStats(pGuild);
	}
...
DWORD CTWorldSvrModule::OnMW_CREATESPOLECNIKMON_ACK(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKey;
	DWORD dwMonID;
	...
	for(itCON =pChar->m_mapTCHARCON.begin(); itCON != pChar->m_mapTCHARCON.end(); itCON++)
	{
		if((*itCON).second->m_bValid)
		{
			...
			if(pMAP)
			{
				pMAP->SendMW_CREATESPOLECNIKMON_REQ(
					dwCharID,
					dwKey,
					dwMonID,
					wMon,
...
DWORD CTWorldSvrModule::OnMW_SPOLECNIKMONDEL_ACK(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKey;
	DWORD dwMonID;
	...
	for(itCON =pChar->m_mapTCHARCON.begin(); itCON != pChar->m_mapTCHARCON.end(); itCON++)
	{
		if((*itCON).second->m_bValid)
		{
			...
			if(pMAP)
			{
				pMAP->SendMW_SPOLECNIKMONDEL_REQ(
					dwCharID,
					dwKey,
					dwMonID,
					bForever);
...
DWORD CTWorldSvrModule::OnMW_BEGINTELEPORT_ACK(LPPACKETBUF pBUF)
{
	...
	if(!pTCHAR)
	{
		...
		CTBowSystem::Log("No Char > SameChann");
	...
	case MW_TOURNAMENTAPPLY_ACK:
		{
			...
			TournamentApply(pChar, pServer, bEntryID, strHWID, dwIPAddr);
		}
		break;
...
DWORD CTWorldSvrModule::OnDM_TOURNAMENTAPPLY_REQ(LPPACKETBUF pBUF)
{
	...
	lstrcpy(query->m_szHWID, LPCSTR(strHWID));
...
DWORD CTWorldSvrModule::OnMW_ADDTOBOWQUEUE_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	...
	BYTE bCountry = TCONTRY_PEACE;
	if(pTCHAR->m_bCountry > TCONTRY_C)
		bCountry = pTCHAR->m_bAidCountry;
	else
		bCountry = pTCHAR->m_bCountry;

	BYTE bResult = m_pBOWModule->AddPlayerToQueue(dwCharID,	dwKEY, bCountry, pTCHAR->m_pTactics ? pTCHAR->m_pTactics->m_dwID : pTCHAR->m_pGuild ? pTCHAR->m_pGuild->m_dwID : 0);
	pServer->SendMW_ADDTOBOWQUEUE_ACK(
		bResult,
		pTCHAR->m_dwCharID, 
		pTCHAR->m_dwKEY, 
		m_pBOWModule->m_dwTick);

	return EC_NOERROR;
}

DWORD CTWorldSvrModule::OnMW_CANCELBOWQUEUE_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	...
	LPTCHARACTER pTCHAR = FindTChar(dwCharID,
		dwKEY);

	if (!pTCHAR || !m_pBOWModule)
		return EC_NOERROR;

	BYTE bResult = m_pBOWModule->DeletePlayerFromQueue(dwCharID, dwKEY);
	if (bResult == BOWREG_FAIL && m_pBRModule) {
		
	/*	 m_pBRModule->AddPlayerToQueue(
			rand() % 6584, 
			rand() % 65877, 
			0,
			"BOT");
	*/	 
		bResult = m_pBRModule->ErasePlayerFromQueue(dwCharID, dwKEY);
	}
	CTServer *pServer = FindMapSvr(pTCHAR->m_bMainID);
	if (pServer)
		pServer->SendMW_CANCELBOWQUEUE_ACK(
			bResult, 
			pTCHAR->m_dwCharID, 
			pTCHAR->m_dwKEY,
			max(m_pBOWModule->m_dwTick, m_pBRModule->m_dwTick));

	return EC_NOERROR;
}

DWORD CTWorldSvrModule::OnMW_BOWPOINTSUPDATE_REQ(LPPACKETBUF pBUF)
{
	BYTE bCountry;
	...
	if (m_pBOWModule)
		m_pBOWModule->UpdatePoints(bCountry);

	return EC_NOERROR;
}

DWORD CTWorldSvrModule::OnMW_LEAVEBATTLEFIELD_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	...
	if (pCHAR->m_bChannel == BR_SERVER_ID && m_pBRModule)
		m_pBRModule->ReleaseSinglePlayer(dwCharID, dwKEY);
	else if (pCHAR->m_wMapID == BOW_MAP_ID && m_pBOWModule)
		m_pBOWModule->ReleaseSinglePlayer(dwCharID, dwKEY);

	return EC_NOERROR;
}

DWORD CTWorldSvrModule::OnMW_ADDTOBRQUEUE_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	BYTE bOnlyReady;
	...
	{
		BYTE bResult = FALSE;
		if (m_pBRModule->GetPremadePlayerCountByChief(dwCharID) > 0)
			bResult = m_pBRModule->FlagTeamReady(dwCharID);
		else
			bResult = m_pBRModule->FlagPlayerReady(dwCharID, dwKEY);

		if (bResult)
			UpdateBRTeam(m_pBRModule->GetChiefIDByMateID(dwCharID));

		return EC_NOERROR;
	}

	BYTE bResult = m_pBRModule->AddPlayerToQueue(
		dwCharID, 
		dwKEY, 
		pTCHAR->m_bClass,
		pTCHAR->m_strNAME);

	pServer->SendMW_ADDTOBRQUEUE_ACK(
		bResult,
		pTCHAR->m_dwCharID, 
		pTCHAR->m_dwKEY, 
		m_pBRModule->m_dwTick);

	return EC_NOERROR;
}

DWORD CTWorldSvrModule::OnMW_BRTEAMMATEADD_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	CString strNAME;
	...
	LPTCHARACTER pMATE = FindTChar(strNAME);
	if (!pMATE)
	{
		pServer->SendMW_BRTEAMMATEADD_ACK( 
			dwCharID, 
			dwKEY, 
			TEAMADD_NOTFOUND);
	...
	pServer = FindMapSvr(pMATE->m_bMainID);
	if (pServer)
	{
		pServer->SendMW_BRTEAMMATEADD_ACK(
			pMATE->m_dwCharID, 
...
DWORD CTWorldSvrModule::OnMW_BRTEAMMATEDEL_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	CString strNAME;
	...
	LPTCHARACTER pChar = FindTChar(dwCharID, dwKEY);
	LPTCHARACTER pEraseChar = FindTChar(strNAME);
	if (!pChar || !pEraseChar || !m_pBRModule)
		return EC_NOERROR;

	if (m_pBRModule->GetPremadePlayerCountByChief(pChar->m_dwCharID) ||
		pChar == pEraseChar)
	{
		m_pBRModule->ErasePlayerFromPremade(pEraseChar->m_dwCharID);
	}
	
	return EC_NOERROR;
}

DWORD CTWorldSvrModule::OnMW_BRTEAMMATEADDRESULT_ACK(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	...
	switch (bResult)
	{
	case TEAMADD_SUCCESS:
		{
			if (!m_pBRModule)
				return EC_NOERROR;

			LPTCHARACTER pMATE = FindTChar(dwCharID, dwKEY);
			LPTCHARACTER pCHIEF = FindTChar(strNAME);
			if (!pMATE || !pCHIEF)
				return EC_NOERROR;

			if (m_pBRModule->FindPlayerInPremade(pMATE->m_dwCharID))
			{
				CTServer* pServer = FindMapSvr(pCHIEF->m_bMainID);
				if (pServer)
				{
					pServer->SendMW_BRTEAMMATEADD_ACK(
						pCHIEF->m_dwCharID, 
						pCHIEF->m_dwKEY, 
						TEAMADD_ALREADYINTEAM);
				}

				return EC_NOERROR;
			}

			if (m_pBRModule->GetPremadePlayerCountByChief
				(pCHIEF->m_dwCharID) + 1 > BR_TEAMMATE_MAX_COUNT(BR_3V3))
			{
				CTServer* pServer = FindMapSvr(pCHIEF->m_bMainID);
				if (pServer)
				{
...
DWORD CTWorldSvrModule::OnMW_VOTEFORBRMAP_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	...
	{
		if (strMap != NAME_NULL)
			m_pBRModule->VoteForMap(pCHAR->m_dwUserID, strMap);
		else if (Mode != (BYTE)-1)
			m_pBRModule->VoteForMode(pCHAR->m_dwUserID, Mode);
	}
...
DWORD CTWorldSvrModule::OnMW_CMTELEPORTBATTLEMODE_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	BYTE bType;
	...
	{
		if (m_pBOWModule)
			m_pBOWModule->AddPlayerToQueue(dwCharID, dwKEY, TCONTRY_C, pCHAR->m_pGuild ? pCHAR->m_pGuild->m_dwID : 0, TRUE);
	}
	break;
	case SYSTEM_BR:
	{
...
DWORD CTWorldSvrModule::OnMW_GUILDSKILLACTION_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	BYTE Action;
	...
	Param = new WORD[ParamSize];

	for (BYTE i = 0; i < ParamSize; ++i)
		pBUF->m_packet >> Param[i];

	auto Char = FindTChar(dwCharID, dwKEY);
	if (!Char || !Char->m_pGuild)
		return EC_NOERROR;

	auto Guild = Char->m_pGuild;
	BOOL SendAll = FALSE;
	...
	if (SendAll)
	{
		for (const auto& Member : Guild->m_mapTMember)
		{
			if (!Member.second->m_pChar)
				continue;

			CTServer* pMap = FindMapSvr(Member.second->m_pChar->m_bMainID);
			if (!pMap)
				continue;

			pMap->SendMW_GUILDSKILLACTION_ACK(Member.second->m_pChar->m_dwCharID, Member.second->m_pChar->m_dwKEY, Action, Param);
		}
	}
	else
	{
		...
		pMap->SendMW_GUILDSKILLACTION_ACK(dwCharID, dwKEY, Action, Param);
	}

	SaveGuildStats(Guild);
...
DWORD CTWorldSvrModule::OnMW_UPDATEGUILDCOOLDOWN_ACK(LPPACKETBUF pBUF)
{
	DWORD dwGuildID;
	WORD wSkillID;
	BYTE bLevel;
	...
	auto Guild = FindTGuild(dwGuildID);
	if (!Guild)
		return EC_NOERROR;
	...
	for (const auto& Member : Guild->m_mapTMember)
	{
		...
		pMap->SendMW_UPDATEGUILDCOOLDOWN_REQ(Member.second->m_pChar->m_dwCharID, Member.second->m_pChar->m_dwKEY, wSkillID, bLevel, DidRenew, DidUse);
	}

	if (DidUse && !DidRenew)
	{
		for (const auto& Map : m_mapSERVER)
			Map.second->SendMW_ADDCOOLDOWN_REQ(dwGuildID, wSkillID);
	}
