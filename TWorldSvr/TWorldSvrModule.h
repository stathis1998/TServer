...
class CTWorldSvrModule : public CAtlServiceModuleT< CTWorldSvrModule, IDS_SERVICENAME >, public TServerSystem
{
public:
	MAPTSERVER GetMapSvr()
	{
		return m_mapSERVER;
	}
...
protected:
	...
	VECTORDWORD m_vCUSTOMTIMES;

	TBATTLETIME m_battletime[BT_COUNT];

	CTServer * m_pRelay;
	CTServer * m_pCtrlSvr;
	CTBowSystem * m_pBOWModule;
	CBRSystem* m_pBRModule;
...
public:
	...
	void TeleportBOWPlayer(const MAPBOWPLAYER& mapBOWPlayers, BOOL bTeleportIn, BYTE bWinner = TCONTRY_N);
	void TeleportBOWPlayer(DWORD dwCharID, DWORD dwKEY, BYTE bWinner = TCONTRY_N);
	void BOWNotify(BYTE bStatus, DWORD dwSecond, BYTE bDefugelPoints, BYTE bCraxionPoints);
	void NotifyBOWNonQueuedPlayers();
	void CheckHourlyReward();

	void BRNotify(BYTE bStatus, DWORD dwSecond, BYTE bType);
	void UpdateBRTeam(DWORD dwChiefID);
	void RemoveFromBRTeam(DWORD dwCharID);
	void TeleportBRTeam(const MAPBRTEAMS & mapTeams, BOOL TeleportIn);
	BOOL IsSomeoneAliveInTeam(LPTBRTEAMS pTEAM);
	void TeleportBRPlayer(DWORD dwCharID, DWORD dwKEY);
	void RecalcCountryBalance();
...
protected:
	...
	BYTE FindTNMTPlayerApply(CString strHWID, DWORD dwIPAddr);
	...
	void TournamentApply(LPTCHARACTER pChar, CTServer * pServer, BYTE bEntryID, CString strHWID, DWORD dwIPAddr);
...
protected:
	...
	DWORD OnMW_CREATESPOLECNIKMON_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_SPOLECNIKMONDEL_ACK(LPPACKETBUF pBUF);
...
//	DWORD OnMW_TERMINATE_ACK(LPPACKETBUF pBUF);
	...
	DWORD OnMW_ADDTOBOWQUEUE_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_CANCELBOWQUEUE_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_BOWPOINTSUPDATE_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_LEAVEBATTLEFIELD_REQ(LPPACKETBUF pBUF);

	DWORD OnMW_ADDTOBRQUEUE_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_BRTEAMMATEADD_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_BRTEAMMATEDEL_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_BRTEAMMATEADDRESULT_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_VOTEFORBRMAP_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_GUILDSKILLACTION_REQ(LPPACKETBUF pBUF);
	DWORD OnMW_UPDATEGUILDCOOLDOWN_ACK(LPPACKETBUF pBUF);
	DWORD OnMW_BATTLEMODESTATUS_REQ(LPPACKETBUF pBUF);
...
// APEX
	...
	DWORD OnMW_CMTELEPORTBATTLEMODE_REQ(LPPACKETBUF pBUF);
...
void SendDM_TOURNAMENTAPPLY_REQ(BYTE bAdd, DWORD dwCharID, BYTE bEntry=0, DWORD dwChiefID = 0, CString strHWID = 0, DWORD dwIPAddr = 0);
